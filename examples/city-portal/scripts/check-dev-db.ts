...
const defaultDatabaseUrl =
	"postgresql://cityportal:cityportal_dev_password@localhost:5433/cityportal";
...
async function main() {
	...
	for (let attempt = 1; attempt <= maxAttempts; attempt++) {
		...
		try {
			...
			await sql.close();
			console.log(
				`[city-portal] Postgres ready at ${describeTarget(databaseUrl)}`,
			);
			return;
		} catch (error) {
			lastError = error;
	...
	console.error(
		[
			`[city-portal] Cannot connect to Postgres at ${describeTarget(databaseUrl)}.`,
