...
export const gentleAzureEagle20260206T174642: Migration = {
	...
	"created_at" timestamp DEFAULT now() NOT NULL
		...
		await db.execute(sql`CREATE UNIQUE INDEX "barbers_slug_unique" ON "barbers" ("slug");`)
		await db.execute(sql`CREATE UNIQUE INDEX "barbers_email_unique" ON "barbers" ("email");`)
		...
		await db.execute(sql`CREATE UNIQUE INDEX "pages_slug_unique" ON "pages" ("slug");`)
	...
	async down({ db }) {
		await db.execute(sql`ALTER TABLE "barbers_i18n" DROP CONSTRAINT IF EXISTS "barbers_i18n_parent_id_barbers_id_fkey";`)
		await db.execute(sql`ALTER TABLE "services_i18n" DROP CONSTRAINT IF EXISTS "services_i18n_parent_id_services_id_fkey";`)
		...
		await db.execute(sql`ALTER TABLE "reviews_i18n" DROP CONSTRAINT IF EXISTS "reviews_i18n_parent_id_reviews_id_fkey";`)
		await db.execute(sql`ALTER TABLE "pages_i18n" DROP CONSTRAINT IF EXISTS "pages_i18n_parent_id_pages_id_fkey";`)
		await db.execute(sql`ALTER TABLE "site_settings_i18n" DROP CONSTRAINT IF EXISTS "site_settings_i18n_parent_id_site_settings_id_fkey";`)
