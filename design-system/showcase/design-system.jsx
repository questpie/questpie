		...
		<section style={{ padding: "80px 24px 56px" }}>
			<div style={{ maxWidth: 1120, margin: "0 auto" }}>
				...
				<h1
					style={{
						marginTop: 16,
						fontSize: "clamp(34px, 2.2rem + 1.4vw, 48px)",
						lineHeight: 1.05,
				...
				>
					Tokens, primitives and recipes used across Framework admin, docs,
					Cloud and Autopilot. Tailwind + shadcn ready handoff at the bottom.
				</p>
				<div
...
function Foundations() {
	return (
		...
		>
			...
			<div className="depth-surface" style={{ padding: 0, overflow: "hidden" }}>
				<div
					style={{
						display: "grid",
						gridTemplateColumns: "200px 1fr 80px",
				...
				<TokenRow
					k="--motion-base"
					v="150ms cubic-bezier(0.2,0,0,1)"
					sample={
						<span
							className="mono"
		...
		>
			<div className="depth-surface" style={{ padding: "28px 24px" }}>
				...
				>
					...
					>
						Body / lede. text-wrap pretty, 1.55–1.6 line height, max ~580px so
						it doesn't sprawl on wide screens.
					</p>
					<code
						className="mono"
						style={{ fontSize: 11, color: "var(--foreground-subtle)" }}
				...
				>
					<span style={{ fontSize: 14, color: "var(--foreground)" }}>
						Body 14 — UI text inside controls and rows.
					</span>
					<code
						className="mono"
						style={{ fontSize: 11, color: "var(--foreground-subtle)" }}
	...
	return (
		...
		>
			<div
				...
				className="ds-buttons-grid"
			>
				{[
					{
						variant: "primary",
						cls: "btn-primary",
						note: "Brand CTA. Linear gradient + inset highlight + brand glow + bottom shadow. translateY(1px) on press.",
					},
					{
						variant: "secondary",
						cls: "btn-secondary",
						note: "Quiet action. Hairline border + soft inset highlight + 1px lift. Same press feedback.",
					},
					{
						variant: "ghost",
						cls: "btn-ghost",
						note: "Transparent. Hover applies surface tint only. No depth.",
					},
				].map((b) => (
					<div
						key={b.variant}
			...
			>
				...
				· inset top highlight (4–22% white) + inset bottom shade (8–18% black) +
				outer 2–6px brand-tinted glow. Press state swaps to inset shadow,
...
function InputsSection() {
	...
	return (
		...
		>
			<div
				className="depth-surface ds-inputs-grid"
				style={{
					padding: 24,
					display: "grid",
			...
			>
				<Field
					label="Text input"
					hint="Default state. 40px height, 12px radius."
				>
					<input placeholder="you@company.com" style={inputStyle} />
				</Field>
				<Field
					label="Filled + focus"
					hint="Focus uses --border-strong (not brand)."
				>
					<input
...
function Field({ label, hint, children }) {
	return (
		<div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
			...
			{children}
			{hint && (
				<span style={{ fontSize: 11.5, color: "var(--foreground-subtle)" }}>
					{hint}
				</span>
			)}
		</div>
			...
			>
				<div className="panel" style={{ padding: 18 }}>
					...
					<p style={{ fontSize: 13.5, color: "var(--foreground)" }}>
						border-subtle on var(--card), radius 14, no shadow.
					</p>
				</div>
				<div className="panel-low" style={{ padding: 18 }}>
					...
					<p style={{ fontSize: 13.5, color: "var(--foreground)" }}>
						For grouped content on the page background.
					</p>
				</div>
				<div className="depth-surface" style={{ padding: 18 }}>
					...
					<p style={{ fontSize: 13.5, color: "var(--foreground)" }}>
						Elevated CTA / modal. Subtle gradient + inset highlight + drop.
					</p>
				</div>
			</div>
...
function CodeChrome() {
	const lines = [
		[
			window.KW("export const "),
			window.TYP("posts"),
			" = ",
			window.FN("collection"),
			"(",
			window.STR('"posts"'),
			")",
		],
	...
	return (
		...
		>
			...
			>
				...
				>
					<window.Eyebrow dot color="var(--primary)">
						Coming Q4 2026
					</window.Eyebrow>
					<h3
						style={{
					...
					>
						CTA recipe: depth-surface + eyebrow + balanced headline + lede +
						primary button.
					</p>
				...
				<div className="panel" style={{ padding: 20 }}>
					<window.Icon
						name="bolt"
						size={16}
						style={{ color: "var(--pillar-framework)", marginBottom: 12 }}
					/>
					...
					>
						Flat panel + accent icon + title + 1–2 line description. Live in
						2/3/4-col hairline grids.
					</p>
				</div>
				...
				<div className="panel" style={{ padding: 0, overflow: "hidden" }}>
					...
					{["Publish Q3 launch", "Fix DNS edge case", "Weekly newsletter"].map(
						(t, i) => (
							<div
								key={t}
								style={{
...
function HandoffSection() {
	return (
		...
		>
			<HandoffBlock
				filename="app.css — tokens + @theme inline"
				code={TAILWIND_V4_CSS}
			/>
			<div style={{ height: 14 }} />
			<HandoffBlock
				filename="app.css — shadcn token mapping"
				code={SHADCN_TOKENS}
			/>
			<div style={{ height: 14 }} />
			<HandoffBlock
				filename="components/ui/button.tsx — shadcn, neutral flat"
				code={SHADCN_BUTTON}
			/>
			...
			>
				<strong style={{ color: "var(--foreground)" }}>
					Three blocks, one CSS file.
				</strong>{" "}
				Tokens at the top, @theme inline in the middle, shadcn aliases at the
				bottom. Restart dev, theme works in light + dark, primary stays reserved
				for CTAs and brand marks.
			</div>
		</SectionBlock>
	);
}
