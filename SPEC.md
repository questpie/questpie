...
program: effect-free `normalize` and `validate` with no generated effect
capability, Policy-aware transactional `check`, and pre-commit bounded
`afterWrite`. Collections raise payloadless issues; each named Mutation
explicitly maps every transitively reachable issue to its own declared error.
`ctx.now` is the transaction-stable clock, and
