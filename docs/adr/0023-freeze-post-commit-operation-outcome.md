...
- Status: Accepted
- Date: 2026-08-16
- Supersedes: only the incomplete post-commit outcome edge of ADR-0014
...
- The carrier protocol and media type remain version 1. The required
  `wireDigest` selects the exact Operation Wire contract. A v1 digest receives
  a v1-readable uncorrelated `CLIENT_OUTDATED` rejection before Context
  Resolution or Operation execution. A v1 Mutation is never executed and then
  answered with a v2-only frame.
...
exact call without an automatic retry loop. Old clients fail before execution.
The revision adds no authoring grammar, database schema, provider seam, host
adapter, or public transaction handle.
