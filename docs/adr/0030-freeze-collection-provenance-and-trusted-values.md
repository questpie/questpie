...
Every Collection owns one generated internal CRUD kernel. It exists whether or
not an application publishes any Operation. Only named Operations with explicit
network exposure are callable by a client; the internal kernel is not a runtime
dispatcher or an automatic public CRUD API.
...
Unknown caller keys fail before PostgreSQL. Adding a Field intentionally changes
...
present in both lanes. `values` may use Mutation input, the locked current row,
transaction-scoped Policy-aware Collection reads, `ctx.now`, `ctx.callId`, and
deterministic ordinary TypeScript.

The lanes merge inside the owning Mutation transaction. `values` bypasses only
caller Field authority. The complete candidate still passes normalization,
validation, full candidate Policy, PostgreSQL constraints, selection, output
authority, receipt, and Change Ledger capture. Policy never supplies or rewrites
a value. A trusted assignment that forges Tenant, requester, ownership, or any
other invariant is denied by the same candidate Policy as caller data.
...
- Named Mutations can express domain transitions without reimplementing CRUD.
...
This decision narrowly supersedes ADR-0011's `defineCollectionOperations` as
the permanent CRUD authoring model and extends its server-values ordering with
Field provenance plus the trusted `values` lane. It preserves ADR-0011's fixed
Mutation ownership, candidate Policy, exact codecs, constraints, receipts,
post-commit semantics, and prohibition on raw transaction authority. It does
not supersede ADR-0011's lifecycle decision in this slice.
