...
| Basic MCP                                                        | compiler and Runtime ingress               | canonical catalogue/binding and `POST /_questpie/mcp`                  | current-protocol MCP client         | schema/outcome parity, Policy hostiles, cancellation, no retry/fallback        | accepted; MCP-01 through MCP-03 implementation pending               |
...
| Aggregate release                                                | release tooling                            | two same-version tarballs and release manifest                         | relocated clean consumer            | PostgreSQL/browser, `quality:release`, two byte-identical dry-runs             | two-package graph complete; beta.2 remains blocked by MCP and skill  |
...
MCP production implementation and the public skill remain downstream work. No
push, tag, publish, or deployment occurred.
