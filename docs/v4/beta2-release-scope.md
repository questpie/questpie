...
the static-schedule and Mutation-checkpoint extension accepted by
[ADR-0043](../adr/0043-freeze-static-job-schedules-and-mutation-checkpoints.md)
and native React Query integration accepted by
[ADR-0044](../adr/0044-native-react-query-integration.md).
Rows marked complete record that baseline; they do not establish final
verification of the extended candidate. ADR-0043 is Accepted; aggregate
ADR-0039 remains Proposed.
...
| Native React Query integration                                   | `questpie/react-query`                     | generated native options, optional React/TanStack peers, identity bootstrap | Team Support Desk and native Start  | lifetime, invalidation, Suspense/infinite, SSR/hydration, credential retirement, packed tutorials                  | ADR-0044 accepted; golden and Start evidence pass; packed/docs closure in progress |
...
| Public beta.2 documentation                                      | docs application                           | versioned release inventory and finished guides                             | human and agent readers             | links, snippets, package existence, docs typecheck/build                                                           | schedule projection reviewed and verified; final aggregate verification pending    |
| Aggregate release                                                | release tooling                            | two same-version tarballs and release manifest                              | relocated clean consumer            | PostgreSQL/browser, `quality:release`, two byte-identical dry-runs                                                 | implementation complete; final integrated gates and ADR-0039 PASS pending          |
...
guide and skill guidance. The extended beta.2 scope still needs a fresh
...
acknowledgement rules belong to Accepted ADR-0043.

The public [scheduled-Job guide](../../apps/docs/content/docs/v4/scheduled-jobs.mdx)
projects the accepted subset from the retained internal draft. Its exact source
fences compile against the reference application. Independent documentation
reviews and the affected documentation checks are complete, as recorded in the
[projection evidence](./implementation/static-schedule-public-projection/EVIDENCE.md).
Final release checks and the latest-release projection still wait for
ADR-0039 PASS.

The accepted package-identity inventory for the beta.2 candidate is `questpie`, including
`questpie/react-query`, and `questpie-opentelemetry`. Both packages advance at the
