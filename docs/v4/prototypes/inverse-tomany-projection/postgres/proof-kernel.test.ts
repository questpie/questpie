...
postgresTest(
	"one statement preserves Policy-before-limit, empty arrays, disclosure, and the root sentinel",
	async () => {
		assertChildOrderDisclosure({
...
postgresTest(
	"a conditional or unselected child order Field is refused before PostgreSQL",
	async () => {
		let dispatches = 0;
		const execute = async (
...
postgresTest(
	"the decoded root cursor order boundary participates in the same statement",
	async () => {
...
postgresTest(
	"PostgreSQL cancellation and deadline abort the whole read-only snapshot",
	async () => {
...
postgresTest(
	"decoder rejects forged ordinals, row overflow, cancellation, and byte overflow",
	() => {
		const row = {
			root_id: "t1",
			root_title: "first",
