...
| Nothing schedules on it — **falsified by BETA-10**          | `packages/runtime/src/durable/postgres-kernel.ts:365`, `:378`                    | true at this record's base, false now: `tenant_id` was projection-only and admission was `ORDER BY available_at, run_id`. BETA-10 partitions on it      |
...
| ----------------------------- | --------------------------------------------- | --------------------------------------------------- | ------------------------------------------------------------ |
| Durable admission share       | the claim predicate, `postgres-kernel.ts:357` | nothing — no caller is present                      | **shipped in BETA-10** as a scheduling change, not a refusal |
