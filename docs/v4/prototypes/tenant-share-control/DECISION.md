...
| Nothing schedules on it                                     | `packages/runtime/src/durable/postgres-kernel.ts:123`, `:463`                    | `tenant_id` is selected for projection only; admission is `ORDER BY available_at, run_id` with no tenant term                                           |
...
| ----------------------------- | --------------------------------------------- | --------------------------------------------------- | ---------------------------------------------------------- |
| Durable admission share       | the claim predicate, `postgres-kernel.ts:463` | nothing — no caller is present                      | **decide now**: a scheduling change, not a refusal         |
