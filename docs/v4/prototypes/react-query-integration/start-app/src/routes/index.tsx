...
function Page() {
	...
	useEffect(() => {
		if (data?.title !== "Finite SSR task") return;
	...
	return (
		<main>
			...
			<p id="finite">{data === null ? "Task removed by Policy" : data.title}</p>
			<Duplicate />
...
function Duplicate() {
	...
	return <p id="duplicate">{data === null ? "No visible task" : data.title}</p>;
}

function Streamed() {
	const { owner } = Route.useRouteContext();
	const { data } = useSuspenseQuery(
		owner.api.queries["tasks.detail"].options({ id: streamedId }),
	);
	...
	useEffect(() => {
		if (!clicked || data?.title !== "Streamed SSR task") return;
