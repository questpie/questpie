...
const peer = createPeer({
	result: (id, request) => ({
		id,
		title: isNew(request) ? "New credential task" : "Old credential task",
	...
	async fetch(request) {
		...
		if (url.pathname === "/_questpie/query/tasks.detail") {
			...
			return reply(request, {
				id,
				title: "Old credential task",
				updatedAt: "2026-09-08T10:00:00.000Z",
			});
		}
		if (url.pathname === "/_questpie/mutation/tasks.transition") {
...
try {
	...
	assert.equal(result.title, "New credential task");
