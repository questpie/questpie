...
const owner = `questpie-r5-${crypto.randomUUID()}`;
const temporary = await mkdtemp("/home/drepkovsky/code/questpie-r5-");
...
try {
	...
	const child = Bun.spawn(
		[
			"bun",
			"test",
			join(import.meta.dir, "collaboration-native.test.ts"),
			"--timeout",
			"120000",
		],
		{
			...
			env: {
				...
				QUESTPIE_R5_CONTAINER: container,
			},
			stdout: "inherit",
			stderr: "inherit",
		},
