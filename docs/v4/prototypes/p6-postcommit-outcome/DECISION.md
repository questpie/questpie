...
- The carrier protocol and media type remain version 1; the required
  `wireDigest` selects the exact wire contract. A v1 digest is rejected with a
  v1-readable uncorrelated `CLIENT_OUTDATED` frame before Context Resolution or
  Operation execution. The server never executes a v1 Mutation and then emits
  a v2-only failure.
