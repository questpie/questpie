...
| `drainRuntime`                                            | **absent by that name.** `beginDrain()` is a lifecycle method; in the compiled application it is reachable only through `close()` | `packages/runtime/src/durable/worker.ts:270`, driven at `packages/runtime/src/application/index.ts:592` and `packages/compiler/src/runtime/application.ts:493` |
