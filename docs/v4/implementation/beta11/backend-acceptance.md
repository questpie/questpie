...
also require an active permit (`:73`–`:110`).
...
the Reaction projection, the complete text parameter codec, absence of RLS
claims, and absence of collaboration names from every generated artifact
(`tests/integration/beta11-archive.test.ts:13`–`:112`).
...
(`tests/integration/postgres/beta11-archive.test.ts:283`–`:315`). A Context for
the foreign Institution cannot disclose its seeded Record (`:314`–`:315`). The
generated client opens a watch, the generated direct Mutation deposits N-003,
and the watch observes that append (`:317`–`:379`). The application then closes
before durable polling, a fresh generated application claims the retained run,
and the final Query observes provenance sequences 1 and 2 (`:381`–`:411`).
...
(`docs/v4/implementation/beta11/cross-fixture-conformance-report.md:1`–`:48`).
...
The compiler now emits the complete codec
(`packages/compiler/src/relational/discovery.ts:185`–`:193`), and the archive
test pins it (`tests/integration/beta11-archive.test.ts:73`–`:86`).
...
Policy retains `query.always()` (`fixtures/archive/src/archive-policy.ts:49`–`:58`,
`:99`–`:108`).

## Budgets and verification

The BETA-11 selected-PR micro scenario measured three reference-local archive
compiles. The median was 1,640.806 ms, with 751,524 generated bytes and 16,828
...
PostgreSQL 17 scenario passed in 12.232 seconds with 12 assertions. The final
