...
A [QUESTPIE](https://questpie.com) **headless API** built with [Hono](https://hono.dev),
...
- **Headless REST API** — `@questpie/hono` exposes every collection and global
  under `/api` while Hono retains sibling host routes.
...
The entry (`src/index.ts`) exports a Bun-style default (`{ port, fetch }`). To
run on Node, install `@hono/node-server` and serve the app:
...
```text
src/
  index.ts                           # Hono entry — mounts the QUESTPIE Hono adapter
  questpie/
    server/
