...
- **`src/questpie/server/app.ts`** — Composition root. Registers collections, globals, sidebar, dashboard, auth, and calls `.build()`. Also exports `appRpc`, `App`, `AppRpc`.
- **`src/questpie/server/rpc.ts`** — Creates the RPC builder `r = rpc()` used by all server functions.
- **`src/questpie/admin/builder.ts`** — Creates the client builder `admin = qa<App>().use(adminModule)`.
...
QUESTPIE provides standalone RPC — `app` and `appRpc` are two independent instances, no circular dependency.
...
```ts
// app.ts — imports r (runtime), exports app and appRpc separately
import { r } from "./rpc.js";

export const app = qb.collections({...}).build({...});
...
r.router({ myFn })                  → AppRpc type (preserves all function types)
  ↓
createClient<App, AppRpc>(...)    → client.rpc is fully typed
  ↓
...
- `blocks.ts` wants to import `App` from `app.ts` (for typed prefetch)
- **Circular!**

**The workaround: split into `baseApp` and final `app`:**
...
// Step 2: Export the base type — blocks import THIS (not App)
...
```ts
// blocks.ts — imports BaseCMS (not App) to avoid circular dependency
...
**Key points:**
- `BaseCMS` has the same collections/globals as `App` — blocks just aren't part of the type yet
- `typedApp<BaseCMS>(ctx.app)` casts the untyped `ctx.app` to the typed API
...
const handler = withOpenApi(
  ...
  { app, rpc: appRpc, basePath: "/api", info: { title: "My API", version: "1.0.0" } },
);
```
...
- **Importing `App` in `blocks.ts`** — Use `BaseCMS` pattern to avoid circular dependencies (see Blocks section).
