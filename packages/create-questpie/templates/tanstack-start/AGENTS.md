...
src/
  ...
  lib/
    ...
    client.ts            ← Typed client instance (createClient<AppConfig>)
    query.ts             ← Typed TanStack Query options (createQuestpieQueryOptions)
  routes/
...
### Typed Query Options (TanStack Query)

`src/lib/query.ts` wires `createQuestpieQueryOptions` over the typed `client`,
exposing `queryOptions()` / `mutationOptions()` builders for every collection,
global, and route. Pass them straight into `useQuery` / `useMutation`.
