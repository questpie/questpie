...
describe("channel ChannelsService", () => {
	...
	test("keeps lazy service namespaces valid when an authorizer freezes its context", async () => {
		let observed:
			| {
					keys: string[];
					policy: unknown;
		...
		(setup.app.config as any).channels = {
			room: channel("room-[roomId]").authorize((operation) => {
				...
				observed = {
					...
					policy: context.services.policy,
					tracker: context.analytics.tracker,
		...
		expect(
			await (context.channels as ChannelsService<any>).authorize(
				"room",
				{ roomId: "one" },
				"subscribe",
			),
