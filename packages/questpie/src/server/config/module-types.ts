 ...
 * Codegen writes most modules from the file system. `module()` writes one by
 * hand, which is how `@questpie/openapi` ships its module.
 *
...
export interface ModuleDefinition {
	 ...
	 *
	 * Readonly so `module<const T>` can keep the tuple. A mutable constraint
	 * still preserves arity today, but it says the wrong thing: nothing may
	 * mutate this list, and codegen writes it `as const`.
	 */
	 ...
	 *
	 * Merge strategies are core-owned. A plugin cannot register one for its own
	 * key. `app`, `auth` and `admin` merge below the key. Every other key is
	 * last-wins on the whole object, so when two modules write the same config
	 * key the later one replaces it outright.
	 */
	 ...
	 * Excluded from module merge — nothing reads it at runtime.
	 *
	 * It is still a live value on the generated module, so the plugin file and
	 * everything it imports load in the server process of every app that lists
	 * this module. Keep the plugin file's imports light.
	 */
	plugin?:
