...
export default collection("oauthClient")
	...
	.fields(({ f }) => ({
		...
		backchannelLogoutUri: f.text(2048),
		backchannelLogoutSessionRequired: f.boolean(),
		tokenEndpointAuthMethod: f.text(255),
		grantTypes: f.json(),
		responseTypes: f.json(),
		applicationType: f.text(255),
		jwks: f.textarea(),
		jwksUri: f.text(2048),
		// Better Auth 1.7 no longer reads `public`/`type`; kept so an upgrade never drops data.
		public: f.boolean(),
		type: f.text(255),
		requirePKCE: f.boolean(),
		dpopBoundAccessTokens: f.boolean().default(false),
