...
import {
	...
	type Projection,
	type ReadDescriptor,
} from "../internal/client-projection";
...
function fenceRetiredHydration(
	...
	const unsubscribe = client.getQueryCache().subscribe(({ type, query }) => {
		...
		clearing.add(query);
		try {
			const failures = retireQuery(client, query, new Error("SCOPE_RETIRED"));
			if (failures.length)
				throw new AggregateError(failures, "Query subscriber cleanup failed");
		} finally {
...
/** Bind one generated credential/Context lifetime to its native QueryClient. */
export function createQueryAdapter<Source extends Projection>(
	...
	const binding = Object.freeze({
		...
		async dispose() {
			...
			const ownedQueries = client.getQueryCache().findAll({ queryKey: prefix });
			...
			try {
				cancelled = client.cancelQueries(
					{ queryKey: prefix },
					{ revert: false },
				);
			} catch (error) {
			...
			for (const query of ownedQueries)
				failures.push(
					...retireQuery(client, query, new Error("SCOPE_RETIRED")),
				);
			liveEntries.clear();
			...
			if (failures.length)
				throw new AggregateError(failures, "Query Adapter cleanup failed");
		},
	});
