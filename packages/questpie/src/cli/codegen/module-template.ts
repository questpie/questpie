/**
 * Module Template Generation
 *
 * Generates the `.generated/module.ts` file content for npm package modules.
 * The generated file:
 * 1. Imports all discovered entities (collections, routes, jobs, etc.)
 * 2. Imports sub-modules from `modules.ts` (if present)
 * 3. Emits type interfaces using typeof references (zero inference cost)
 * 4. Exports a plain static module definition object
 *
 * Unlike the root app template (template.ts), this does NOT:
 * - Import a runtime config
 * - Call createApp()
 * - Generate an App type
 *
 * All category-specific behavior is driven by `CategoryDeclaration` metadata
 * from plugins — no hardcoded category names.
 *
 */

import {
	categoryRecordEntry,
	categoryTypeEntry,
	importStatement,
	safeKey,
	sortedValues,
} from "./category-emit.js";
import { CODEGEN_MODULE_METADATA_SYMBOL } from "./module-metadata.js";
import type {
	CategoryDeclaration,
	DiscoveredFile,
	DiscoverPattern,
	DiscoveryResult,
	FactoryArgumentMetadata,
} from "./types.js";

// ============================================================================
// Module template options
// ============================================================================

export interface ModuleTemplateOptions {
	/** Module name (e.g. "questpie-admin", "questpie-audit"). */
	moduleName: string;
	/** Discovery result with all found files. */
	discovered: DiscoveryResult;
	/**
	 * Merged category declarations from all plugins.
	 * Key = category name, value = declaration metadata.
	 * Used to determine emit strategy, type emission, etc.
	 */
	categoryMeta: Map<string, CategoryDeclaration>;
	/** Additional imports from plugins (e.g. admin view/component factories). */
	extraImports?: Array<{ name: string; path: string }>;
	/** Additional type declarations from plugins. */
	extraTypeDeclarations?: string[];
	/**
	 * Additional runtime code to add to the module object.
	 * Each entry is a line like `listViews: { table: listView("table") },`.
	 */
	extraModuleProperties?: string[];
	/**
	 * Command printed in the file header. Defaults to the bare command, which is
	 * right whenever the package's config sits at the CLI default location.
	 */
	regenerateCommand?: string;
	/** @deprecated No longer used — Registry augmentation moved to root template. */
	discoverPatterns?: Record<string, DiscoverPattern>;
}

/**
 * Result of module template generation.
 * May include an additional registries file for factory registry augmentations.
 */
export interface ModuleTemplateResult {
	/** Main module.ts code. */
	code: string;
	/**
	 * Optional registries.ts code for factory registry augmentations.
	 * Generated in a separate file to avoid circular references:
	 * module.ts contains CollectionBuilder instances whose augmented interface
	 * references Questpie.ViewsRegistry etc. — augmenting those in the same
	 * file creates a within-file circular dependency.
	 */
	registriesCode: string | null;
}

// ============================================================================
// Module template generation
// ============================================================================

/**
 * Generate the .generated/module.ts file content for a package module.
 *
 */
export function generateModuleTemplate(
	options: ModuleTemplateOptions,
): ModuleTemplateResult {
	const lines: string[] = [];
	const {
		moduleName,
		discovered,
		categoryMeta,
		extraImports,
		extraTypeDeclarations,
		extraModuleProperties,
		regenerateCommand = "questpie generate",
	} = options;

	const modulesFile = discovered.singles.get("modules") ?? null;
	const factoryArguments: FactoryArgumentMetadata[] = [];
	for (const [category, declaration] of categoryMeta) {
		if (!declaration.factoryArgument) continue;
		for (const file of discovered.categories.get(category)?.values() ?? []) {
			if (file.factoryArgument === undefined) continue;
			factoryArguments.push({
				category,
				key: file.key,
				value: file.factoryArgument,
				source: file.namedExportName
					? `${file.source} (export ${file.namedExportName})`
					: file.source,
			});
		}
	}

	// Derive a PascalCase type prefix from the module name
	// "questpie-admin" → "Admin", "questpie-audit" → "Audit"
	const typePrefix = deriveTypePrefix(moduleName);

	// The key a category takes on the emitted module object.
	//
	// The app template applies the same `createAppKey` override (template.ts,
	// emitNewArchitectureRuntime). This template used to ignore it, so a module
	// shipping an emails/ directory emitted `emails` while create-app only ever
	// reads `emailTemplates`. Those templates never reached the mailer, and
	// `emails` is not a consumed key either, so they died on instance.state.
	const moduleKeyFor = (catName: string): string =>
		categoryMeta.get(catName)?.createAppKey ?? catName;

	// Name of the exported named type for a category. Stays keyed by the category
	// name even when the module key differs — the type is a local artifact of
	// this file, nothing outside imports it by name.
	const namedTypeFor = (catName: string): string =>
		`${typePrefix}${catName.charAt(0).toUpperCase()}${catName.slice(1)}`;

	// Header. There is no --module flag, and commander rejects unknown options,
	// so the old header handed the package author a command that errors.
	lines.push("/* oxlint-disable */");
	lines.push("// AUTO-GENERATED by questpie codegen — DO NOT EDIT");
	lines.push(`// Regenerate with: ${regenerateCommand}`);
	lines.push("");

	// Import sub-modules
	if (modulesFile) {
		lines.push(
			"// ── Sub-Modules ────────────────────────────────────────────",
		);
		lines.push(importStatement(modulesFile));
		lines.push("");
	}

	// Import all categories (collections, globals, jobs, routes, etc.)
	for (const [catName, fileMap] of discovered.categories) {
		if (fileMap.size === 0) continue;
		const label = catName.charAt(0).toUpperCase() + catName.slice(1);
		lines.push(`// ── ${label} ────────────────────────────────────────────`);
		for (const file of sortedValues(fileMap)) {
			lines.push(importStatement(file));
		}
		lines.push("");
	}

	// Import singles (auth, locale, hooks, access, context, fields, sidebar, etc.)
	const singles = getNonModuleSingles(discovered.singles);
	if (singles.length > 0) {
		lines.push(
			"// ── Singles ────────────────────────────────────────────────",
		);
		for (const file of singles) {
			lines.push(importStatement(file));
		}
		lines.push("");
	}

	// Import spread singles (sidebar, dashboard — mergeStrategy: "spread").
	// Discovery fills `spreads` in module mode exactly as it does for an app, and
	// the app template imports, types and emits them. This template read none of
	// it, so a module contributing sidebar entries dropped them on the floor.
	for (const [stateKey, files] of discovered.spreads) {
		if (files.length === 0) continue;
		const label = stateKey.charAt(0).toUpperCase() + stateKey.slice(1);
		lines.push(
			`// ── ${label} (spread) ──────────────────────────────────────`,
		);
		for (const file of files) {
			lines.push(importStatement(file));
		}
		lines.push("");
	}

	// Extra plugin imports
	if (extraImports && extraImports.length > 0) {
		lines.push(
			"// ── Plugin Imports ─────────────────────────────────────────",
		);
		for (const imp of extraImports) {
			lines.push(`import ${imp.name} from "${imp.path}";`);
		}
		lines.push("");
	}

	// ════════════════════════════════════════════════════════════
	// TYPES — module type extraction
	// ════════════════════════════════════════════════════════════

	// Determine which categories carry FILES needing a non-trivial named type
	// (collection/global/job/route/view/component/block records built from
	// `typeof` references). Use typeEmit from CategoryDeclaration — skip "none"
	// and "messages".
	const categoriesNeedingTypes = new Set<string>();
	for (const [catName, fileMap] of discovered.categories) {
		if (fileMap.size === 0) continue;
		const decl = categoryMeta.get(catName);
		const typeEmit = decl?.typeEmit ?? "standard";
		// Skip categories that don't produce meaningful named types for modules
		if (typeEmit === "none" || typeEmit === "messages") continue;
		categoriesNeedingTypes.add(catName);
	}

	// The "safe" categories — those that get a PURE STATIC named type on every
	// module (collections/globals/jobs/routes/views/components/blocks/fieldTypes),
	// emitting `Record<never, never>` when the module ships none. This lets the
	// app index enumerate contributors via `interface AppX extends ${Prefix}X …`
	// (a static register, never a fold). The set is data-driven: a category is
	// "safe" when its `typeEmit` is the default "standard" AND it is not an array
	// category (migrations/seeds). This deliberately excludes the carrier
	// `services` (typeEmit "services" — reaches AppContext, §2.2), `emails`
	// (typeEmit "emails", standalone), and `messages` (typeEmit "messages").
	const safeNamedCategories: string[] = [];
	for (const [catName, decl] of categoryMeta) {
		if (isSafeNamedCategory(decl)) safeNamedCategories.push(catName);
	}

	if (categoriesNeedingTypes.size > 0 || safeNamedCategories.length > 0) {
		lines.push(
			"// ════════════════════════════════════════════════════════════",
		);
		lines.push(
			"// TYPES — composed from typeof references (zero inference cost)",
		);
		lines.push(
			"// ════════════════════════════════════════════════════════════",
		);
		lines.push("");

		if (categoriesNeedingTypes.has("routes")) {
			lines.push(
				'import type { RouteDefinition, RouteParamsFromKey } from "questpie/types";',
			);
			lines.push("");
		}

		// Always emit simple interfaces (no sub-module extends).
		//
		// Why not `extends _SubCollections`:
		//   Modules declare their OWN collections. When a module overrides a
		//   sub-module's collection (e.g. admin overrides starter's `account` with
		//   .merge().set("admin",...)), the override type is NOT a subtype of the
		//   original — TypeScript's `extends` constraint breaks.
		//
		//   Sub-module collection composition is handled upstream in the root app's
		//   index.ts via `ExtractFromModuleArray` and intersection types. Each module
		//   only needs to declare what IT directly provides.
		//
		// Why emit at all (not just infer from `_module`):
		//   Deep .merge().set() builder chains cause TS7056 ("inferred type exceeds
		//   maximum length"). Named interfaces give TypeScript a stable anchor.
		// Emit a named type for every SAFE category (pure static enumeration target),
		// plus any file-bearing category that is not itself safe but still needs a
		// named anchor. Empty safe categories collapse to `Record<never, never>` so
		// the app index can `extends ${Prefix}X` uniformly without a fold.
		const emittedTypeCats = new Set<string>();
		const emitNamedTypeFor = (catName: string): void => {
			if (emittedTypeCats.has(catName)) return;
			emittedTypeCats.add(catName);
			const fileMap = discovered.categories.get(catName);
			const decl = categoryMeta.get(catName);
			const typeName = namedTypeFor(catName);

			// No files → static empty register (`Record<never, never>`).
			if (!fileMap || fileMap.size === 0) {
				lines.push(`export type ${typeName} = Record<never, never>;`);
				lines.push("");
				return;
			}

			if (catName === "routes") {
				emitSimpleRouteTypeInterface(lines, typeName, fileMap);
			} else {
				emitSimpleTypeInterface(lines, typeName, fileMap, decl, catName);
			}
		};

		for (const catName of safeNamedCategories) emitNamedTypeFor(catName);
		// File-bearing categories that are NOT in the safe set (none today, but
		// keeps the emission complete if a plugin marks a file-bearing category
		// non-standard) still get their named anchor.
		for (const catName of categoriesNeedingTypes) emitNamedTypeFor(catName);
	}

	// Extra type declarations from plugins
	if (extraTypeDeclarations && extraTypeDeclarations.length > 0) {
		for (const decl of extraTypeDeclarations) {
			lines.push(decl);
		}
		lines.push("");
	}

	// ════════════════════════════════════════════════════════════
	// MODULE DEFINITION — static plain object
	// ════════════════════════════════════════════════════════════

	lines.push("// ════════════════════════════════════════════════════════════");
	lines.push("// MODULE DEFINITION — static plain object");
	lines.push("// ════════════════════════════════════════════════════════════");
	lines.push("");

	// Pre-scan extraModuleProperties to identify registry-matching properties.
	// These need separate const declarations BEFORE _module to avoid circular
	// typeof _module references in Registry augmentation:
	//   typeof _module → collections → CollectionBuilder augmentation → Registry
	//   → typeof _module["listViews"] → cycle!
	// By extracting to a separate const, Registry can use typeof _reg_listViews.
	const extractedConsts = new Map<
		string,
		{ constName: string; expression: string }
	>();
	const extraPropNames = new Set<string>();
	if (extraModuleProperties && extraModuleProperties.length > 0) {
		// Build the set of module keys that have registryKey — these need
		// extraction. Keyed by the module key, because a plugin's extra property
		// carries the name the property will have on the module object.
		const registryPropNames = new Set<string>();
		for (const [catName, decl] of categoryMeta) {
			if (decl.registryKey) registryPropNames.add(moduleKeyFor(catName));
		}
		for (const prop of extraModuleProperties) {
			const match = prop.match(/^(\w+)\s*:\s*(.+?),?\s*$/);
			if (match) {
				const [, propName, expression] = match;
				extraPropNames.add(propName);
				if (registryPropNames.has(propName)) {
					const constName = `_reg_${propName}`;
					extractedConsts.set(propName, { constName, expression });
				}
			}
		}
	}

	// Emit extracted consts before _module to break typeof circular references
	if (extractedConsts.size > 0) {
		for (const [, { constName, expression }] of extractedConsts) {
			lines.push(`const ${constName} = ${expression};`);
		}
		lines.push("");
	}

	const configSingles = singles.filter((f) => f.configKey);
	const plainSingles = singles.filter((f) => !f.configKey);

	lines.push(`export type ${typePrefix}Module = {`);
	lines.push(`\tname: "${moduleName}";`);
	if (factoryArguments.length > 0) {
		lines.push("\t[key: symbol]: unknown;");
	}
	if (modulesFile) {
		lines.push(`\tmodules: typeof ${modulesFile.varName};`);
	}
	// Categories with a named type — both file-bearing (categoriesNeedingTypes)
	// and every safe category (which always has a named type now, `Record<never,
	// never>` when empty). Reference the named type so the module declaration and
	// the app index's `extends ${Prefix}X` read the SAME static type.
	const namedTypeCats = new Set<string>([
		...categoriesNeedingTypes,
		...safeNamedCategories,
	]);
	for (const catName of namedTypeCats) {
		if (catName === "messages") continue;
		lines.push(
			`\t${safeKey(moduleKeyFor(catName))}: ${namedTypeFor(catName)};`,
		);
	}
	for (const [catName, decl] of categoryMeta) {
		if (namedTypeCats.has(catName)) continue;
		if (extraPropNames.has(moduleKeyFor(catName))) continue;
		const emitStrategy = decl.emit ?? "record";
		if (emitStrategy === "array") {
			lines.push(`\t${safeKey(moduleKeyFor(catName))}: readonly unknown[];`);
		} else {
			// Record<never, never> (NOT Record<string, never>) — a string index
			// signature here poisons `keyof` to `string` once the root template
			// intersects module maps via UnionToIntersection (_MP), which turns
			// service/collection aggregates into never-indexed maps app-wide.
			lines.push(`\t${safeKey(moduleKeyFor(catName))}: Record<never, never>;`);
		}
	}
	if (configSingles.length > 0) {
		lines.push("\tconfig: {");
		for (const file of configSingles) {
			lines.push(`\t\t${safeKey(file.configKey!)}: typeof ${file.varName};`);
		}
		lines.push("\t};");
	}
	for (const file of plainSingles) {
		lines.push(`\t${safeKey(file.key)}: typeof ${file.varName};`);
	}
	// Spread singles are typed loosely on purpose. The module object is
	// explicitly typed, so a precise element type would turn a spread file that
	// exports something other than an array into a compile error inside a
	// generated file. The app template infers instead and never faces this.
	for (const [stateKey, files] of discovered.spreads) {
		if (files.length === 0) continue;
		lines.push(`\t${safeKey(stateKey)}: readonly unknown[];`);
	}
	if (extraModuleProperties && extraModuleProperties.length > 0) {
		for (const prop of extraModuleProperties) {
			const match = prop.match(/^(\w+)\s*:\s*(.+?),?\s*$/);
			if (match) {
				const [, propName, expression] = match;
				lines.push(`\t${propName}: typeof ${expression};`);
			}
		}
	}
	lines.push("};");
	lines.push("");

	lines.push(`const _module: ${typePrefix}Module = {`);
	lines.push(`\tname: "${moduleName}" as const,`);
	if (factoryArguments.length > 0) {
		lines.push(
			`\t[Symbol.for(${JSON.stringify(CODEGEN_MODULE_METADATA_SYMBOL)})]: { factoryArguments: ${JSON.stringify(factoryArguments)} },`,
		);
	}

	// Sub-modules
	if (modulesFile) {
		lines.push(`\tmodules: ${modulesFile.varName},`);
	}

	// Emit all categories — driven by CategoryDeclaration.emit
	// Track which categories were emitted with files
	const emittedCategories = new Set<string>();

	for (const [catName, fileMap] of discovered.categories) {
		if (fileMap.size === 0) continue;
		emittedCategories.add(catName);
		const decl = categoryMeta.get(catName);
		const emitStrategy = decl?.emit ?? "record";
		const typeEmit = decl?.typeEmit ?? "standard";
		const typeName = namedTypeFor(catName);
		const hasNamedType = categoriesNeedingTypes.has(catName);
		const key = safeKey(moduleKeyFor(catName));

		if (emitStrategy === "array") {
			const vars = sortedValues(fileMap)
				.map((f) => f.varName)
				.join(", ");
			lines.push(`\t${key}: [${vars}],`);
		} else {
			lines.push(`\t${key}: {`);
			for (const file of sortedValues(fileMap)) {
				lines.push(`\t\t${categoryRecordEntry(file, decl)},`);
			}
			if (hasNamedType && typeEmit !== "messages") {
				lines.push(`\t} as ${typeName},`);
			} else {
				lines.push("\t},");
			}
		}
	}

	// Emit empty stubs for declared categories with no discovered files.
	// This ensures the module type always includes all target categories,
	// so consumers can spread `module.blocks` without type assertions.
	for (const [catName, decl] of categoryMeta) {
		if (emittedCategories.has(catName)) continue;
		const key = moduleKeyFor(catName);
		if (extraPropNames.has(key)) continue; // provided by extraModuleProperties
		const emitStrategy = decl.emit ?? "record";
		if (emitStrategy === "array") {
			lines.push(`\t${safeKey(key)}: [] as const,`);
		} else {
			lines.push(`\t${safeKey(key)}: {},`);
		}
	}

	// Config bucket — each config file = one key
	if (configSingles.length > 0) {
		lines.push("\tconfig: {");
		for (const file of configSingles) {
			lines.push(`\t\t${safeKey(file.configKey!)}: ${file.varName},`);
		}
		lines.push("\t},");
	}

	// Plain singles (fields, etc.)
	for (const file of plainSingles) {
		lines.push(`\t${safeKey(file.key)}: ${file.varName},`);
	}

	// Spread singles (sidebar, dashboard) — same shape the app template emits.
	// A key with no merge strategy of its own is duck-typed by create-app, and
	// array plus array concatenates, so every module's entries survive the fold.
	for (const [stateKey, files] of discovered.spreads) {
		if (files.length === 0) continue;
		const spread = files.map((f) => `...(${f.varName} ?? [])`).join(", ");
		lines.push(`\t${safeKey(stateKey)}: [${spread}],`);
	}

	// Extra module properties from plugins
	if (extraModuleProperties && extraModuleProperties.length > 0) {
		for (const prop of extraModuleProperties) {
			const match = prop.match(/^(\w+)\s*:\s*(.+?),?\s*$/);
			if (match) {
				const [, propName] = match;
				const extracted = extractedConsts.get(propName);
				if (extracted) {
					// Reference the extracted const instead of inline expression
					lines.push(`\t${propName}: ${extracted.constName},`);
				} else {
					lines.push(`\t${prop}`);
				}
			} else {
				lines.push(`\t${prop}`);
			}
		}
	}

	lines.push("};");
	lines.push("");

	lines.push("export default _module;");
	lines.push("");

	// ── Factory registry augmentations → separate file ───────────────
	// Augmentations for Questpie.ViewsRegistry, Questpie.ComponentsRegistry, and
	// Questpie.FieldTypesMap are emitted in a SEPARATE registries.ts file to avoid
	// circular references. The cycle is:
	//
	//   module.ts defines _module → contains CollectionBuilder instances
	//   → CollectionBuilder augmented interface references Questpie.ViewsRegistry
	//   → ViewsRegistry augmented IN THIS FILE → needs typeof _module to be resolved
	//   → BUT _module IS being resolved → CYCLE
	//
	// By moving augmentations to registries.ts (which imports type-only references
	// from module.ts), the cycle is broken. module.ts uses a side-effect import
	// of registries.ts to ensure the augmentations are always loaded.
	let registriesCode: string | null = null;
	{
		const augmentations: string[] = [];

		// Categories with recordPlaceholder (views, components)
		for (const [catName, decl] of categoryMeta) {
			if (!decl.recordPlaceholder) continue;
			const fileMap = discovered.categories.get(catName);
			if (!fileMap || fileMap.size === 0) continue;

			const cap = catName.charAt(0).toUpperCase() + catName.slice(1);
			const interfaceName = `${cap}Registry`;
			// Use the local type name from registries.ts (NOT from module.ts to avoid circular imports)
			const regLocalTypeName = `_Reg${cap}`;

			// Check if we emitted a named type interface for this category
			if (categoriesNeedingTypes.has(catName)) {
				augmentations.push(
					`\t\tinterface ${interfaceName} extends ${regLocalTypeName} {}`,
				);
			}
		}

		// ~fieldTypes singleton (fields.ts) → FieldTypesMap
		const fieldsSingle = singles.find((f) => f.key === "fields");
		if (fieldsSingle) {
			augmentations.push(`\t\tinterface FieldTypesMap extends _FieldTypes {}`);
		}

		if (augmentations.length > 0) {
			// Generate registries.ts content
			const regLines: string[] = [];
			regLines.push("/* oxlint-disable */");
			regLines.push("// AUTO-GENERATED by questpie codegen — DO NOT EDIT");
			regLines.push(
				"// Factory registry augmentations for module. Separate file to avoid circular refs.",
			);
			regLines.push(
				"// IMPORTANT: This file must NOT import from module.ts to avoid circular imports.",
			);
			regLines.push("");

			// Instead of importing named types from module.ts (which creates circular
			// imports: module.ts → registries.ts → module.ts), we rebuild the needed
			// type interfaces here from the original source files.
			for (const [catName, decl] of categoryMeta) {
				if (!decl.recordPlaceholder) continue;
				const fileMap = discovered.categories.get(catName);
				if (!fileMap || fileMap.size === 0) continue;
				if (!categoriesNeedingTypes.has(catName)) continue;

				const cap = catName.charAt(0).toUpperCase() + catName.slice(1);
				const typeName = `_Reg${cap}`;

				// Import individual source files. Same import rule as module.ts, so
				// a file whose entity is a named export is imported by that name
				// here too. Hardcoding a default import silently produced
				// `typeof undefined` for every named-export view or component.
				for (const file of sortedValues(fileMap)) {
					regLines.push(typeImportStatement(file));
				}
				regLines.push("");

				// Same emitter the module object uses, so the registry keys cannot
				// drift from the module keys. Rebuilt by hand, this branch ignored
				// keyFromProperty and keyed by filename while everything else keyed
				// by the runtime property.
				emitSimpleTypeInterface(
					regLines,
					typeName,
					fileMap,
					decl,
					catName,
					false,
				);
			}

			// Import fields singleton for typeof
			if (fieldsSingle) {
				regLines.push(typeImportStatement(fieldsSingle));
				regLines.push(`type _FieldTypes = typeof ${fieldsSingle.varName};`);
			}
			regLines.push("");

			regLines.push("declare global {");
			regLines.push("\tnamespace Questpie {");
			for (const aug of augmentations) {
				regLines.push(aug);
			}
			regLines.push("\t}");
			regLines.push("}");
			regLines.push("");

			registriesCode = regLines.join("\n");

			// Add side-effect import in module.ts to ensure augmentations are loaded
			// Insert after the header comments (before any other imports)
			lines.splice(3, 0, 'import "./registries.js";', "");
		}
	}

	return {
		code: lines.join("\n"),
		registriesCode,
	};
}

// ============================================================================
// Utilities
// ============================================================================

/**
 * Derive a PascalCase type prefix from a module name.
 * "questpie-admin" → "Admin"
 * "questpie-audit" → "Audit"
 * "billing" → "Billing"
 */
function deriveTypePrefix(moduleName: string): string {
	// Remove "questpie-" prefix if present
	const name = moduleName.replace(/^questpie-/, "");
	// Split on hyphens and PascalCase
	return name
		.split("-")
		.map((s) => s.charAt(0).toUpperCase() + s.slice(1))
		.join("");
}

/**
 * Whether a category gets a PURE STATIC named type (`${Prefix}<Cat>`) on every
 * module — emitted even when the module ships none (collapsing to `Record<never,
 * never>`). These are the categories the app index enumerates via
 * `interface AppX extends ${Prefix}X … { <user literals> }` (a static register,
 * never a fold over `typeof _modules`).
 *
 * Data-driven (no hardcoded category names): a category is "safe" when its
 * `typeEmit` is the default "standard" (or unset) AND it is a record category
 * (not `emit: "array"`). This yields exactly collections / globals / jobs /
 * routes / views / components / blocks / fieldTypes, and excludes:
 *   - `services` (typeEmit "services" — its VALUES reach AppContext, §2.2; a
 *      named type over them would re-introduce the cycle on `extends`)
 *   - `emails` (typeEmit "emails" — standalone record, app-keyed separately)
 *   - `messages` (typeEmit "messages" — a key-union, not a record)
 *   - `migrations`/`seeds` (emit: "array")
 */
function isSafeNamedCategory(decl: CategoryDeclaration): boolean {
	const typeEmit = decl.typeEmit ?? "standard";
	const emit = decl.emit ?? "record";
	return typeEmit === "standard" && emit === "record";
}

/**
 * Type-only form of {@link importStatement}.
 *
 * registries.ts holds nothing but types, and module.ts side-effect imports it,
 * so a value import here would pull every view and component into the runtime
 * graph ahead of the module that owns them. Built on importStatement so the
 * named-export rule cannot drift from the one module.ts uses.
 */
function typeImportStatement(file: DiscoveredFile): string {
	return importStatement(file).replace(/^import /, "import type ");
}

/**
 * Get all singles except "modules" (which is handled separately).
 */
function getNonModuleSingles(
	singles: Map<string, DiscoveredFile>,
): DiscoveredFile[] {
	const result: DiscoveredFile[] = [];
	for (const [key, file] of singles) {
		if (key === "modules") continue;
		// env is a root-app convention — module-contributed env is not supported yet.
		if (key === "env" || key === "envClient") continue;
		result.push(file);
	}
	return result.sort((a, b) => a.key.localeCompare(b.key));
}

/**
 * Emit a plain named type for a module category (no sub-module extends).
 * Always emit named types to prevent TS7056 ("inferred type exceeds maximum
 * length") when collection builder types are deeply nested via .merge().set().
 *
 * IMPORTANT: emitted as `type X = { ... }` — NOT `interface`. Interfaces have
 * no implicit index signature, so an interface-typed category map makes the
 * app-level intersection (e.g. `AppCollections`) fail
 * `extends Record<string, AnyCollectionOrBuilder>` constraints. That silently
 * degrades `GetCollection` to `never` and every `with`-populated relation to
 * `{}` in every consuming app. Type aliases get implicit index signatures and
 * keep the constraint satisfied.
 */
function emitSimpleTypeInterface(
	lines: string[],
	typeName: string,
	fileMap: Map<string, DiscoveredFile>,
	decl: CategoryDeclaration | undefined,
	catName: string,
	/** registries.ts rebuilds these types for its own use and exports none. */
	exported = true,
): void {
	const keyword = exported ? "export type" : "type";
	const leafFiles = sortedValues(fileMap).filter((f) => !f.isBundle);
	const bundleFiles = sortedValues(fileMap).filter((f) => f.isBundle);

	if (decl?.keyFromProperty) {
		lines.push(`${keyword} ${typeName} =`);
		const parts = [
			...leafFiles.map(
				(file) => `{ ${categoryTypeEntry(file, decl, catName)} }`,
			),
			...bundleFiles.map((bundle) => `typeof ${bundle.varName}`),
		];
		for (const [index, part] of parts.entries()) {
			const suffix = index === parts.length - 1 ? ";" : " &";
			lines.push(`\t${part}${suffix}`);
		}
		lines.push("");
		return;
	}

	if (bundleFiles.length === 0) {
		lines.push(`${keyword} ${typeName} = {`);
		for (const file of leafFiles) {
			lines.push(`\t${categoryTypeEntry(file, decl, catName)};`);
		}
		lines.push("};");
	} else {
		const parts: string[] = [];
		if (leafFiles.length > 0) {
			parts.push(
				`{ ${leafFiles.map((f) => `${categoryTypeEntry(f, decl, catName)}`).join("; ")} }`,
			);
		}
		for (const bundle of bundleFiles) {
			parts.push(`typeof ${bundle.varName}`);
		}
		lines.push(`${keyword} ${typeName} = ${parts.join(" & ")};`);
	}
	lines.push("");
}

function emitSimpleRouteTypeInterface(
	lines: string[],
	typeName: string,
	fileMap: Map<string, DiscoveredFile>,
): void {
	const leafFiles = sortedValues(fileMap).filter((f) => !f.isBundle);
	const bundleFiles = sortedValues(fileMap).filter((f) => f.isBundle);

	// Leaf entries guard on the `__brand: "route"` marker: convention dirs may
	// contain helper files whose default/named export is NOT a route definition
	// (e.g. admin's routes/i18n-helpers.ts) — erasing those to RouteDefinition
	// breaks the `as ${typeName}` cast on the module object.
	const leafEntry = (f: DiscoveredFile) =>
		`${safeKey(f.key)}: typeof ${f.varName} extends { __brand: "route" } ? RouteDefinition<unknown, unknown, RouteParamsFromKey<${JSON.stringify(f.key)}>> : typeof ${f.varName}`;

	if (bundleFiles.length === 0) {
		// `type` (not `interface`) — see emitSimpleTypeInterface for why.
		lines.push(`export type ${typeName} = {`);
		for (const file of leafFiles) {
			lines.push(`\t${leafEntry(file)};`);
		}
		lines.push("};");
	} else {
		const parts: string[] = [];
		if (leafFiles.length > 0) {
			parts.push(`{ ${leafFiles.map((f) => leafEntry(f)).join("; ")} }`);
		}
		for (const bundle of bundleFiles) {
			parts.push(`typeof ${bundle.varName}`);
		}
		lines.push(`export type ${typeName} = ${parts.join(" & ")};`);
	}
	lines.push("");
}

// ============================================================================
// Nested structure builder (for emit: "nested" categories)
// ============================================================================
