...
export interface TemplateOptions {
	...
	/**
	 * Command printed in the file header, e.g. `questpie generate -c src/questpie/server/questpie.config.ts`.
	 * It names the config that produced the file, which is the part a reader
	 * cannot guess in a repo with several of them.
	 */
...
export function generateTemplate(options: TemplateOptions): TemplateResult {
	...
	const genHeader = (buf: string[]): void => {
		...
		buf.push(`// Regenerate with: ${regenerateCommand}`);
		buf.push("");
	};
