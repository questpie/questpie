...
export interface FactoryTemplateOptions {
	 ...
	 * only right when the config sits at the CLI default location. `index.ts` is
	 * written into the same directory and names the config, so leaving this out
	 * puts two different commands side by side and one of them fails.
	 */
...
export function generateFactoryTemplate(
	...
	lines.push(
		`// Typed factory functions with plugin extensions. Regenerate with: ${regenerateCommand}`,
	);
	lines.push("");
