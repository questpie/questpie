...
describe("audit policy types", () => {
	it("exposes the typed field extension and policy values", () => {
		...
		expect(policy.persistence).toBe("required");
		...
		// @ts-expect-error persistence supports only transaction-required or best-effort
		const invalidPersistence: AuditPolicy = { persistence: "ignore" };
		// @ts-expect-error queued delivery is a separate future contract
		const invalidExport: AuditPolicy = {
			export: { delivery: "queue", sink: { append: () => {} } },
		};
	...
	it("generates consumer factories that reject invalid audit configuration", async () => {
		...
		await writeFile(
			...
			[
				...
				"// @ts-expect-error persistence is a closed policy value",
				'audit({ persistence: "ignore" });',
				"",
