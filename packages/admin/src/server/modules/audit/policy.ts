...
export type AuditPersistenceMode = "best-effort" | "required";
...
export interface AuditPolicy {
	/**
	 * `required` writes the audit row in the protected mutation transaction.
	 * `best-effort` writes it in a fresh transaction after that mutation commits.
	 */
	persistence?: AuditPersistenceMode;
	retention?: AuditRetentionPolicy;
	/** Non-durable export attempted only after the protected mutation commits. */
	export?: {
		sink: AuditSink;
