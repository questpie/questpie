	...
	const operations = resources
		...
		.flatMap((collection) => {
			...
			return (["create", "update", "delete"] as const).flatMap((member) => {
				...
				if (member === "delete" && collection.contract.appendOnly === true)
					throw new CompilerDiagnosticError(
						"QP-COMPOSE-013",
						"structuralTypeError",
						`${collection.identity} cannot declare a Policy delete operation: it is append-only (ADR-0048) and never accepts DELETE (ADR-0047)`,
					);
