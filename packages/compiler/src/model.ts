...
function codecContractProblem(problem: CodecContractProblem): never {
	const member = problem.member ?? "kind";
	let message: string;
	if (problem.kind === "notObject")
		message =
			problem.path.at(-1) === "properties"
				? "codec properties must be an object"
				: "codec must be an object";
	else if (
		problem.kind === "unexpectedMember" ||
		problem.kind === "missingMember"
	)
		message = `codec has unsupported member ${member}`;
	else if (problem.kind === "invalidKindType")
		message = "codec kind must be a string";
	else if (problem.kind === "optionalPresence")
		message = "codec.optional presence is invalid";
	else if (problem.kind === "optionalPosition")
		message = "codec.optional is valid only for an object property";
	else if (problem.kind === "invalidRange")
		message =
			problem.codecKind === "text"
				? "codec minLength must not exceed maxLength"
				: "codec minimum must not exceed maximum";
	else if (problem.kind === "unsupportedKind")
		message = `unsupported codec kind ${problem.codecKind}`;
	else if (problem.kind === "nonCanonicalBigint")
		message = `codec.${member} must be canonical bigint text`;
	else if (problem.kind === "bigintOutsidePostgres")
		message = `codec.${member} is outside PostgreSQL bigint`;
	else message = `codec.${member} is invalid`;
	if (problem.kind === "notObject" || problem.kind === "invalidKindType")
		throw new CompilerDiagnosticError(
			"QP-COMPOSE-015",
			"invalidAugmentation",
			message,
...
function codecContract(value: unknown): unknown {
	return normalizeCodecContract(value, {
		allowOptionalPresenceMember: true,
		invalid: codecContractProblem,
	});
}
