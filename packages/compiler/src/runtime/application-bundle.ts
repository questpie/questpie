		...
		plugins: [
			{
				...
				setup(builder) {
					...
					builder.onLoad(
						...
						() => ({
							contents:
								'const define = (kind) => (definition) => Object.freeze({ ...definition, kind, identity: `${kind}:${definition.name}`, network: definition.network === true }); export const defineQuery = (definition) => Object.freeze({ ...definition, ...(definition.query === undefined || definition.handler !== undefined ? {} : { handler: ({ input, ctx }) => ctx.data.run(definition.query, input) }), kind: "query", identity: `query:${definition.name}`, network: definition.network === true }); export const defineMutation = define("mutation"); export const defineAction = (definition) => Object.freeze({ ...definition, kind: "action", identity: `action:${definition.name}`, network: Object.hasOwn(definition, "network") ? definition.network : false }); export const defineRoute = define("route"); export const defineReaction = (definition) => Object.freeze({ ...definition, kind: "reaction", identity: `reaction:${definition.name}` }); export const defineJob = (definition) => Object.freeze({ ...definition, version: definition.version ?? 1, kind: "job", identity: `job:${definition.name}` });',
							loader: "js",
						}),
					);
