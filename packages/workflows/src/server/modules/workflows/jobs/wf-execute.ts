 ...
 *
 * Publish options:
 * - singletonKey: instanceId (dedupes on queue adapters that support singleton jobs)
...
import {
	claimWorkflowExecutionLock,
	releaseWorkflowExecutionLock,
	startWorkflowExecutionLockHeartbeat,
} from "./_execution-lock.js";
...
export const wfExecuteJob = job({
	...
	handler: async (ctx) => {
		...
		const executionLock = await claimWorkflowExecutionLock(
			ctx,
			instancesCrud,
			payload.instanceId,
		);

		if (executionLock.status === "locked") {
			logger?.debug?.("Workflow execution already locked; requeueing", {
				instanceId: payload.instanceId,
				retryAt: executionLock.retryAt.toISOString(),
			});
			await executeQueue.publish(
				...
				},
				{
					...
					startAfter: executionLock.retryAt,
				},
			);
			return;
		}

		if (executionLock.status === "skipped") {
			if (executionLock.reason === "missing") {
				throw new Error(`Workflow instance not found: ${payload.instanceId}`);
			}
			logger?.debug?.("Skipping workflow execution job", {
				instanceId: payload.instanceId,
				reason: executionLock.reason,
				currentStatus: executionLock.currentStatus,
			});
			return;
		}

		const heartbeat = startWorkflowExecutionLockHeartbeat(
			ctx,
			instancesCrud,
			executionLock,
			logger,
		...
		try {
			...
			const resumeWaiter = async (
				...
				await resumeQueue.publish(
					...
					{ singletonKey: `${waiterInstanceId}:${stepName}` },
				);
			};
			...
			if (result.status === "completed") {
				...
				if (instance?.parentInstanceId && instance?.parentStepName) {
					...
					await resumeQueue.publish(
						...
						{
							singletonKey: `${instance.parentInstanceId}:${instance.parentStepName}`,
						},
					);
				}
			}
				...
				if (instance?.parentInstanceId && instance?.parentStepName) {
					...
					if (parentStep && parentStep.status === "waiting") {
						...
						await executeQueue.publish(
							...
							{ singletonKey: instance.parentInstanceId },
						);
					}
				}
			}
			...
			heartbeat.stop();
			await releaseWorkflowExecutionLock(ctx, instancesCrud, executionLock);
		}
	},
});
