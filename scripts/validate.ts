...
function discoverWorkspaces(rootDir: string): WorkspacePackage[] {
	...
	for (const dir of dirs) {
		...
		const localDeps = Object.entries(allDeps)
			.filter(
				([_, v]) =>
					typeof v === "string" && (v as string).startsWith("workspace:"),
			)
			.map(([k]) => k);
...
function topoSort(packages: WorkspacePackage[]): WorkspacePackage[] {
	...
	if (sorted.length !== packages.length) {
		const missing = packages
			...
			.map((p) => p.name);
		throw new Error(
			`Cyclic dependency detected involving: ${missing.join(", ")}`,
		);
	}
		...
		if (typeof value === "string") {
			...
			for (const [condition, target] of Object.entries(
				...
				if (typeof target === "string" && !target.includes("*")) {
					...
					if (!existsSync(filePath)) {
						errors.push(
							`Export "${key}".${condition} → "${target}" — file not found`,
						);
					}
				}
			}
		...
		if (existsSync(moduleFile)) {
			try {
				...
				if (!content.includes("AUTO-GENERATED")) {
					errors.push(
						`${moduleFile.replace(pkg.path + "/", "")} — missing AUTO-GENERATED header`,
					);
				}
				...
				if (
					...
					errors.push(
						`${moduleFile.replace(pkg.path + "/", "")} — no export found`,
					);
				}
			} catch (e) {
				errors.push(
					`${moduleFile.replace(pkg.path + "/", "")} — failed to read: ${e}`,
				);
			}
		}
	}
...
function main() {
	...
	if (args.filterSubstring) {
		...
		packages = packages.filter(
			(p) =>
				p.dir.toLowerCase().includes(sub) || p.name.toLowerCase().includes(sub),
		);
		if (packages.length === 0) {
	...
	if (args.json) {
		...
		console.log("── summary ─────────────────────────────────────────────────");
		console.log(
			`  ${passed.length} passed, ${failures.length} failed, ${results.filter((r) => r.skipped).length} skipped  (${(totalMs / 1000).toFixed(1)}s)`,
		);
