	...
	test("retains first Job and Reaction context across duplicate and rollback", async () => {
		...
		try {
			...
			for (const assignment of [
				...
				await expect(
					session.begin(async (transaction) => {
						await transaction`select set_config('questpie.durable_kernel', 'on', true)`;
						await transaction.unsafe(
							`UPDATE questpie_internal.durable_runs SET ${assignment} WHERE run_id = $1::uuid`,
							[jobReceipt.runId],
						);
					}),
				).rejects.toThrow();
...
VALUES ($1, $2::uuid, $3::uuid, 'reaction:reports.created', 1, $4, 'user', $5,
        ...
        false, 1, false, transaction_timestamp())`,
			...
			const [backup] = await session<{ row: string }[]>`
			...
			await session.begin(async (transaction) => {
				...
				await transaction`
					...
					where application_name = ${application}
					  and record_id = ${backupReceipt.dispatchId}::uuid
				`;
				await transaction.unsafe(
					`INSERT INTO questpie_internal.durable_runs
SELECT (jsonb_populate_record(
  NULL::questpie_internal.durable_runs,
  $1::jsonb || jsonb_build_object('run_id', $2::text, 'dispatch_id', $3::text)
)).*`,
					[backup.row, restoredRun, restoredDispatch],
				);
			});
