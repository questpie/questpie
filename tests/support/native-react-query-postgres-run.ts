import { mkdtemp, rm } from "node:fs/promises";
import { tmpdir } from "node:os";
import { join } from "node:path";

export function selectNativePostgresTarget(args: readonly string[]) {
	if (args.length === 0)
		return {
			kind: "collaboration",
			database: "collaboration_native",
			test: "native-react-query.test.ts",
			ports: [],
		} as const;
	if (args.length === 1 && args[0] === "desk")
		return {
			kind: "desk",
			database: "team_support_desk_native",
			test: "team-support-desk.test.ts",
			ports: [43121],
		} as const;
	throw new TypeError("Expected no arguments or desk");
}

export function nativePostgresEnvironment(
	input: NodeJS.ProcessEnv,
	kind: "collaboration" | "desk",
) {
	const environment = { ...input };
	for (const name of Object.keys(environment))
		if (
			name.startsWith("PG") ||
			name === "DATABASE_URL" ||
			name === "SQL_DATABASE_URL" ||
			name === "QUESTPIE_PACKED_TARBALL" ||
			(kind === "desk" &&
				(name === "QUESTPIE_OTEL_PACKED_TARBALL" ||
					name.startsWith("QUESTPIE_TRACER_") ||
					name.startsWith("OTEL_") ||
					name === "BETTER_AUTH_SECRET" ||
					name === "BETTER_AUTH_TRUSTED_HOST"))
		)
			delete environment[name];
	return environment;
}

export async function requireFreeDeskPorts(ports: readonly number[]) {
	for (const port of ports) {
		let probe: ReturnType<typeof Bun.serve>;
		try {
			probe = Bun.serve({
				hostname: "127.0.0.1",
				port,
				fetch: () => new Response(null, { status: 204 }),
			});
		} catch {
			throw new Error(
				`Desk tracer requires unused loopback port ${port}; existing services were not changed`,
			);
		}
		await probe.stop(true);
	}
}

async function run() {
	const target = selectNativePostgresTarget(process.argv.slice(2));
	await requireFreeDeskPorts(target.ports);
	const owner = `questpie-native-query-${crypto.randomUUID()}`;
	const temporary = await mkdtemp(join(tmpdir(), "questpie-native-query-"));
	let container: string | undefined;
	function docker(...args: string[]) {
		const result = Bun.spawnSync(["docker", ...args], {
			stdout: "pipe",
			stderr: "pipe",
		});
		if (result.exitCode !== 0)
			throw new Error(`Disposable PostgreSQL command failed: ${args[0]}`);
		return result.stdout.toString().trim();
	}
	try {
		container = docker(
			"run",
			"--detach",
			"--rm",
			"--name",
			owner,
			"--publish",
			"127.0.0.1::5432",
			"--env",
			"POSTGRES_HOST_AUTH_METHOD=trust",
			"--env",
			`POSTGRES_DB=${target.database}`,
			"--env",
			"POSTGRES_INITDB_ARGS=--encoding=UTF8 --lc-collate=C.UTF-8 --lc-ctype=C.UTF-8",
			"postgres:17",
		);
		if (!/^[a-f0-9]{64}$/.test(container))
			throw new Error("Invalid owned container identity");
		const binding = docker("port", container, "5432/tcp");
		const port = /^127\.0\.0\.1:(\d+)$/.exec(binding)?.[1];
		if (!port || Number(port) < 1 || Number(port) > 65535)
			throw new Error("PostgreSQL is not loopback-bound");
		for (let attempt = 0; ; attempt++) {
			const ready = Bun.spawnSync([
				"docker",
				"exec",
				container,
				"pg_isready",
				"-h",
				"127.0.0.1",
				"-U",
				"postgres",
				"-d",
				target.database,
			]);
			if (ready.exitCode === 0) break;
			if (attempt === 60)
				throw new Error("Owned PostgreSQL did not become ready");
			await Bun.sleep(250);
		}
		const environment = nativePostgresEnvironment(process.env, target.kind);
		const child = Bun.spawn(
			[
				"bun",
				"test",
				join(import.meta.dir, "../integration/postgres", target.test),
				"--timeout",
				"180000",
			],
			{
				cwd: import.meta.dir,
				env: {
					...environment,
					TMPDIR: temporary,
					PGHOST: "127.0.0.1",
					PGPORT: port,
					PGUSER: "postgres",
					PGDATABASE: target.database,
					QUESTPIE_NATIVE_QUERY_CONTAINER: container,
				},
				stdout: "inherit",
				stderr: "inherit",
			},
		);
		process.exitCode = await child.exited;
	} finally {
		try {
			if (container && /^[a-f0-9]{64}$/.test(container))
				docker("rm", "--force", container);
		} finally {
			await rm(temporary, { recursive: true, force: true });
		}
	}
}

if (import.meta.main) await run();
