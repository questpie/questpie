...
const releaseContractTest =
	process.env.QUESTPIE_RELEASE_DRY_RUN_CONTRACT === "1" ? test : test.skip;

async function dryRun(args: readonly string[] = []) {
	const result = await runOwnedBunProcess(
		"run",
		["release", "--", "--dry-run", ...args],
		90_000,
	);
	return { ...result, exitCode: result.timedOut ? -1 : result.exit };
}

releaseContractTest(
	"dry-run packs every exact public package and rejects manifest drift",
	async () => {
		...
		const temporary = await mkdtemp(
			join(tmpdir(), "questpie-beta12-manifest-"),
		);
		try {
			...
			const rejected = await dryRun(["--artifact-manifest", tampered]);
			...
			const missingPackage = await dryRun(["--artifact-manifest", incomplete]);
			...
			const missingTelemetry = await dryRun([
				"--artifact-manifest",
				missingTelemetryPath,
			]);
