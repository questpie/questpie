...
Message interpolation existed as four private copies with two different regexes.
The two in `questpie` used `/\{\{(\w+)\}\}/g`; the two in `@questpie/admin` used
`/\{\{\s*(\w+)\s*\}\}/g`. So a custom message written as `Hello {{ name }}`
interpolated in the admin and rendered literally on the server — with nothing
surfacing the difference, since all four were file-private.
