...
- Bun's test runner now boots `happy-dom` + `@testing-library/react` via a preload, unlocking real component tests. The visual-edit module ships with 415+ tests across pure helpers (path utilities incl. degenerate-block-path edge cases, recursive diff incl. both-undefined / object↔primitive / class-instance opacity branches, group fields, click router incl. blockId-hint default fallback + relation-without-collection branches, patch ops incl. empty-path / empty-batch / unknown-op safety + deep-clone immutability, block operations + the `isBlockContent` type guard, V1 + V2 message-type guards covering every payload type, keyboard predicate, visualEdit meta + nested resolution incl. unknown-type and no-source defaults, strategy map), components (`InspectorErrorBoundary`, `VisualEditProvider` incl. controller-context throws-outside-host guards, `VisualInspectorPanel`), hook integrations (`useFormToPreviewPatcher` incl. baseline-reset and null-ref edge cases, `useVisualEditPreviewBridge` incl. iframe-reload re-seed, `useCollectionPreview` covering every V2 message handler from the iframe side incl. SELECT_TARGET idle-clear and the block-preserves-on-field-target design choice, `useDeselectOnEscape`), and the production-hardening primitive `useInitSnapshotBuffer` (extracted from `PreviewPane` for direct testability).
