...
│   │   │   │   └── pages.ts                  # Blocks, preview, workflow
│   │   │   ├── globals/
...
| Live preview (legacy split view)         | `barbers.ts` → `.preview()` + `v.collectionForm`                                      |
| Visual Edit Workspace                    | `pages.ts` → `.preview()` + `v.visualEditForm` with `visualEdit.group` field metadata |
| Page release workflow                    | `pages.ts` → draft/review/published stages; public routes read `stage: "published"`   |
...
- [Live Preview](/docs/workspace/live-preview) — Preview overview (default split view + Visual Edit Workspace)
- [Visual Edit Workspace](/docs/workspace/live-preview/visual-edit) — Pages collection's editing surface
- [Versioning & Workflow](/docs/backend/data-modeling/versioning-workflow) — Drafts, review, publishing, and stage reads
