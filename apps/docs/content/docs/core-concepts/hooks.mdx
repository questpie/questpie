...
export const posts = defineCollection("posts")
  ...
  .hooks({
    afterCreate: async ({ data, cms }) => {
      ...
      await cms.queue.publish("post-created-notification", {
        postId: data.id,
        title: data.title,
...
export const posts = defineCollection("posts")
  .fields({ /* ... */ })
  .hooks({
    afterUpdate: async ({ data, original, user, cms }) => {
      // Detect what changed
...
export const users = defineCollection("users")
  ...
  .hooks({
    beforeDelete: async ({ data, cms }) => {
      ...
      // Notify admin
      await cms.queue.publish("user-deleted", {
        userId: data.id,
        email: data.email,
      });
    },
  ...
  .hooks({
    afterUpdate: async ({ data, original, cms }) => {
      ...
      if (wasJustPublished) {
        ...
        // Notify subscribers
        await cms.queue.publish("post-published", {
          postId: data.id,
          title: data.title,
        });

        // Send to social media
        await cms.queue.publish("share-to-social-media", {
          postId: data.id,
        });
      }
  ...
  .hooks({
    afterCreate: async ({ data, cms }) => {
      ...
      try {
        ...
        await cms.queue.publish("retry-inventory-sync", {
          productId: data.id,
        });
      }
    },
    ...
    afterUpdate: async ({ data, cms }) => {
      ...
      await cms.queue.publish("sync-product-to-inventory", {
        productId: data.id,
      });
    },
  });
...
export const posts = defineCollection("posts")
  ...
  .hooks({
    ...
    afterCreate: [
      ...
      async ({ data, cms }) => {
        await cms.queue.publish("post-created", { postId: data.id });
      },
...
// ✅ Good - Queue the work
afterCreate: async ({ data, cms }) => {
  await cms.queue.publish("process-image", { imageId: data.imageId });
}
```
...
Always access services through the `cms` parameter:
...
afterCreate: async ({ data }) => {
  await cms.queue.publish(...); // Don't do this
}

// ✅ Good - Use the cms from context
afterCreate: async ({ data, cms }) => {
  await cms.queue.publish(...); // Type-safe and context-aware
}
```
...
export const posts = defineCollection("posts")
  ...
  .hooks({
    beforeCreate: async ({ data, cms }) => {
      ...
      await cms.queue.publish("job-name", { /* typed payload */ });
    },
  });
```
