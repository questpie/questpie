...
This walkthrough goes beyond the quickstart. You'll build a schema-first backend with multiple collections, relations between them, a custom sidebar, reactive fields, and a typed function — the same patterns used in the Barbershop reference project.
...
Collections are defined using the standalone `collection()` factory. Each file exports a single collection.
...
## Step 5: Configure Everything

Set up the file convention files:
...
export default branding({ name: { en: "My Platform" } });
```
...
```ts
// src/questpie/server/functions/get-active-barbers.ts
import { fn } from "questpie";

export default fn({
  handler: async ({ collections }) => {
    const result = await collections.barbers.find({
      where: { isActive: true },
    });
    return {
      barbers: result.docs,
...
## Step 6: Generate and Mount

Run codegen to produce the app entrypoint:
...
- **Typed functions** for custom business logic
