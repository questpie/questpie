...
```ts
// src/questpie/questpie.config.ts
...
Collections defined with `collection(...)` can now use `.admin()`, `.list()`, and `.form()`. Admin login also requires runtime auth configuration via `.auth({ ... })`.
