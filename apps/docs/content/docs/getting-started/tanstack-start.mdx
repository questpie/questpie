...
my-app/
  ...
  questpie.config.ts                 # CLI config (points at the app)
  src/
    questpie/
      server/
        questpie.config.ts           # runtime config — db, adapters, secrets
        app.ts                       # re-exports the generated app + AppConfig
        modules.ts                   # enabled modules (admin, openapi, …)
        collections/
          posts.collection.ts        # starter collection
        globals/
          site-settings.global.ts    # starter global
...
<Callout type="info" title="`#questpie/factories`, not `questpie`">
	Definition files import the builders (`collection`, `global`) from `#questpie/factories` — the generated, app-typed factory module. `f` isn't imported; it arrives as the `.fields()` callback argument. See [Collections](/docs/concepts/collections) for the full builder.
</Callout>
...
<Callout type="warn" title="QUESTPIE does not auto-create Postgres extensions">
	This is deliberate and drizzle-native: extensions can't be reliably created by the app at runtime, so QUESTPIE never silently tries. Locally, `docker compose up` provisions them for you. On **managed Postgres**, enable the extensions you need (e.g. `pg_trgm`) through your provider before deploying. The [Search adapter](/docs/adapters/search) covers which extensions each search backend requires.
</Callout>
...
<Callout type="info" title="Re-run codegen after adding or removing files">
	If you add or remove a collection or global later, regenerate first (`bun run questpie:generate`, or `bun run scaffold:verify` to also type-check), then `bun run db:push` again. Editing a definition's body doesn't need a regen. The `questpie add` CLI runs codegen for you automatically — see [Codegen](/docs/concepts/codegen).
</Callout>
...
{/* TODO @user: screenshot the landing page at localhost:3000 with the three entry-point cards (preview tools or manual capture). */}
...
{/* TODO @user: screenshot the admin setup screen (create first admin). */}
...
{/* TODO @user: screenshot the admin Posts list view after creating a post. */}
...
- [Client SDK](/docs/concepts/client-sdk) — the typed client in depth: `createClient<AppConfig>()`, every CRUD method, and the query options.
- [TanStack Query](/docs/concepts/tanstack-query) — the React hooks layer: `q.collections.*` / `q.globals.*` / `q.routes.*`.
- [Codegen](/docs/concepts/codegen) — what `questpie generate` / `questpie add` do and when to re-run them.
