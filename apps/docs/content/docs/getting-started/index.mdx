...
**[Hooks](/docs/concepts/hooks)**, lifecycle callbacks on `.hooks({ ... })`: `beforeChange`, `afterChange`, `beforeDelete`, and more. Each receives the full app context. Queue `publish()` joins the ambient transaction automatically; use `onAfterCommit(cb)` for non-transactional effects such as direct email or HTTP calls.
