...
<Callout type="info" title="Import from `#questpie/factories`, not `questpie`">
	...
	[Codegen](/docs/code/codegen) for how the generated factory is wired.
</Callout>
...
**[Collections](/docs/schema/collections)**, typed, versioned database tables with hooks, access rules, relations, and (with the admin module) an automatic admin UI. The `collection(name)` builder is the unit you compose everything else onto.

```ts
collection("posts").fields(({ f }) => ({ title: f.text().required() }));
```

**[Globals](/docs/schema/globals)**, singletons (site settings, a homepage). Same field API as collections, but `get`/`update` only, no create/delete, no `where`.

```ts
global("settings").fields(({ f }) => ({ siteName: f.text().required() }));
```

**[Fields](/docs/schema/fields)**, the building blocks of a row. 15 built-ins cover the common cases; chain modifiers like `.required()`, `.default()`, `.localized()`, and `.array()` on any of them.
...
**[Relations](/docs/schema/relations)**, typed foreign keys. `f.relation("user")` is a belongs-to; `.hasMany({ foreignKey })` and `.manyToMany({ through })` give you the other directions. Load them with `with: { author: true }`.
...
**[Access control](/docs/schema/access-control)**, per-operation rules on `.access({ read, create, update, delete })`. Return a boolean to allow/deny, or a `where`-shaped object to filter rows (row-level security). Rules run with the full request context (`session`, `db`, …).
...
<Callout type="warn" title="Secure by default, except routes">
	...
	**public** until you add one. Be deliberate about both. See [Access
	control](/docs/schema/access-control) and [Routes](/docs/code/routes).
</Callout>

**[Hooks](/docs/schema/hooks)**, lifecycle callbacks on `.hooks({ ... })`: `beforeChange`, `afterChange`, `beforeDelete`, and more. Each receives the full app context. Queue `publish()` joins the ambient transaction automatically; use `onAfterCommit(cb)` for non-transactional effects such as direct email or HTTP calls.
...
**[Validation](/docs/schema/validation)**, Zod schemas for create and update are generated automatically from your field definitions. Use `.validation({ refine, exclude })` only when you need to tighten or drop a specific field.

### Business logic

**[Routes](/docs/code/routes)**, custom HTTP endpoints as files in `src/questpie/server/routes/`. Chain `.post().schema(...).handler(...)`; the handler gets a typed `input` plus the full context (`collections`, `queue`, `db`, `session`). No imports, no singletons, everything arrives through context.
...
**[Jobs](/docs/code/jobs)**, background work as files in `jobs/`. Define a payload schema and handler; dispatch with `queue.<name>.publish(payload)`. Add `options.cron` for a recurring schedule. Adapters: pg-boss, BullMQ, Cloudflare Queues.
...
<Callout type="info" title="Workers start from application entrypoints">
	...
	or `app.queue.runOnce()` for one serverless batch. See
	[Jobs](/docs/code/jobs).
</Callout>

**[Services](/docs/code/services)**, reusable singletons (a Stripe client, a third-party SDK) injected into context. Resolve dependencies from the `create(ctx)` argument.
...
**[Emails](/docs/code/emails)**, typed templates as `emails/*.ts`. A template returns `{ subject, html }`; send it with `email.sendTemplate({ template, to, input })`. Adapters: SMTP, Resend, Plunk, or a dev console logger.
...
**[Blocks](/docs/schema/blocks)**, composable content blocks for page-builder fields (admin module). Declare a block with `block(name).fields(...)`, then store a tree of them in a `f.blocks()` field.
...
**[Modules and plugins](/docs/code/modules)**, a module is plain data (`collections`, `routes`, `jobs`, `services`, a codegen `plugin`) you add to `src/questpie/server/modules.ts`. Admin, OpenAPI, MCP, and workflows all use this path. [Build your own](/docs/extend/build-a-plugin) to package reusable schema + behavior.

**[Codegen](/docs/code/codegen)**, the engine that turns your files into the typed runtime. `questpie generate` runs it once; `questpie dev` watches and regenerates on file add/remove. Codegen output is committed to git. A module's codegen `plugin` lets a package contribute its own file conventions without you editing config.
...
- **[Collections](/docs/schema/collections)**, the deep dive on the unit you'll use most.
- **[Fields](/docs/schema/fields)**, every built-in field type and the chain-modifier model.
- **[Client SDK](/docs/client/sdk)**, the typed client and how every call is inferred from your schema.
- **[Codegen](/docs/code/codegen)**, the engine that turns your files into the typed runtime.
...
- **New dev (build something)**, [TanStack Start](/docs/getting-started/tanstack-start) → [Collections](/docs/schema/collections) → [Fields](/docs/schema/fields) → [Relations](/docs/schema/relations) → [Access control](/docs/schema/access-control) → [Client SDK](/docs/client/sdk) → [TanStack Query](/docs/client/tanstack-query). Add [Hooks](/docs/schema/hooks), [Jobs](/docs/code/jobs), and [Routes](/docs/code/routes) when you need behavior.
- **Backend / data modeler**, [Collections](/docs/schema/collections) → [Fields](/docs/schema/fields) → [Relations](/docs/schema/relations) → [Validation](/docs/schema/validation) → [Access control](/docs/schema/access-control) → [Hooks](/docs/schema/hooks).
- **Frontend integrator**, [Client SDK](/docs/client/sdk) → [TanStack Query](/docs/client/tanstack-query) → [Realtime](/docs/client/realtime).
- **Extender / plugin author**, [Codegen](/docs/code/codegen) → [Modules](/docs/code/modules) → [Build a plugin](/docs/extend/build-a-plugin).
