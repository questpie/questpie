...
<Callout type="warn" title="Secure by default, except routes">
	...
	**public** until you add one. Be deliberate about both. See [Access
	control](/docs/schema/access-control) and [Routes](/docs/concepts/routes).
</Callout>
...
- **New dev (build something)**, [TanStack Start](/docs/getting-started/tanstack-start) → [Collections](/docs/schema/collections) → [Fields](/docs/schema/fields) → [Relations](/docs/schema/relations) → [Access control](/docs/schema/access-control) → [Client SDK](/docs/client/sdk) → [TanStack Query](/docs/client/tanstack-query). Add [Hooks](/docs/schema/hooks), [Jobs](/docs/concepts/jobs), and [Routes](/docs/concepts/routes) when you need behavior.
