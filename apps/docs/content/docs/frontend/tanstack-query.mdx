...
`find()`, `count()`, and `get()` accept a typed `{ realtime: true }` second argument. The query fetches normally, then auto-updates as the server pushes live snapshots over SSE:
...
function LiveServiceList() {
	...
	return <ul>{data?.docs.map((s) => <li key={s.id}>{s.name}</li>)}</ul>;
}
```

See [Realtime Client](/docs/frontend/realtime) for how snapshots work, the wire protocol, and the vanilla `live()` API.
