...
`app.queue.listen()` registers cron schedules automatically before starting workers. If you need a schedule-only setup step, call `await app.queue.registerSchedules()` and then `await app.queue.stop()`.

<Callout type="note">
Cron jobs must accept an empty payload because schedule registration validates `schema.parse({})`.
</Callout>
...
<Cards>
  <Card title="Hooks" href="/docs/collections/hooks" description="Trigger jobs from collection hooks" />
  <Card title="Typed Routes" href="/docs/server/routes" description="Server-side logic" />
  <Card title="Queue" href="/docs/infrastructure/queue" description="Queue infrastructure setup" />
  <Card title="Email" href="/docs/infrastructure/email" description="Email adapter configuration" />
</Cards>
