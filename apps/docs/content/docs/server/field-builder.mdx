...
In traditional app development, you repeat field information across layers: database migration, API validation, admin form config, client types. QUESTPIE eliminates this by making the `.fields()` callback the single source of truth. Every layer reads from the same definition.
...
When the app builds, each field definition goes through these steps:
