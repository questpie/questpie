...
Keep long-running processes out of services. A service may publish a job with `queue.someJob.publish(...)`, but it should not call `queue.listen()` or start background loops. Job execution belongs in a worker process; see [Jobs](/docs/server/jobs) and [Queue](/docs/infrastructure/queue).
...
<Cards>
  <Card title="Typed Routes" href="/docs/server/routes" description="Use services in route handlers" />
  <Card title="Hooks" href="/docs/collections/hooks" description="Use services in lifecycle hooks" />
  <Card title="Jobs" href="/docs/server/jobs" description="Publish background work from services" />
  <Card title="Codegen" href="/docs/concepts/codegen" description="How services appear in AppContext" />
</Cards>
