...
- **History sidebar** — Transitions appear in the audit timeline (when `audit()` module is enabled) with stage change details
...
const articles = collection("articles")
  ...
  .hooks({
    ...
    afterTransition: async ({ fromStage, toStage, data, email, collections }) => {
      if (toStage === "review") {
...
```ts
const siteConfig = global("site_config")
  .fields(({ f }) => ({
