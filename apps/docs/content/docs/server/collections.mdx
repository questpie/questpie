...
an app needs structured data. Instead of manually creating database tables, writing API routes, building admin forms, and keeping them in sync, you define a collection once and everything derives from it. The collection definition is your single source of truth.
...
Custom server functions live in the `functions/` directory and are automatically composed via codegen.
See [Functions](/docs/server/functions) for patterns and usage.
...
const withSeo = collection("temp").fields(({ f }) => ({
...
When the app is built via `questpie generate`, each collection generates:
...
- **Not exporting the collection** — The collection must be exported from a file in the `collections/` directory. Codegen discovers exported collection builders automatically — if it's not exported, it won't be included in the generated app.
