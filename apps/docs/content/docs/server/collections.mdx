...
See [Functions](/docs/server/rpc) for patterns and usage.
...
When the app is built from your `config()`, each collection generates:
...
- **Not exporting the collection** — The collection must be passed to `.collections({})` in your app builder. If it's not registered, it won't exist at runtime.
