...
| `db` | app database | Database client (may be overridden with a transaction) |
...
the app normalizes context with defaults:
...
### Flat Context in Handlers

All handlers (hooks, functions, jobs) receive the fully typed `AppContext` directly — no type helpers needed:

```ts
.hooks({
  afterChange: async ({ db, kv, session }) => {
    // All properties are typed via AppContext augmentation
...
### Implicit Context (runtime)

Use `getContext()` and `tryGetContext()` to access context from utility functions outside of handlers:
...
await runWithContext({ session, db, locale: "sk" }, async () => {
...
await app.api.collections.pages.find({}, { locale: "sk" });

// Inside hooks or nested calls, locale is inherited automatically
.hooks({
  afterRead: async ({ collections }) => {
    // This automatically uses locale: "sk" from parent context
    const related = await collections.posts.find({
      where: { featured: true },
    });
...
Create a context for programmatic API operations:
...
const result = await app.api.collections.posts.find({}, ctx);
```
...
handler: async ({ input, db, collections }) => {
  return withTransaction(db, async (tx) => {
    ...
    for (const item of order.items) {
      await collections.products.updateById(
...
withTransaction(db, async (tx) => {
  await app.api.collections.posts.create(data, { db: tx });
...
export default fn({
  ...
  handler: async ({ input, db, collections, queue }) => {
    return withTransaction(db, async (tx) => {
...
- **Using old type helpers** — With `AppContext` augmentation, you don't need `typedApp()`, `typedDb()`, or `typedContext()`. All handler context is typed automatically.
