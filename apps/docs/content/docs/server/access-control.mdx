...
Admin panels and APIs need permissions. QUESTPIE's access system lets you define rules once on the collection and have them enforced automatically across the REST API, custom functions, and the admin UI. Rules can be static booleans, functions that check the session, or queries that filter rows.
...
```ts
export const employees = collection("employees")
  .fields(({ f }) => ({
...
Collection and global access functions receive a flat typed context:

```ts
interface AccessContext<TData> {
  ...
  db: Database;                  // Database client (typed via AppContext)
  locale?: string;               // Current locale
  collections: AppCollections;   // Typed collection APIs
  globals: AppGlobals;           // Typed global APIs
}
```

### Using Typed Access

Access functions receive a fully typed context via `AppContext` augmentation — no manual type assertions needed:

```ts
.access({
  delete: async ({ session, data, db }) => {
    // db is already typed — no getDb() or type assertions needed
    const refs = await db
...
QUESTPIE is **secure by default**. When no access rules are defined, the system requires an authenticated session. This prevents accidentally exposing data to unauthenticated users.

### Resolution Order

For each CRUD operation, access rules are resolved in this order:

```text
1. Collection/global's own .access() rule   → checked first
2. App-level defaultAccess (from builder)   → fallback
3. Framework default: !!session             → last resort (require auth)
```

If a collection defines `.access({ read: true })` but omits `create`, then `read` uses the explicit rule (`true` = public) while `create` falls through to `defaultAccess` or the framework default.

### Admin Module Defaults

The `adminModule` sets `defaultAccess` to require authentication for all operations:
...
// All collections now require authentication for ALL operations
// unless they define their own .access() rules
```

### Custom Default Access

Override the default via `defaultAccess` in your runtime config:
...
export default runtimeConfig({
  // Override: public reads, authenticated writes
  defaultAccess: {
    read: true,
    create: ({ session }) => !!session,
    update: ({ session }) => !!session,
...
When multiple modules provide `defaultAccess`, the config-level setting wins (last-wins strategy).

### Making a Collection Public

To make a specific collection publicly accessible while keeping the default restrictive, set `.access()` on that collection:

```ts
const publicPosts = collection("posts")
  .access({
    read: true,  // Public reads
    // create, update, delete fall through to defaultAccess (require auth)
  })
  .fields(({ f }) => ({ ... }));
```
...
Even without `admin()` or any `defaultAccess`, the framework falls back to requiring a session (`!!session`). This means you can never accidentally expose data by forgetting to set access rules.

To explicitly make everything public (not recommended), set in `questpie.config.ts`:
...
- **Returning `undefined` instead of `false`** — Access functions must return `boolean` or `AccessWhere`. Returning `undefined` is treated as "no rule" and falls through to the framework default (require session).
- **Assuming public access** — The framework is secure by default. If you want unauthenticated access, you must explicitly set `read: true` on the collection or in `defaultAccess`.
