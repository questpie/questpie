...
### Starter Module Defaults

The `starterModule` sets `defaultAccess` to require authentication for all operations:

```ts
import { questpie, starterModule } from "@questpie/server";

// starterModule includes:
// .defaultAccess({
...
// posts and products now require authentication for ALL operations
// unless they define their own .access() rules
```

### Custom Default Access

Override the default with `.defaultAccess()` on the builder:
...
When composed via `.use()`, the consuming builder's `defaultAccess` wins (last-wins strategy, same as `.locale()`).
...
Even without `starterModule` or any `defaultAccess`, the framework falls back to requiring a session (`!!session`). This means you can never accidentally expose data by forgetting to set access rules.

To explicitly make everything public (not recommended), set:
