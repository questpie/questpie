...
// Read with locale
const settingsSk = await app.api.globals.siteSettings.get({
  locale: "sk",
});
```
...
Globals are auto-discovered from the `globals/` directory via codegen. Place each global in its own file:

```text
src/questpie/server/
├── globals/
│   ├── site-settings.ts      # global("siteSettings")
│   └── booking-settings.ts   # global("bookingSettings")
├── questpie.config.ts
└── modules.ts
```

When you run `questpie generate`, all exported globals are collected into the generated app entrypoint automatically.

## Sidebar Integration

Globals appear in the sidebar via the `sidebar.ts` file convention:
...
- **Forgetting to export** — The global must be exported from a file in the `globals/` directory. Codegen discovers exports automatically.
