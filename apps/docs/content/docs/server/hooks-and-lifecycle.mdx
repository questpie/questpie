...
export const barbers = collection("barbers")
  ...
  .hooks({
    ...
    afterChange: async ({ data, operation, logger }) => {
      if (operation === "create") {
        logger.info("New barber created", { id: data.id });
      }
    },
  });
```
...
```ts
beforeOperation: async ({ operation, logger }) => {
...
```ts
afterDelete: async ({ data, logger, storage }) => {
  logger.info("Record deleted", { id: data.id });

  // Clean up related files
  if (data.avatarUrl) {
    await storage.disk().delete(data.avatarUrl);
  }
}
```
...
```ts
afterTransition: async ({ fromStage, toStage, data, queue }) => {
...
| `collections` | `AppCollections` | All hooks | Typed collection CRUD APIs |
| `globals` | `AppGlobals` | All hooks | Typed global CRUD APIs |
| `queue` | `QueueClient` | All hooks | Typed job queue |
| `email` | `MailerService` | All hooks | Typed email service |
| `logger` | `Logger` | All hooks | Logger instance |
| `kv` | `KVStore` | All hooks | Key-value store |
| `storage` | `StorageService` | All hooks | File storage service |

### Accessing Typed Services

Hook context is fully typed via `AppContext` augmentation — no manual type casting needed:

```ts
afterChange: async ({ db, kv, data }) => {
  // db, kv, collections, queue, etc. are all typed automatically
  await kv.set(`cache:${data.id}`, data, 300);
  ...
  .hooks({
    ...
    afterRead: ({ data }) => {
      ...
      const date = d.scheduledAt
        ...
        : "";
      d.displayTitle = `Appointment — ${date}`.trim();
    },
...
- **Ignoring available context properties** — Hook context provides flat access to `collections`, `queue`, `email`, `kv`, `logger`, `storage`, and more via `AppContext` augmentation. Destructure what you need directly.
