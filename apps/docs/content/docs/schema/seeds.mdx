...
Write the body to survive a second entry anyway. `--force`, `seed:reset`, an
undo and a fresh database all send the runner back through `run`. Look before
you insert, or patch a singleton the way the example above does.
