...
<Callout type="info" title="Global or collection">
	Use a [collection](/docs/schema/collections) for a set of records. Use a
...
See [Fields](/docs/concepts/fields) for the full field catalog and [Relations](/docs/schema/relations) for `f.upload()` / `f.relation()`.
...
- [Collections](/docs/schema/collections), multi-row tables with full CRUD and list views.
- [Fields](/docs/concepts/fields), the field catalog used by `.fields()`.
- [Relations](/docs/schema/relations), `f.relation()` / `f.upload()` and relation hydration with `with`.
- [Access control](/docs/schema/access-control), the access-rule model shared with collections.
- [Hooks](/docs/schema/hooks), the lifecycle-hook model.
