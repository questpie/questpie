...
> **Prerequisites:** read [Fields](/docs/schema/fields) first, this page covers only the `number` type. The `f` proxy, the chain-modifier model, and shared modifiers like `.required()` / `.default()` / `.localized()` are taught there.
...
<Callout type="info" title="`bigint` and `decimal` read back as JS `number`">
	Both use Drizzle's `mode: "number"`, so the value type on read/insert/update
	is a plain JS `number`. That keeps the API uniform, but a `number` only holds
	integers exactly up to `Number.MAX_SAFE_INTEGER` (2^53 − 1) and `decimal`
	arithmetic goes through floating point. For money or IDs beyond that range,
	store an exact string value with `f.text()` and validate the format with
	`.zod()`, or create a dedicated custom field type.
</Callout>

## Chained methods

These methods are specific to `number` (on top of the [shared modifiers](/docs/schema/fields#shared-methods) every field has). Each returns a new immutable field, so they chain in any order.
...
<Callout
	type="info"
	title="`.min()` / `.max()` here are VALUE bounds, not string length"
>
	On a number field, `.min(n)` / `.max(n)` constrain the numeric **value**. On
	[`f.text()`](/docs/schema/fields/text) the same method names constrain
	**string length** (they map to `minLength` / `maxLength`). Same names,
	different meaning, sized by the field's type.
</Callout>
...
Chain `.array()` to store a list of numbers in a single `jsonb` column, and bound the list length with `.minItems(n)` / `.maxItems(n)`. This is a [shared modifier](/docs/schema/fields#shared-methods), not number-specific:
...
`.required()` makes the field non-null and required on insert; without it the column is nullable and the insert field optional. `.default(0)` makes the input optional and is type-checked against `number`. See [Fields → inferred types](/docs/schema/fields#example--inferred-types) for how modifiers flow into the generated types.

## Related

- [Fields](/docs/schema/fields), the `f` proxy, the chain-modifier model, and the shared modifiers (`.required()`, `.default()`, `.array()`, `.localized()`, …).
- [`f.text()`](/docs/schema/fields/text), strings, where `.min()` / `.max()` mean length instead of value.
- [Validation](/docs/schema/validation), the auto-derived Zod schema and the `.zod()` escape hatch for custom numeric rules.
