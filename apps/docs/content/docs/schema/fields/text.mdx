...
<Callout type="warn" title="`.trim()`, `.lowercase()` and `.uppercase()` do not touch the value">
	Each sets a flag. No code transforms the stored string, so `.trim()` writes
	`"  hi  "` unchanged. Their one reader is the CRDT eligibility check, which
	counts them as refinements and rejects the field. Normalize with `.zod()` or a
...
A `.max(n)` above the constructor width reaches the attribute and the browser
schema, but neither the server schema nor the column. The form then accepts what
the write rejects.
