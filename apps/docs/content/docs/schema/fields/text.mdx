...
> **Prerequisites:** read [Fields](/docs/schema/fields) first, this page covers only the `text` type. The `f` proxy, the chain-modifier model, and shared modifiers like `.required()` / `.localized()` / `.default()` are taught there.
...
- **Renders a text input** in the admin form (a multiline editor is what [`f.textarea()`](/docs/schema/fields/textarea) gives you instead).
...
These methods are specific to `text` (on top of the [shared modifiers](/docs/schema/fields) every field has). Each returns a new immutable field, so they chain in any order.
...
>
	...
	[`f.number()`](/docs/schema/fields/number) the same method names constrain the
	numeric **value**. Same names, different meaning, sized by the field's type.
</Callout>
...
- For **multi-line** content with a textarea editor in the admin, reach for [`f.textarea()`](/docs/schema/fields/textarea). For **formatted** content, use [`f.richText()`](/docs/schema/fields/rich-text).

## Multiple values

Chain `.array()` to store a list of strings in a single `jsonb` column, and bound it with `.minItems(n)` / `.maxItems(n)`. This is a [shared modifier](/docs/schema/fields), not text-specific:
...
- [Textarea field](/docs/schema/fields/textarea), the multi-line sibling.
