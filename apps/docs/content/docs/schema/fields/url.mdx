...
> **Prerequisites:** read [Fields](/docs/schema/fields) first, this page covers only the `url` type. The `f` proxy, the chain-modifier model, and shared modifiers like `.required()` / `.localized()` / `.default()` are taught there.
...
These methods are specific to `url` (on top of the [shared modifiers](/docs/schema/fields) every field has). Each returns a new immutable field, so they chain in any order.
...
`url` uses the **URL operator set** (`urlOps`), which is the [string operator set](/docs/schema/fields/text) extended with three URL-specific operators. Every `url` field is filterable with these in a `where` clause.
...
- For a **plain string** with no URL validation (a path, a slug, an arbitrary identifier), use [`f.text()`](/docs/schema/fields/text).
- For an **email address**, use `f.email()`, the same string shape with `z.string().email()` validation and `domain` operators instead (see [Email field](/docs/schema/fields/email)).
- To **upload a file** and store a link to it, reach for the [Upload field](/docs/schema/fields/upload) (`f.upload()`), or the collection-level [`.upload()`](/docs/schema/collections#uploads) builder, not a `url` field.

## Multiple values

Chain `.array()` to store a list of URLs in a single `jsonb` column, and bound it with `.minItems(n)` / `.maxItems(n)`. This is a [shared modifier](/docs/schema/fields), not url-specific:
...
- [Text field](/docs/schema/fields/text), the plain-string sibling and the `stringOps` operator set `url` extends.
