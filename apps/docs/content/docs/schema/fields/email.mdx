...
> **Prerequisites:** read [Fields](/docs/schema/fields) first, this page covers only the `email` type. The `f` proxy, the chain-modifier model, and shared modifiers like `.required()` / `.localized()` / `.default()` are taught there.
...
These methods are specific to `email` (on top of the [shared modifiers](/docs/schema/fields) every field has). Each returns a new immutable field, so they chain in any order.
...
>
	...
	[`f.text()`](/docs/schema/fields/text)). On `f.number()` the same method names
	constrain the numeric **value**. Same names, different meaning, sized by the
	field's type.
</Callout>
...
For the full query language, combining field filters with `AND` / `OR` / `NOT`, pagination, and `orderBy`, see [Fields → filtering](/docs/schema/fields). Relation filters and hydration are covered in [Relations](/docs/schema/relations).
...
- For a free-form short string with no format constraint, use [`f.text()`](/docs/schema/fields/text) instead.
- For a website or link, use [`f.url()`](/docs/schema/fields/url), the same idea (a validated string), but with `z.string().url()` and host/protocol filter operators.
...
Chain `.array()` to store a list of email strings in a single `jsonb` column, and bound it with `.minItems(n)` / `.maxItems(n)`. This is a [shared modifier](/docs/schema/fields), not email-specific:
...
`.required()` makes the field non-null and required on insert; without it the column is nullable and the insert field optional. See [Fields → inferred types](/docs/schema/fields) for how modifiers flow into the generated types.

## Related

- [Fields](/docs/schema/fields), the `f` proxy, the chain-modifier model, and the shared modifiers (`.required()`, `.localized()`, `.default()`, `.array()`, …).
- [Text field](/docs/schema/fields/text), the unvalidated single-line string sibling.
