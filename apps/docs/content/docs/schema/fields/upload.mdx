...
> **Prerequisites:** read [Fields](/docs/schema/fields) first, this page covers only the `upload` type. The `f` proxy, the chain-modifier model, and shared modifiers like `.required()` / `.localized()` / `.label()` are taught there.
...
This method is specific to `upload` (on top of the [shared modifiers](/docs/schema/fields) every field has). It returns a new immutable field.
...
For the full query language, combining field filters with `AND` / `OR` / `NOT`, pagination, and `orderBy`, see [Fields → filtering](/docs/schema/fields). Relation filters (`some` / `none` / `every`, `is` / `isNot`) and hydrating the linked asset with `with:` are covered in [Relations](/docs/schema/relations).
...
`.required()` makes the FK non-null and required on insert; without it the column is nullable and the insert field optional. See [Fields → inferred types](/docs/schema/fields) for how modifiers flow into the generated types.

<Callout type="warn" title="The many forms keep `string` at the type level, only the runtime value is `string[]`">
`through` and `.multiple()` change the **runtime** schema to `z.array(z.string().uuid())`, but they do **not** transition the field's static type-state: `UploadFieldState` always pins `data: string`, the M2M branch still returns `FieldWithMethods<UploadFieldState<TTo>, …>`, and `.multiple()` is typed `(): any`. So a many-upload field's generated row/insert/`where` types stay `string`, not `string[]`, unlike [`f.relation().manyToMany()` / `.multiple()`](/docs/schema/relations), which *do* transition the type-state to a to-many shape with `data: string[]`.
</Callout>

## Related

- [Fields](/docs/schema/fields), the `f` proxy, the chain-modifier model, and the shared modifiers (`.required()`, `.localized()`, `.label()`, …).
...
- [Relations](/docs/schema/relations), belongsTo / hasMany / manyToMany hydration and relation filters that the upload variants share.
