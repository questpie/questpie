...
| `.outputFalse()`           | Key leaves the read type and is deleted from every response         |
...
<Callout type="info" title="Undeclared keys are dropped">
	The composed `z.object()` strips keys you did not declare. A client that
	posts an extra key gets it removed before the write, with no error. Declare
	every key you intend to store.
</Callout>
...
| `hasKey`     | `string`                  | `col ? $1`                             |
| `hasKeys`    | `string[]`                | `col ?& $1::text[]`                    |
| `hasAnyKeys` | `string[]`                | `col ?\| $1::text[]`                   |
| `pathEquals` | `{ path: string[]; val }` | `col #>> path = $val`                  |
| `jsonPath`   | `string`                  | `col @@ $1::jsonpath`                  |
| `isEmpty`    | `boolean`                 | `col = '{}' OR col IS NULL`            |
| `isNotEmpty` | `boolean`                 | `col != '{}' AND col IS NOT NULL`      |
...
`isNotEmpty`, `isNull`, `isNotNull`. See
[Arrays](/docs/schema/fields/array).
...
<Callout type="warn" title="The outer call wins">
	Mark the object `.localized()` and the per-key split is skipped entirely, so
	a nested `.localized()` below it does nothing. Pick one level.
</Callout>
...
Both recurse into deeper objects and into array items. See
[Access control](/docs/schema/access-control).

## Admin

Both methods below come from `@questpie/admin` and appear on `f` only when that
module is enabled. See [Collections and Globals](/docs/admin/collections).
...
address: f
	...
	.admin({ layout: "grid", columns: 2 })
	.form(({ f }) => ({
		fields: [{ type: "section", label: "Postal", fields: [f.zip, f.city] }],
	})),
```
