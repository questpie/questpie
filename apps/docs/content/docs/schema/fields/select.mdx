...
> **Prerequisites:** read [Fields](/docs/schema/fields) first, this page covers only the `select` type. The `f` proxy, the chain-modifier model, and shared modifiers like `.required()` / `.default()` / `.localized()` / `.array()` are taught there.
...
`.enum(name)` is the only method specific to `select`, on top of the [shared modifiers](/docs/schema/fields#shared-methods) every field has and `.array()` for multi-select (below).
...
Chain the shared [`.array()`](/docs/schema/fields/array) modifier for multi-select. The field stores option values in one `jsonb` column, returns the value union as an array (`("news" | "guide")[]`), and uses multi-select operators. Bound list length with `.minItems(n)` and `.maxItems(n)`:
...
- [Array](/docs/schema/fields/array), how `.array()` turns a select (or any field) into a multi-value list, with the full `selectMultiOps` membership operators.
