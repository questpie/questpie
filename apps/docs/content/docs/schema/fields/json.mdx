...
`f.json()` stores arbitrary JSON. By default it produces a Postgres `jsonb` column and derives a recursive JSON schema (`jsonValueSchema`), so any valid JSON value round-trips. It is the escape hatch for free-form data, settings blobs, third-party payloads, metadata, where the shape isn't fixed enough for [`f.object()`](/docs/schema/fields#structured). Pass a type argument to narrow the stored value, and `.zod()` to validate it at runtime.

> **Prerequisites:** read [Fields](/docs/schema/fields) first, this page covers only the `json` type. The `f` proxy, the chain-modifier model, and shared modifiers like `.required()` / `.default()` / `.localized()` are taught there.
...
`f.json()` uses the common `Field` builder. Shape it with the [shared modifiers](/docs/schema/fields#shared-methods); the following two are especially useful for JSON values.
...
The built-in operators compare the JSON value as a whole. For nested-key queries, model known keys with `f.object()` or top-level columns, or add a custom operator set with `.operators()` (see [Fields → custom operator sets](/docs/schema/fields#custom-operator-sets)).
...
- **Reach for [`f.object()`](/docs/schema/fields#structured) instead** when the nested shape is fixed and you want per-key validation and admin form fields generated automatically, `f.object()` also stores `jsonb`, but each key is its own typed, validated field. Use `f.json()` only when the data is genuinely schema-less.
...
`.array()` is a [shared modifier](/docs/schema/fields#shared-methods) for making _any field type_ repeat, it isn't needed here, since the JSON value can already be an array.
...
`.required()` makes the column non-null and the input required; without it the column is nullable and the insert field optional. The `JsonValue` type is exported from `questpie` if you need it directly. See [Fields → inferred types](/docs/schema/fields#example--inferred-types) for how modifiers flow into the generated types.

## Related

- [Fields](/docs/schema/fields), the `f` proxy, the chain-modifier model, and the shared modifiers (`.$type()`, `.zod()`, `.required()`, `.localized()`, …).
- [`f.object()`](/docs/schema/fields#structured), the typed, per-key sibling for fixed nested shapes.
- [Validation](/docs/schema/validation), the auto-derived Zod schema and the `.zod()` escape hatch (the way to actually validate a JSON shape).
- [Relations](/docs/schema/relations), filtering by a relation and hydrating related rows with `with`.
