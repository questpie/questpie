...
> **Prerequisites:** read [Fields](/docs/schema/fields) first, this page covers only the `richText` type. The `f` proxy, the chain-modifier model, and shared modifiers like `.required()` / `.localized()` / `.label()` are taught there.
...
- **Localizes per locale**, chain `.localized()` to keep a separate document per locale (a [shared modifier](/docs/schema/fields)).
...
`richText` adds **no type-specific chained methods**, its field-type definition declares only the state factory, no extra methods. You configure it entirely through the [shared modifiers](/docs/schema/fields) every field has.
...
- **Just a plain multi-line string with no formatting?** Use [`f.textarea()`](/docs/schema/fields/textarea).
...
`@questpie/admin/fields` exports `richText`, `richTextFieldType`, `RichTextFieldState`, `RichTextFeature`, and the augmentable `RichTextFieldMeta`. See [Fields → inferred types](/docs/schema/fields) for how modifiers flow into generated types.

## Related

- [Fields](/docs/schema/fields), the `f` proxy, the chain-modifier model, and the shared modifiers (`.required()`, `.localized()`, `.label()`, `.default()`, …).
...
- [Textarea field](/docs/schema/fields/textarea), a plain multi-line string with no formatting.
