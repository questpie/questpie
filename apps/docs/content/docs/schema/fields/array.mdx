...
Every [shared modifier](/docs/schema/fields) applies to the list as a whole.
`.required()` makes the column `NOT NULL` and the key required on insert.
...
Any field can be wrapped. Storage and validation behave the same for all of
them, since `.array()` replaces the column factory outright. What differs is the
control the admin picks for one item.
...
`f.boolean().array()` stores real booleans and edits them as free text.

<Callout type="warn" title="Not for relations or uploads">
	QUESTPIE registers a relation by reading `getMetadata().type`, and `.array()`
	rewrites that to `"array"`. A wrapped `f.relation()` or `f.upload()` loses its
	foreign key, its hydration and its picker, and keeps only a bare jsonb column.
	Reach for `f.relation().multiple()` or `f.upload({ through })` instead.
</Callout>
...
decoded before it reaches the driver, so a client that stringified its list
twice still stores an array.
...
<Callout type="warn" title="`isEmpty` ignores its operand">
	`isEmpty: false` compiles to the same SQL as `isEmpty: true`, the value is
	never read. That condition is also unparenthesized, and the builder brackets
	only the whole `AND` group, so beside another filter the `IS NULL` arm escapes
	it. Use `isNotEmpty` for the opposite.
</Callout>
...
[Fields](/docs/schema/fields) has the shared modifiers and the table of every
type you can wrap.
...
`.crdt({ format: "set" })`, which takes an array of `f.text({ mode: "text" })`
that is required, defaults to `[]`, and carries no length bounds.
