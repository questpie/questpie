...
> **Prerequisites:** read [Fields](/docs/schema/fields) first, this page covers only the `.array()` modifier. The `f` proxy, the chain-modifier model, and the other shared modifiers (`.required()` / `.default()` / `.localized()`) are taught there.
...
<Callout type="info" title="Arrays start from the element field">
	...
	modifiers](/docs/schema/fields#shared-methods) available on field definitions.
</Callout>
...
Everything else is a [shared modifier](/docs/schema/fields#shared-methods) and applies to the array as a whole:
...
- `.localized()` stores a **per-locale array** in the i18n side-table (a [shared modifier](/docs/schema/fields#shared-methods)).
...
- **Multi-select**, `f.select([...]).array()` is the canonical multi-value select. The inner `select` keeps its literal-union typing and option list; `.array()` makes it multiple. See [`f.select()`](/docs/schema/fields#field-types).
...
- **Reach for [Relations](/docs/schema/relations) instead** when the elements are rows in another collection, a list of FKs you want to query, join, and hydrate with `with:`. `.array()` is for inline values, not normalized references.
- **Reach for [`f.object()`](/docs/schema/fields#field-types) (then `.array()`)** when each element needs multiple sub-fields, `f.object({ ... }).array()` gives a list of structured items.

## TypeScript

An array field contributes `Item[]` to the generated row and insert types, and an [`ArrayWhereInput`](/docs/schema/relations) shape to the `where` type. The element filter value is derived from the inner field, not the stored data, a `f.datetime().array()` filters on the inner field's date input, not `Date[]`.
...
`.required()` makes the array non-null and required on insert; without it the column is nullable (`Item[] | null`) and the insert field optional. `.minItems()` / `.maxItems()` add length validation to the derived `z.array(...)` schema but have no type-level effect. See [Fields → inferred types](/docs/schema/fields#example--inferred-types) for how modifiers flow into the generated types.

## Related

- [Fields](/docs/schema/fields), the `f` proxy, the chain-modifier model, and the other shared modifiers (`.required()`, `.default()`, `.localized()`, …).
- [`f.select()`](/docs/schema/fields#field-types), multi-select is `f.select([...]).array()`.
- [`f.object()`](/docs/schema/fields#field-types), for a list of structured items, wrap an object: `f.object({ ... }).array()`.
- [Relations](/docs/schema/relations), when the list elements are rows in another collection (and the full query language for the operators above).
- [Validation](/docs/schema/validation), the auto-derived `z.array(...)` schema and the `.zod()` escape hatch.
