...
`logger.redact` is an array of additional Pino-style paths. It extends the
framework's recursive credential and error redaction and cannot turn those
safe defaults off. See [Request logs](/docs/ship/monitoring/request-logs) for
the path syntax and correlation trust rules.
