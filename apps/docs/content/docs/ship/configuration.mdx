...
| `env.ts`             | env var schema + validation                                | the env definition         | `env()`, see [Environment](/docs/ship/environment) |
...
| `autoSeed`     | `boolean \| SeedCategory \| SeedCategory[]` | Run [seeds](/docs/schema/seeds) on startup.                                    |
| `cli`          | `{ migrations?, seeds? }`                   | Override generated-migration / [seed](/docs/schema/seeds) directories.         |
...
The full module surface (`ModuleDefinition`), how modules contribute each category, and the depth-first merge are covered in [Extend → Modules](/docs/code/modules).
...
`create-questpie` scaffolds this shape. CLI settings such as migration and [seed](/docs/schema/seeds) directories live in the server config when you need to override the defaults.
...
- [Collections](/docs/schema/collections), the entities `runtimeConfig` deliberately keeps out of `questpie.config.ts`.
- [Access control](/docs/schema/access-control), how `config/app.ts`'s `access` defaults sit at the bottom of the per-collection rule chain.
- [Hooks](/docs/schema/hooks), per-entity hooks; `config/app.ts`'s `hooks` are the global counterpart.
- [Environment](/docs/ship/environment), `env.ts` / `env()`, boot validation, and server-vs-client vars that feed `runtimeConfig`.
