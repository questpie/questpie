...
QUESTPIE ships no server. Its common HTTP engine is `createFetchHandler` from
`questpie/http`: TanStack Start mounts it directly, while the Next, Hono and
Elysia templates use native adapters around it. The host still owns the server
and process lifecycle, so the start command is whatever your template's
`package.json` says.
