...
For generated optimistic concurrency, the mutation builders preserve the
client preconditions: by-id variables include `expectedRevision`, batch
entries include `expectedRevision`, and bulk mutations include exact
`expectedRevisions`. A global update uses
`{ data: { data: fields, expectedRevision } }` because the outer `data` is the
TanStack mutation-variable envelope. Revert and workflow params also carry
`expectedRevision`.
...
- [Temporal values](/docs/schema/fields/temporal-values), including `Date` preservation through TanStack Query dehydration, TanStack Start Seroval hydration, realtime, and custom JSON boundaries.
...
- [Collections](/docs/schema/collections), the CRUD surface (`find`, `create`, `updateMany`, …) these builders wrap, and the `find()` envelope shape.
- [Globals](/docs/schema/globals), the singleton counterpart, and why `get()` is non-nullable.
- [Routes](/docs/code/routes), custom server functions exposed as `q.routes.<path>.query()` / `.mutation()`.
- [Channels](/docs/client/channels), typed application-event subscriptions exposed as `q.channels.<name>.subscription()`.
- [Relations](/docs/schema/relations), the `with` hydration and `where` query language available inside `find()` options.
