...
`presence()` resolves one typed member snapshot; it is not a continuous presence-change subscription. Pusher/Soketi uses native provider presence and internally follows membership while the channel is mounted. The SSE tier provides coarse app-instance-local presence from subscription lifecycle, so it is suitable for lightweight indicators, not a globally exact occupancy counter. The public client does not currently expose `subscribePresence()` or a TanStack presence query; see [Reactive Apps & Performance](/docs/client/reactive-apps#presence-current-contract) before promising a live occupancy UI.
...
- [Reactive Apps & Performance](/docs/client/reactive-apps), channel sizing, bounded event state, React isolation, and the current presence contract.
