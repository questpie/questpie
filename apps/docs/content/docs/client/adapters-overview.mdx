...
1. **`createFetchHandler(app, config)`** — The universal request handler
2. **`createAdapterRoutes(app, config)`** — Granular route handlers for each endpoint
...
|---|---|---|---|
| `basePath` | `string` | `"/"` | URL prefix for all API routes |
| `accessMode` | `string` | `"user"` | Default access mode for requests |
| `getLocale` | `(req, app) => string` | — | Extract locale from request |
| `getSession` | `(req, app) => Session` | — | Extract session from request |
...
| Method | Path | Operation |
|---|---|---|
| POST | `/functions/:path...` | Call server function |
...
- **Wrong `basePath`** — The client and server must use the same base path. If the server uses `/api`, the client must too.
- **Missing functions directory** — Functions are discovered from the `functions/` directory automatically. Ensure your `fn()` definitions are in the right path.
