...
const app = createQuestpieQueryOptions(client, {
  realtime: {
    baseUrl: "/api",       // Must match your adapter basePath
    enabled: true,
...
const stream = sseSnapshotStream({
  url: "/api/realtime/collections/posts?where={\"status\":\"published\"}",
...
// "/api/realtime/collections/posts?where=..."

const globalUrl = buildGlobalRealtimeUrl(
  { baseUrl: "/api", enabled: true },
  "siteSettings",
);
// "/api/realtime/globals/siteSettings"
```
...
- **Wrong `baseUrl`** — The realtime `baseUrl` must match your adapter's `basePath`. If your API is at `/api`, the realtime base must be `/api` too.
