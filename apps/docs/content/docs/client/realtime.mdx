...
- **Temporal values stay typed.** Snapshot and row-delta frames carry explicit versioned path metadata, so actual datetime fields remain `Date` while ISO-looking strings remain strings. See [Temporal values](/docs/concepts/temporal-values).
