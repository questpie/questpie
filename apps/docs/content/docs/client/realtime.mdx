...
this topic, or when the connection fails. Wire it. Without it a broken
subscription looks exactly like one with no news.
...
<Callout type="info" title="A dropped connection heals itself">
	...
	number it last saw. You do not write that loop.
</Callout>
