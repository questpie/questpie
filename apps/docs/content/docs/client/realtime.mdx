...
Delta reads reuse the normal row/field/`afterRead` access path. Changes to configured `watchedResources` trigger an immediate reset, and every delta group re-bootstraps at least once per minute. That one-minute interval is the maximum access-staleness window for claim or relational-access changes that do not emit a watched-resource change; use snapshot mode when that window is unacceptable.
