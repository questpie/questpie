...
The Elysia adapter mounts QUESTPIE as an Elysia plugin. The client factory creates a merged client with CRUD and full Eden Treaty type safety for custom routes.
...
### `questpieElysia(app, config)`

Returns an Elysia plugin with all API routes mounted. Configuration is the same as `createFetchHandler` — see [Adapters Overview](/docs/client/adapters-overview).
...
- **Missing type parameters** — Pass both `AppType` (Elysia app type) and `App` (app type) for full type safety.
