...
changes QUESTPIE authorization. Use `getSession` on `questpieHono` instead,
without `questpieMiddleware`, when the mount needs a custom session resolver.
...
Hono and Elysia accept `basePath`, request logging, search policy,
`getSession`, `getLocale` and `extendContext`. They intentionally do not expose
`accessMode`: public HTTP requests always run in user mode.
