...
core handler exactly once. The optional `questpieMiddleware(app)` remains
available when your own native Hono routes need `app`, `appContext` or `user`;
it is not an authority input to the QUESTPIE mount.

When both helpers are composed, the mount reuses the middleware's immutable
adapter context so the request has one identity and one request scope. Native
routes receive a detached `appContext` view; mutating it or `c.user` no longer
changes QUESTPIE authorization. Framework resources and non-cloneable extension
values such as functions, promises and class instances are omitted from that
native view. Use `getSession` on `questpieHono` instead, without
`questpieMiddleware`, when the mount needs a custom session resolver.
