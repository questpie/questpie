...
A target is one output directory. `server` writes `index.ts`, `factories.ts` and
the type layers beside them. `admin-client` writes `client.ts`. A plugin can
invent its own target id.
...
exports `generateModule()`, for a package that builds its module at build time.
