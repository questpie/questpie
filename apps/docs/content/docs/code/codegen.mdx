...
Codegen is driven by three commands. The broader migrate / seed / push surface is covered by [Configuration](/docs/ship/configuration) and [Seeds](/docs/schema/seeds).
...
- [Configuration](/docs/ship/configuration), the convention files codegen discovers (`questpie.config.ts`, `config/app.ts`, `config/auth.ts`, `modules.ts`) and how `runtimeConfig` registers `plugins`.
- [Environment](/docs/ship/environment), `env.ts` / `env.client.ts`, also discovered by codegen, and boot-time validation.
- [Collections](/docs/schema/collections), the export/discovery rules in practice and the `#questpie/factories` builder.
- [Blocks](/docs/schema/blocks), a category contributed by the admin module's codegen plugin (the `block()` builder factory).
- [Get started](/docs/learn), where `questpie generate` first appears in the workflow.
