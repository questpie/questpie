...
You declare the modules you depend on in `modules.ts` (covered in [Configuration → `modules.ts`](/docs/ship/configuration)). Codegen reads that file in a dedicated **pre-pass** before the rest of discovery, and folds every module's contributions into one flat app.
...
The plugin itself, what categories and discovery it declares, and how you write one, is covered in its own pages: see [Building a plugin](/docs/guides/build-a-plugin) for the authoring loop and [Codegen](/docs/code/codegen) for the full `CodegenPlugin` API. A short treatment of how a module _ships_ one is [below](#shipping-a-codegen-plugin).
...
- **[Building a plugin](/docs/guides/build-a-plugin#the-codegen-plugin-contract)**, the build-your-own loop: the `CodegenPlugin` contract, targets, categories, discover patterns, builder extensions, scaffolds, and validators, plus the smaller per-feature seams (custom field type, operator, adapter, block) you'll reach for far more often.
- **[Codegen → the `CodegenPlugin` API](/docs/code/codegen#extending-codegen-the-codegenplugin-api)**, the reference for every type: `CodegenTargetContribution`, `CategoryDeclaration`, `DiscoverPattern`, and the registry/scaffold surfaces.
...
- [Configuration](/docs/ship/configuration), `modules.ts`, the `config/*.ts` convention, `runtimeConfig` / `appConfig` / `authConfig`, and `packageConfig` in context.
...
- [Routes](/docs/code/routes) and [Jobs](/docs/code/jobs), other categories a module contributes.
- [Seeds](/docs/schema/seeds), bootstrap and demo data a module can contribute.
- [Building a plugin](/docs/guides/build-a-plugin), author the `CodegenPlugin` a module ships, plus the smaller per-feature seams (custom field type, operator, adapter, block).
- [Codegen](/docs/code/codegen), the pipeline that reads `modules.ts`, merges modules, and the full `CodegenPlugin` API reference.
