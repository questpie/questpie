...
For singleton services, `dispose(instance)` runs on `app.destroy()`. The
framework closes request services after their HTTP request, queue attempt, seed
or top-level operation. Standalone generated contexts own their scope; the
[lifecycle reference](/docs/code/services/lifecycles) shows the required
`await using` form. Use `dispose` to close what `create` opened.
