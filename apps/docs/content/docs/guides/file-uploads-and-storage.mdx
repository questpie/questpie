...
Files are stored in the `./uploads` directory and served via the app file endpoint.
...
const url = await app.storage.disk().getSignedUrl("private/document.pdf", {
...
const response = await fetch("/api/collections/barbers/upload", {
  method: "POST",
  body: formData,
});
...
- **Large file limits** — Check your reverse proxy (nginx, Cloudflare) upload size limits. They may be lower than your app `maxSize`.
