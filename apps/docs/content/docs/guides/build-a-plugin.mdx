...
src/questpie/server/
  ...
  plugin.ts            # the plugin, codegen only
...
themselves. Give the plugin its own entry point, like `@acme/webhooks/plugin`.
A config file importing it then does not pull in your runtime code.
[Publishing](/docs/code/modules/publishing) has the layout.

## Where each topic lives

| Topic                                   | Page                                                           |
| --------------------------------------- | -------------------------------------------------------------- |
| Every field on the contract, in full    | [Plugins](/docs/code/codegen/plugins)                          |
...
| Your own `collection()` method          | [Builder methods](/docs/guides/build-a-plugin/builder-methods) |
