...
2. Call `createFetchHandler(app, config)` with that request
...
The handler accepts a `Request` and returns `Promise<Response | null>`. It returns `null` when the URL doesn't match any app route, allowing fallthrough to other handlers.
...
If your framework doesn't support streaming responses, realtime features won't work — but all other app functionality will.
