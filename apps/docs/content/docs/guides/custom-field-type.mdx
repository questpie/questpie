...
## Step 2: Use in a Collection

Once registered via module augmentation, use the field in any collection:

```ts
// collections/brands.ts
import { collection } from "#questpie";

export const brands = collection("brands").fields(({ f }) => ({
  name: f.text({ required: true }),
  primaryColor: f.color({ swatches: ["#FF5733", "#33FF57", "#3357FF"], required: true }),
...
field({ type: "color" })  →  getMetadata() emits { type: "color" }
         ↓                              ↓
module augmentation             qa.fields({ color: { type: "color", component } })
         ↓                              ↓
