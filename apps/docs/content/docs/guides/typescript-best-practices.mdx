...
description: Leveraging TypeScript for a type-safe QUESTPIE CMS experience.

---

# TypeScript Best Practices

QUESTPIE CMS is built with TypeScript from the ground up to provide an unparalleled developer experience with full type safety.

## End-to-End Type Safety

One of the core strengths of QUESTPIE CMS is its ability to provide end-to-end type safety, from your database schema definitions to your client-side applications.
...
app.post("/api/posts", async (c) => {
  ...
  const newPost = await cms.api.collections.posts.create(
    validatedData,
    c.get("cmsContext")
  );
