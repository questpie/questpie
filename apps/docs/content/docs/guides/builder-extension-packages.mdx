...
Modules are composed via the `config()` call:
...
```ts
// App server
import { collection, config } from "questpie";
import { admin } from "@questpie/admin/server";
import { ecommerceModule } from "@my-org/ecommerce-plugin/server";

export default config({
  modules: [admin(), ecommerceModule],
  collections: {
...
// App client — client-side patterns stay as-is
import { qa, adminModule } from "@questpie/admin/client";
import { ecommerceAdminModule } from "@my-org/ecommerce-plugin/client";

export const admin = qa<typeof app>()
  .use(adminModule)
...
- **Building the app inside the module** — modules export builder state, the app uses `config()`
