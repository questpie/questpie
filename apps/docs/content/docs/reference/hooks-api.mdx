...
| `db` | `Database` | Typed database client (may be a transaction) |
...
All context properties are fully typed via `AppContext` augmentation — no manual type casting needed.

### Usage

```ts
afterChange: async ({ data, operation, queue, logger }) => {
  logger.info("Record changed", { id: data.id });
  if (operation === "create") {
    await queue.sendNotification.publish({ id: data.id });
  }
}
```
...
```ts
(ctx: { operation: string; session?: Session; logger: Logger; ... }) => void | Promise<void>
```
...
```ts
(ctx: { data: TSelect; fromStage: string; toStage: string; session?: Session; db: Database; queue: QueueClient; ... }) => void | Promise<void>
```
