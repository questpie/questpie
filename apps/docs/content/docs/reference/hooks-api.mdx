...
<Callout type="info">For usage examples and patterns, see the [Hooks Guide](/docs/collections/hooks).</Callout>

## Overview

All available hook types and their signatures:
...
```mermaid
flowchart LR
  beforeOperation --> beforeValidate --> SchemaValidation["Schema Validation"] --> beforeChange --> DBInsert["DB INSERT"] --> afterChange --> afterRead
```
...
```mermaid
flowchart LR
  beforeOperation --> beforeValidate --> SchemaValidation["Schema Validation"] --> beforeChange --> DBUpdate["DB UPDATE"] --> afterChange --> afterRead
```
...
```mermaid
flowchart LR
  beforeOperation --> beforeDelete --> DBDelete["DB DELETE"] --> afterDelete
```
...
```mermaid
flowchart LR
  beforeOperation --> beforeRead --> DBSelect["DB SELECT"] --> afterRead
```
...
```mermaid
flowchart LR
  beforeTransition --> VersionSnapshot["Version Snapshot"] --> afterTransition
```
...
| `recordId`    | `string \| number`    | Collection record ID       |
| `fromStage`   | `string`              | Stage transitioning from   |
| `toStage`     | `string`              | Stage transitioning to     |
| `scheduledAt` | `Date \| undefined`   | Future transition time     |
| `locale`      | `string \| undefined` | Current locale             |
| `accessMode`  | `"system" \| "user"`  | Access mode for transition |
...
| Custom        | `AppContext` extensions | Services from `services/` |
