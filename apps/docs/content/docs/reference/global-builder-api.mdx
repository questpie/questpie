...
`GlobalBuilder` defines a singleton document — a single record with no list view, only a form. Created via `global(name)` from `#questpie`.
...
Global builders are placed in the `globals/` directory and auto-discovered by codegen. Export the builder from each file:
