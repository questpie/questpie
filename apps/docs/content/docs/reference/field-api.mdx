...
`inputFalse()`, `outputFalse()`, and field `.access()` rules are enforced as framework-level access primitives in user-mode CRUD. `inputFalse()` rejects create/update writes to that field, including nested object paths. `outputFalse()` redacts the field from read/create/update responses. Field access handlers receive `{ req, user, doc, operation }`; `req` is present only when the operation runs from an HTTP request. System-mode CRUD still bypasses field access for server-side jobs, hooks, migrations, and seeds. OpenAPI request/response schemas use the same flags: `inputFalse()` becomes response-only/read-only, and `outputFalse()` becomes request-only/write-only.
