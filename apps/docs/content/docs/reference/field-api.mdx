...
`inputFalse()` and `outputFalse()` are enforced as framework-level access primitives in user-mode CRUD. `inputFalse()` rejects create/update writes to that field, including nested object paths. `outputFalse()` redacts the field from read/create/update responses. System-mode CRUD still bypasses field access for server-side jobs, hooks, migrations, and seeds.
