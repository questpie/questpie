...
The admin panel, admin config RPC, widget loaders, action handlers, and reactive field handlers are available only when `session.user.role === "admin"`. A logged-in user with `role: "user"` or no role is not allowed into the admin surface.
