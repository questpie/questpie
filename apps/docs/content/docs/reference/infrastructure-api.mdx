...
Reference for all infrastructure service methods. These are available on the `app` instance and also directly in handler context (`{ queue, email, storage, kv, logger }`).

## Queue — `app.queue`

Queue methods are job-scoped. Each job defined in your `jobs/` directory becomes a property on the queue — e.g., `queue.sendConfirmation.*`.
...
### Typed Queue in Handlers

Queue is available directly in the handler context:

```ts
afterChange: async ({ data, queue }) => {
  await queue.sendConfirmation.publish({ appointmentId: data.id });
}
```
