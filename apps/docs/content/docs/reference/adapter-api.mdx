...
### `createFetchHandler(app, config?)`

Create the main HTTP handler for all API routes.
...
|---|---|---|---|
| `app` | `Questpie` | Yes | Built app instance |
| `config.basePath` | `string` | No | URL prefix (default: `"/"`) |
| `config.rpc` | `RpcRouter` | No | RPC router tree |

**Returns:** `(request: Request, context?: AdapterContext) => Promise<Response | null>`

Returns `null` if the URL doesn't match any app route, allowing fallthrough to other handlers.

### `createAdapterRoutes(app, config?)`
...
```ts
const routes = createAdapterRoutes(app, { basePath: "/api" });
...
const app = new Hono();
app.route("/api", questpieHono(app, { rpc: appRpc }));
```
...
const app = new Elysia()
  .use(questpieElysia(app, { rpc: appRpc, basePath: "/api" }));
```
...
const { GET, POST, PATCH, DELETE } = questpieNextRouteHandlers(app, {
  basePath: "/api",
  rpc: appRpc,
});
