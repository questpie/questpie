...
For stage reads, `transitionStage()`, scheduled transitions, and admin toolbar behavior, see [Versioning & Workflow](/docs/backend/data-modeling/versioning-workflow).
...
Create and update enforce these schemas server-side. Each field's own schema (including `.zod()` transforms, email format, select enums, array shapes) is the source of truth for its key; system fields (`id`, timestamps) and relation/upload foreign keys validate against their column shape. `refine` applies on top of the field schema, `exclude` removes the key from validation entirely.
...
`visibility` governs file serving only — `"public"` means the file BYTES are
servable by key without authentication, `"private"` requires a signed-token
URL. Listing and reading the upload ROWS always resolves through the normal
access chain (`.access()` → `defaultAccess` → session required). Use the
`serve` access kind to override the serving rule — see
[Access Control](/docs/backend/rules/access-control).
...
Merge another builder for the **same collection name** into this one. This is the way to extend a collection a module already provides — don't redefine it from scratch (registering the same key replaces the module's collection wholesale, dropping its fields, hooks, and auth wiring).
...
- **Fields, virtuals, relations, indexes, options, extension keys** — combine by key; the merged-in builder wins on conflicts. Unresolved relation fields from both sides are preserved.
- **Hooks** — concatenate; both builders' hooks run.
- **Access** — merges by rule key; the merged-in builder's rules win.
- **`title`, `searchable`, `upload`, `validation`** — the merged-in builder's value wins when set.
- **`.fields()` after `.merge()`** is cumulative — it adds to and overrides merged fields by key, never wipes them, so merge-first recipes are safe.

The result is fully typed: field types from both builders flow into `$infer` and CRUD types.
