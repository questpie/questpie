...
<Callout type="info">For usage examples and patterns, see the [Collections Guide](/docs/collections).</Callout>

## `collection(name)`

Creates a new collection builder. `name` becomes the database table name and the key used to access the collection via `context.collections`.
...
| ------------- | --------------------------- | ---------- | ------------------------------------------------------------ |
| `schema`      | `string`                    | `"public"` | Postgres schema to place the collection's tables under       |
| `timestamps`  | `boolean`                   | `true`     | Adds `createdAt` and `updatedAt` columns                     |
| `softDelete`  | `boolean`                   | `false`    | Adds `deletedAt`; records are marked deleted instead of removed |
| `versioning`  | `boolean \| VersioningConfig` | `false`  | Creates a versions table for record history                  |

```ts
.options({
  schema: "auth",        // Places tables in the "auth" Postgres schema
...
Places all four collection tables (main, i18n, versions, i18n_versions) under a named Postgres schema instead of `public`. Cross-schema foreign keys are rendered as `REFERENCES "other_schema"."table"("id")`, making it safe to partition multiple QUESTPIE apps on one Postgres instance (e.g., `auth.*`, `web.*`, `brain.*`).
...
For stage reads, `transitionStage()`, scheduled transitions, and admin toolbar behavior, see [Versioning & Workflow](/docs/collections/versioning).
