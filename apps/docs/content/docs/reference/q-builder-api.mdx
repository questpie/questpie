...
**Parameter:** `Record<string, JobDefinition>` — map of job names to definitions created with `job()`.
...
Add backend translation messages. These power `app.t()` for type-safe translations.
...
Define a background job. Import `job` from `"questpie"`.
...
Define a server function. Import `fn` from `"questpie"`.

```ts
import { fn } from "questpie";

const getAvailableSlots = fn({
...
After build, export `App` once and reuse it in hooks/access/RPC/jobs:
...
Finalize the builder into a runnable app instance.
...
**Returns:** `Questpie<TConfig>` — the runtime app instance with typed access to all APIs.

## Runtime Instance

After `.build()`, the app instance exposes:
