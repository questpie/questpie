...
`CollectionBuilder` defines a single collection's schema, behavior, and admin configuration. Created via `collection(name)` from `#questpie`.
...
These methods configure how the collection appears in the admin panel. They are available when `adminModule` is registered in `modules.ts`.
...
Collection builders are placed in the `collections/` directory and auto-discovered by codegen. Export the builder from each file:
...
Run `questpie generate` to include them in the app entrypoint.
