...
The type parameter connects the RPC builder to your app instance for typed `app` access in handlers.

If you do not want to pass generic parameters to `rpc()`, you can still type handler context with [`getContext<App>()`](/docs/server/type-safe-contexts).
...
For strongly typed `app`, `db`, and `session`, use `rpc<App>()`, or cast inside the handler:
...
```
POST /api/rpc/booking/getAvailableSlots
