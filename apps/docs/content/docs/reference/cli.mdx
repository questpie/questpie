...
The CLI discovers your app configuration from `questpie.config.ts` and the file convention structure. Collections, globals, functions, jobs, and migrations are all auto-discovered from their conventional directories.
...
Migrations in the `migrations/` directory are automatically discovered by codegen and loaded at runtime.
...
The CLI saves generated migrations to `./src/migrations` by default. Migrations are automatically discovered by codegen from this directory.
...
# Edit src/questpie/server/collections/posts.ts
...
Migrations are automatically discovered from the `migrations/` directory — no manual imports needed.
...
Ensure `questpie.config.ts` exists in your server directory:
...
Ensure migration files exist in the `migrations/` directory and have been discovered by running `questpie generate`.
...
- [Config API](/docs/reference/q-builder-api) - Runtime config reference
