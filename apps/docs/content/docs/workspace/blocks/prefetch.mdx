...
## Access Control in Prefetch

Declared `with` expansion of `f.upload()` fields populates through the PARENT
row's read decision — if the caller could read the row containing the blocks,
its upload fields expand even when the upload collection itself is not
readable for that caller (field-level read rules still apply). Plain
`f.relation()` fields keep normal target-collection access.

Custom prefetch functions run with the caller's access. If a function fetches
upload records by editor-curated ids stored in block values (content the
parent row already authorized), pass `{ accessMode: "system" }` explicitly and
keep the query bounded to those ids:
...
Never use system mode for queries influenced by request input.
