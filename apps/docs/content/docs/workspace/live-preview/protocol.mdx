...
<Callout type="info">
	The Preview V2 protocol is implemented end-to-end. Admin-side plumbing is
	exposed through `VisualEditFormHost` / `useVisualEditPreviewBridge` (see
	[Visual Edit Workspace](./visual-edit)), and `useCollectionPreview` on the
	iframe side already handles `INIT_SNAPSHOT` / `PATCH_BATCH` / `COMMIT` /
	`FULL_RESYNC` alongside the V1 `PREVIEW_REFRESH` flow.
</Callout>
