...
<Callout type="info">
	Preview V2 ships today as the **Visual Edit Workspace** — opt in per
	collection with `.form(({ v }) => v.visualEditForm({...}))` and the
	workspace's bridge + patcher handle the V2 transport automatically.
	`useCollectionPreview` already understands V2 messages (`INIT_SNAPSHOT`,
	`PATCH_BATCH`, `COMMIT`, `FULL_RESYNC`) alongside the V1
	`PREVIEW_REFRESH` flow. The export names in the snippets below
	(`useQuestpiePreview`, `PreviewRoot`, `PreviewBlock`) are illustrative —
	the shipped names are `VisualEditFormHost`, `VisualEditWorkspace`,
	`useCollectionPreview`, `PreviewProvider`, `PreviewField`,
	`BlockScopeProvider`. See [Visual Edit Workspace](./visual-edit) for the
	turn-key flow.
</Callout>
