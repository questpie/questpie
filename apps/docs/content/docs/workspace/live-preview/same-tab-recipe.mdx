...
<Callout type="info">
	Preview V2 ships today as the **Visual Edit Workspace**. For the standard
	collection-edit flow you don't need the lower-level recipe on this page —
	enable `.form(({ v }) => v.visualEditForm({...}))` and the workspace
	handles the wire transport for you. See
	[Visual Edit Workspace](./visual-edit). Read this page when you need a
	**custom iframe runtime** that consumes the patch protocol directly. The
	illustrative API names below (`useQuestpiePreview`, `PreviewRoot`) map to
	the shipped `useCollectionPreview` + `PreviewProvider` exports.
</Callout>
