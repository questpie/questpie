...
If the same collection is reachable through the public client or HTTP API, protect anonymous reads in `.access()` too:
...
This keeps the collection policy aligned with the route policy. A public visitor can fetch the published snapshot, but cannot omit `stage` to read the working draft. Preview and admin reads still work because the preview token resolves to the editor session.
...
<Cards>
  <Card title="Collections" href="/docs/collections" description="Collection builder options" />
  <Card title="Globals" href="/docs/collections/globals" description="Global builder options" />
  <Card title="Hooks API" href="/docs/reference/hooks-api" description="Transition hook context" />
  <Card title="Built-in Actions" href="/docs/admin/actions/builtin" description="Admin toolbar actions" />
</Cards>
