...
`f.blocks()` and the `block()` factory only exist when `adminModule` is registered in `src/questpie/server/modules.ts`. The admin module registers the `blocks` (and `richText`) field types, and codegen discovers your `blocks/` files and wires `f.blocks()` onto the generated `f`. Without it, `f.blocks()` is not on the field proxy. Source: `packages/admin/src/server/fields/blocks.ts`, `packages/admin/src/server/modules/admin/block/block-builder.ts`.
</Callout>
...
Source: `packages/admin/src/server/fields/blocks.ts`.
...
Source: `packages/admin/src/server/modules/admin/block/block-builder.ts`.
...
| `hidden` | `boolean` | Hide from the picker (e.g. deprecated blocks). |
...
Source: `packages/admin/src/server/augmentation/form-layout.ts` (`AdminBlockConfig`, `BlockCategoryConfig`).
...
Then `category: sections(c)` in each block's `.admin()`. Source: `examples/tanstack-barbershop/src/questpie/server/blocks/_categories.ts`.
</Callout>

### `.fields()` — the block's data

`.fields()` is the same callback as collections and globals — `({ f }) => ({ ... })` — so [every field type and chain method](/docs/concepts/fields) is available, including `.required()`, `.localized()`, `.default()`, `.label()`, relations, and uploads. Each field's per-field admin config (e.g. reactive `hidden`) works exactly as it does on a collection.
...
Source: `packages/admin/src/server/modules/admin/block/block-builder.ts` (`form`), `packages/admin/src/server/augmentation/form-layout.ts` (`FieldLayoutItem`).
...
Children render via the `children` prop your renderer receives (see [Rendering](#rendering-on-the-frontend)). Source: `examples/city-portal/src/questpie/server/blocks/columns.ts`.
...
Each expanded field becomes `ExpandedRecord | null` under `_data[blockId][field]` (an array of records for `.multiple()` relations). Only fields whose metadata is a relation are expanded — a non-relation field name in `with` is silently ignored, never partially resolved. Source: `packages/admin/src/server/modules/admin/block/block-builder.ts` (prefetch overload 2), `prefetch.ts` (`expandDeclaredFields`).
...
`ctx` is an `AppContext` plus `{ blockId, blockType, locale? }`, so `ctx.collections.<name>.find(...)`, `ctx.db`, and `ctx.session` are available. Source: `packages/admin/src/server/modules/admin/block/block-builder.ts` (`BlockPrefetchContext`, prefetch overload 1), `examples/city-portal/src/questpie/server/blocks/latest-news.ts`.
...
On key conflicts, loader/function data wins over expanded data (it is merged on top). Source: `packages/admin/src/server/modules/admin/block/prefetch.ts` (`processBlocksDocument`).
...
| `selectedBlockId`, `onBlockClick`, `onBlockInsert` | — | Editor / live-preview wiring (select + insert affordances). |
...
At runtime, `BlockRenderer` passes each renderer component `id`, `type` (the block type name, an extra prop the typed shape below omits), `values`, `data`, and the rendered `children` — and nothing else. The `isSelected` / `isPreview` fields in the renderer-props type are **reserved and not wired by `BlockRenderer`**: they are always `undefined` here, so don't build rendering logic on them (selection state lives on the editor wrapper, not the renderer). Stick to `values`, `data`, and `children`. Source: `packages/admin/src/client/blocks/block-renderer.tsx` (lines 118-126).
</Callout>

<Callout type="info" title="Same renderers power live preview">
Wrap fields in `PreviewField` to make them inline-editable in live preview, and forward `selectedBlockId` / `onBlockClick` / `onBlockInsert` from `useCollectionPreview()` — `BlockRenderer` then adds click-to-select and insert affordances in preview mode while rendering the exact same components in production. Source: `packages/admin/src/client/blocks/block-renderer.tsx`, `examples/tanstack-barbershop/src/components/pages/PageRenderer.tsx`.
</Callout>

{/* TODO @user: add a screenshot of the admin block editor — picker open with categorized blocks on the left, the block tree with a selected block, and its fields in the inspector. */}
...
| `isEmpty` / `isNotEmpty` | — | Documents with no / at least one root block. |
| `isNull` / `isNotNull` | — | Documents where the field is / isn't null. |
...
`hasBlockType` and `blockCount` inspect `_tree` only (root level for the count), via Postgres JSONB functions (`jsonb_array_elements`, `jsonb_array_length`). Source: `packages/admin/src/server/fields/blocks.ts` (`getBlocksOperators`).
...
In app code, always import `block` from `#questpie/factories`. The generated factory wraps the field proxy with your enabled modules so `f.richText()` / `f.blocks()` are available inside a block's `.fields()`; the bare `block()` from `@questpie/admin/server` only sees builtin fields. Source: `examples/tanstack-barbershop/src/questpie/server/.generated/factories.ts`.
</Callout>

## Related

- [Fields](/docs/concepts/fields) — every field type and chain method you use inside `block().fields()`, plus `f.blocks()` in the field reference and how to build your own field type.
- [Collections](/docs/concepts/collections) — where the `f.blocks()` field lives and how the document is read and stored.
- [Relations](/docs/concepts/relations) — what `.prefetch({ with })` expands, and the `with` hydration it reuses.
- [Hooks](/docs/concepts/hooks) — the `afterRead` lifecycle that prefetch runs in (and the open-transaction caveat).
- [Access control](/docs/concepts/access-control) — the read rules that apply to expanded relations (and the upload-inherits-parent-row exception).
