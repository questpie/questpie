...
<Callout type="warn" title="Don't send inside an open transaction">
	...
	transaction rolls back. See [Hooks](/docs/schema/hooks) for the commit
	lifecycle.
</Callout>
...
- [Hooks](/docs/schema/hooks), fire transactional emails from `onAfterCommit` after a write commits.
...
- [Validation](/docs/schema/validation), the Zod schema on a template is the same validation primitive used across the framework.
