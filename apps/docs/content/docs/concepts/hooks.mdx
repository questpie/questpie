...
<Callout type="warn">
	...
	*only* for the `update` operation. On those two hooks its type is
    ...
    hooks, `afterDelete`, `beforePurge`, and `afterPurge`) `ctx.original` is
    `never`-typed, you can't read it
...
| `beforePurge`      | purge                               | the deleted record         | none                 | **yes**, propagates + rolls back                                            |
| `afterPurge`       | purge                               | the purged record          | none                 | **yes**, propagates + rolls back                                            |
...
- **`beforePurge`**, guard permanent removal of an already soft-deleted record. Throwing rolls back the purge.
- **`afterPurge`**, schedule purge cleanup or record transactional facts. Unlike `afterDelete`, throwing is fatal and rolls back the purge. Use `onAfterCommit` for external side effects. See [Soft delete and durable retention](/docs/concepts/soft-delete-retention).
