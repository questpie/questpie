...
| `afterDelete`      | delete                              | the deleted record         | same deleted record  | no, caught + logged                                                         |
...
Ordinary `afterChange` and `afterDelete` hooks retain their compatibility error
semantics. When an application-side database effect must either commit with the
row or abort the row mutation, declare it separately with
...
`afterChange`, `afterDelete`, and `afterPurge` accept one handler or an array,
merge across repeated builder calls and receive the same typed context as the
corresponding hook. Each handler is awaited while the outer transaction is
open. A thrown or rejected error always fails the CRUD call and rolls back the
row, QUESTPIE realtime/outbox rows, and transaction-aware work performed
...
A nested managed transaction does not run an effect a second time.

Ordinary hooks run first, followed by the matching transactional effect.
`afterRead` still runs after the transaction closes. Do not use transactional
effects for direct email, HTTP, or other work that cannot join the database
transaction; use `onAfterCommit` or a transaction-aware queue/outbox instead.
...
`afterChange`, `afterDelete`, `afterTransition`, and
`.transactionalEffects()` run **inside** the CRUD transaction.
...
_after_ the transaction closes.)
...
<Callout type="warn">
	**Keep external effects out of an `after*` hook.** Transactional database
	writes through `ctx.db` are allowed when they intentionally commit or roll
	back with the operation; the core purge upload hook uses this for its durable
	cleanup outbox. Dispatching a job, calling `realtime.notify`, or using an
	independent database connection directly can escape the transaction or
	deadlock against it. Put non-transactional effects in `onAfterCommit`.
</Callout>
