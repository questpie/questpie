...
|---|---|---|---|---|
| `beforeOperation` | create, update, read, delete | input (type depends on op) | — | yes |
| `beforeValidate` | create, update | **mutable** input | — | yes |
| `beforeChange` | create, update | **validated** input | — | yes |
...
| `beforeRead` | read | query options | — | yes |
| `afterRead` | create, update, read, delete | the record | record (update only) | **yes** — propagates to the caller |
| `beforeDelete` | delete | record to be deleted | — | yes |
| `afterDelete` | delete | the deleted record | — | no — caught + logged |
| `beforeTransition` | workflow transition | the record | — | yes |
| `afterTransition` | workflow transition | the record | — | no — caught + logged |
...
<Callout type="warn">
	**Cycle rule for the generated context aliases.** Import `HookRuleContext` /
	...
	the package-level `HookContext` / `AccessContext` from `questpie` instead —
	importing the generated `#questpie` barrel from a collection creates a codegen
	cycle.
</Callout>
...
{/* TODO @user: add an admin screenshot showing a hook-derived field (e.g. an auto-generated slug) populated after save. */}
