...
`afterChange`, `afterDelete`, and `afterTransition` run **inside** the CRUD transaction. `queue.<job>.publish()` is transaction-aware: call and await it directly so pg-boss joins the transaction or the framework commits an external-adapter dispatch intent with the data. Direct email, HTTP calls, and other non-transactional effects must still wait until commit. (`afterRead` runs _after_ the transaction closes.)
