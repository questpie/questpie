...
The `f.upload()` *field* is a relation **to** an upload collection; the bytes live in that target collection (default `"assets"`). Turning a collection *itself* into the byte store (storage adapter, URL generation, signed URLs, direct-to-storage uploads) is a different concern with the same name. See [Storage adapters](/docs/adapters/storage) for where bytes are persisted and served. Source: `packages/questpie/src/server/modules/core/fields/upload.ts` (the `upload` factory).
</Callout>

## Constructor argument

`f.upload()` takes a single optional config object. Every key is optional. Source: `packages/questpie/src/server/modules/core/fields/upload.ts:53` (`UploadConfig`), `upload.ts:89` (factory).

| Option | Type | Default | Effect |
|---|---|---|---|
| `to` | `string` | `"assets"` | Target upload collection the FK points at. Narrows the field's typed target. |
| `mimeTypes` | `string[]` | — | Accepted by the config but **currently ignored** — no effect anywhere (see gotcha). Use `.admin({ accept })` to constrain the picker. |
| `maxSize` | `number` | — | Accepted by the config but **currently ignored** — no effect anywhere (see gotcha). Use `.admin({ maxSize })` to constrain the picker. |
| `through` | `string` | — | Junction collection name. Setting it makes the field **many-to-many** (see below). |
| `sourceField` | `string` | — | Name of the source FK column on the junction (M2M only). |
| `targetField` | `string` | — | Name of the target FK column on the junction (M2M only). |

The single-upload field is always `varchar(36)`, data `string`, schema `z.string().uuid()`. Source: `packages/questpie/src/server/modules/core/fields/upload.ts` (factory `columnFactory` / `schemaFactory`).

<Callout type="warn" title="`mimeTypes` / `maxSize` are accepted but currently ignored">
`mimeTypes` and `maxSize` are destructured from the config and then never used — they reach **no** part of the field. They are not applied to the column, not to the derived Zod schema, and not to the field's metadata (the `metadataFactory` emits no `mimeTypes`/`maxSize`, and `RelationFieldMetadata` has no such keys), so they have no admin-picker effect either. Today they are dead config. To actually constrain the admin file picker, set them on the **field's admin config** instead — `f.upload().admin({ accept: "image/*", maxSize: 5_000_000 })` — which flows to the picker's `accept` / `maxSize` props. Enforce real upload limits on the target upload collection / storage adapter. Source: `packages/questpie/src/server/modules/core/fields/upload.ts` (`mimeTypes`/`maxSize` destructured at the factory + `create` but never referenced; `metadataFactory` does not read them), `packages/questpie/src/server/fields/types.ts` (`RelationFieldMetadata`), `packages/admin/src/client/components/fields/upload-field.tsx` (picker `accept` / `maxSize` come from the field's admin config).
</Callout>
...
Several assets, joined through a junction collection. Setting `through` makes the field **virtual** (no column on this table; the links live in the junction), with a runtime schema of `z.array(z.string().uuid())` — so the value you write/read back is `string[]`. Uses the **toMany** operator set. Source: `packages/questpie/src/server/modules/core/fields/upload.ts` (factory: `virtual: isM2M`, `schemaFactory` → array of uuid, `operatorSet: toManyOps`).
...
Several assets stored *inline* as a JSONB array of IDs on this row (no junction). `.multiple()` switches the field to a virtual JSONB column with a runtime schema of `z.array(z.string().uuid())` — the value you write/read back is `string[]`. Uses the **multiple** operator set. Source: `packages/questpie/src/server/modules/core/fields/upload.ts` (`uploadFieldType.methods.multiple` → `virtual: true`, `columnFactory: null`, `schemaFactory` → array of uuid, `operatorSet: multipleOps`).
...
Both hold many assets. **`through`** stores the links in a separate junction collection (queryable, supports the `some` / `none` / `every` quantifiers). **`.multiple()`** stores the IDs inline as a JSONB array on the row (no extra table, queried with array operators like `contains`). Use `through` when the relationship itself needs rows (ordering, pivot data); use `.multiple()` for a simple ordered list of files. Source: `packages/questpie/src/server/modules/core/fields/upload.ts` (`through` → `virtual: isM2M` vs `multiple` → inline jsonb).
</Callout>

## Chained methods

This method is specific to `upload` (on top of the [shared modifiers](/docs/concepts/fields) every field has). It returns a new immutable field. Source: `packages/questpie/src/server/modules/core/fields/upload.ts` (`UploadFieldMethods`, `uploadFieldType.methods.multiple`).
...
**Single upload** — the **belongsTo** operator set (`belongsToOps`). Source: `packages/questpie/src/server/fields/operators/builtin.ts:551`.
...
**Many-to-many (`through`)** — the **toMany** operator set (`toManyOps`): the relation quantifiers. Source: `builtin.ts:576`.
...
**Inline array (`.multiple()`)** — the **multiple** operator set (`multipleOps`), querying the JSONB array. Source: `builtin.ts:590`.
...
{/* TODO @user: screenshot of an upload field rendered in the admin form (file picker with upload + "Browse Library"). */}
