...
These methods are specific to `text` (on top of the [shared modifiers](/docs/concepts/fields) every field has). Each returns a new immutable field, so they chain in any order.
...
On a text field, `.min(n)` / `.max(n)` constrain the **number of characters** (they map to `minLength` / `maxLength`). On [`f.number()`](/docs/concepts/fields/number) the same method names constrain the numeric **value**. Same names, different meaning — sized by the field's type.
</Callout>
