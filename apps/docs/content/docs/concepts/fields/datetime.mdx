...
You rarely hand-write `createdAt` / `updatedAt`. A collection's [`timestamps` option](/docs/concepts/collections#timestamps) (on by default) adds both for you. Declare your own `f.datetime().autoNow()` columns only when you want extra managed timestamps (e.g. `publishedAt`, `archivedAt`) or different names.
</Callout>
