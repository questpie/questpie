...
`.array()` is a **chain modifier**, not a field type — you never call `f.array()`. You build the element field first, then wrap it: `f.text(40).array()`. It is one of the [shared modifiers](/docs/concepts/fields#shared-methods) every field carries. Source: `packages/questpie/src/server/fields/field-class.ts:187`.
</Callout>
...
`.array()` rewrites four things on the field state. Source: `packages/questpie/src/server/fields/field-class.ts:187`, `field-class-types.ts:118`.
...
`.minItems(n)` and `.maxItems(n)` are no-ops unless the field is an array — they refine the `z.array(...)` schema (`arraySchema.min(n)` / `.max(n)`). On a non-array field they set state that nothing reads. Source: `packages/questpie/src/server/fields/derive-schema.ts:80`, `field-class.ts:336`.
</Callout>
...
An array field uses the **multi-value operator set** (`selectMultiOps`). Every operand element is typed to the inner field's filter value — a `number[]` filters on `number`, a `f.select([...]).array()` filters on its literal union. Source: `packages/questpie/src/server/fields/operators/builtin.ts:387`.
...
The generated `where` type lists a single-item `contains: Item` operator, but the runtime `selectMultiOps` set has **no** `contains` — only `containsAll` and `containsAny`. Using `{ tags: { contains: "ts" } }` type-checks but is silently ignored at query time. Use `containsAny: ["ts"]` to match a single element. Source: type at `packages/questpie/src/server/fields/operators/builtin.ts` (`ArrayWhereInput`); runtime set at `builtin.ts:387` (no `contains` key).
</Callout>
...
An array field contributes `Item[]` to the generated row and insert types, and an [`ArrayWhereInput`](/docs/concepts/relations) shape to the `where` type. The element filter value is derived from the inner field, not the stored data — a `f.datetime().array()` filters on the inner field's date input, not `Date[]`. Source: `packages/questpie/src/server/fields/field-types.ts:84`.
...
{/* TODO @user: screenshot of an array field rendered in the admin form (a repeatable list of the inner field's input). */}
