...
`f.object()` has **no type-specific chain methods** — it is refined entirely with the [shared modifiers](/docs/concepts/fields) every field has (`.required()`, `.default()`, `.label()`, `.localized()`, `.array()`, `.access()`, …). Source: `object.ts:124` (the `fieldType` registers no `methods`).
...
`object` uses the **object operator set** (`objectOps`), built on structural JSONB operations. The whole set is JSONB-path aware: the same operators work on a top-level object field and on a key *inside* a nested object. Source: `packages/questpie/src/server/fields/operators/builtin.ts:459`.
...
{/* TODO @user: screenshot of an object field rendered in the admin form (grouped nested inputs). */}
