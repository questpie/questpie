...
- To **upload a file** and store a link to it, reach for the [Upload field](/docs/concepts/fields/upload) (`f.upload()`), or the collection-level [`.upload()`](/docs/schema/collections#uploads) builder, not a `url` field.
...
- [Validation](/docs/schema/validation), the auto-derived Zod schema and the `.zod()` escape hatch for stricter URL rules.
