...
On a `url` field, `.min(n)` / `.max(n)` constrain the **number of characters** (they map to `minLength` / `maxLength`), exactly as on [`f.text()`](/docs/concepts/fields/text). On `f.number()` the same method names constrain the numeric **value**. Same names, different meaning, sized by the field's type.
</Callout>
