...
`f.select()` takes one argument — either a **static options array** or a dynamic **`OptionsConfig`**. There is no options-object constructor. Source: `packages/questpie/src/server/modules/core/fields/select.ts:137`.
...
`value`, `label`, `description`, `disabled`, `icon`, and `className` are all serialized into the admin introspection metadata, so the dropdown and table cells render consistently without any per-project component override. Source: `select.ts:109`, `select.ts:180`.
...
`.enum(name)` is the only method specific to `select`, on top of the [shared modifiers](/docs/concepts/fields#shared-methods) every field has and `.array()` for multi-select (below). Source: `select.ts:55`, `select.ts:257`.
...
A single (non-array) select uses the **single-select operator set** (`selectSingleOps`) — equality and membership against the value union. Source: `packages/questpie/src/server/fields/operators/builtin.ts:365`.
...
A **multi-select** (`.array()`) uses `selectMultiOps` instead — the value is a `jsonb` array, so the operators are structural. Source: `builtin.ts:387`.
...
When the choices depend on other field values or come from a runtime source rather than a fixed list, pass an `OptionsConfig` instead of an array. The handler returns the options on demand and receives the current form data, search term, and pagination, so the admin dropdown can react to siblings, search, and lazy-load. Source: `packages/questpie/src/server/fields/reactive-types.ts:73`.
...
- [`f.relation()`](/docs/concepts/relations) — when the choices are rows in another collection and you want a typed FK + hydration instead of a stored string.
- [Validation](/docs/concepts/validation) — the auto-derived `z.enum` schema and the `.zod()` escape hatch for custom rules.
- [Relations](/docs/concepts/relations) — the full query language for combining the operators above.

{/* TODO @user: screenshot of a select field rendered in the admin form (dropdown with option icons/badges). */}
