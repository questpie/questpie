...
<Callout type="warn" title="The value is a STRING, not a Date">
	...
	[`f.datetime()`](/docs/concepts/fields/datetime) when the value needs a
	calendar date or instant.
</Callout>

## Chained methods

`f.time()` has **no type-specific chain methods**, it returns a plain field, so only the [shared modifiers](/docs/concepts/fields#shared-methods) apply (`.required()`, `.default("09:00:00")`, `.label()`, `.localized()`, `.array()`, `.virtual()`, …).
...
<Callout type="info" title="No `autoNow()` on time fields">
	...
	[`f.datetime()`](/docs/concepts/fields/datetime) methods (they set the column
	to the current `Date`). They do **not** exist on `f.time()`. For a literal
