...
`f.textarea()` declares a multi-line string. It stores an unbounded Postgres `text` column, reads back as a `string`, validates as a string (no length cap by default), and filters with the standard string operators — the same surface as [`f.text()`](/docs/concepts/fields/text), but it renders as a multiline textarea in the admin instead of a single-line input.

Reach for it when you have free-form prose that has no real length limit: a bio, an excerpt, an internal note, a description. For short, capped strings use `f.text(maxLength)`; for formatted/structured content use [`f.richText()`](/docs/concepts/fields/rich-text).

> **Prerequisites:** [Fields](/docs/concepts/fields) — the `f` proxy, how a field becomes a column + schema + filters + admin input, and the chain modifiers shared by every field type.
...
`f.textarea()` and `f.text({ mode: "text" })` both produce an unbounded Postgres `text` column with the same `string` type and the same string operators. The **only** difference is the type discriminator: `textarea` reports `type: "textarea"`, which drives the admin to a multiline editor; `text` reports `type: "text"` (a single-line input). Pick `textarea` when you want the multiline UI. Source: `packages/questpie/src/server/modules/core/fields/textarea.ts`.
</Callout>
...
`.localized()` is a shared modifier with one home — see [Fields → shared methods](/docs/concepts/fields#shared-methods) for how localized fields move to the i18n table and how labels accept `I18nText`.
...
The field primitives (`TextareaFieldState`, `TextareaFieldMethods`) are exported from `questpie` for plugin/library authors; app code reaches the value through the generated collection types above. Source: `packages/questpie/src/server/modules/core/fields/textarea.ts`.

{/* TODO @user: add a screenshot of a textarea field rendered in the admin form (multiline input). */}

## Related

- [Fields](/docs/concepts/fields) — the field model, the `f` proxy, and the chain modifiers shared by every type.
- [`f.text()`](/docs/concepts/fields/text) — single-line / length-capped strings; same operators, different admin UI.
- [`f.richText()`](/docs/concepts/fields/rich-text) — formatted content (TipTap JSON or markdown) when prose needs structure.
- [`f.email()`](/docs/concepts/fields/email) / [`f.url()`](/docs/concepts/fields/url) — string fields with built-in format validation.
