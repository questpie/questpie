...
`f.richText()` is contributed by `@questpie/admin`, not by the core builtins. It appears on `f` (alongside `f.blocks()`) only when the admin module is enabled and you import `collection` from `#questpie/factories`. Without the admin module, `f.richText` is not a registered type and `f.richText()` throws `Unknown field type`. Source: `packages/admin/src/server/fields/index.ts:52` (`adminFields = { richText, blocks }`), registered in `packages/admin/src/server/plugin.ts:92`.
</Callout>
...
`TipTapDocument` and `TipTapNode` are exported types — see [TypeScript](#typescript). Source: `packages/admin/src/server/fields/rich-text.ts:34`, `:49`.

## Constructor argument

`f.richText()` has two call signatures. The only option is the storage `mode`. Source: `packages/admin/src/server/fields/rich-text.ts:300`.
...
`mode` defaults to `"json"`. In json mode the derived Zod schema validates the `{ type: "doc", content?: TipTapNode[] }` shape recursively; in markdown mode it is a plain `z.string()`. The chosen mode is recorded on the field metadata as `outputMode`, which the editor reads to decide whether to emit JSON or a markdown string. Source: `rich-text.ts:240` (mode default), `:288` (`outputMode` on metadata).
...
`richText` adds **no type-specific chained methods** — its field-type definition declares only the state factory, no extra methods. You configure it entirely through the [shared modifiers](/docs/concepts/fields) every field has. Source: `rich-text.ts:314` (`richTextFieldType` has `create` only, no `methods`).
...
`richText` ships a small, content-aware operator set. The operators are the same names in both modes, but the implementation differs: json mode searches the text inside the JSON tree; markdown mode runs `ILIKE` on the text column. Source: `rich-text.ts:112` (json), `:193` (markdown).
...
Unlike text / number / select fields, a `richText` field's `where` slot is **not** narrowed to these operators. The operators are wired and enforced at runtime, but the generated `where` type for a richText column resolves to `Record<string, any>` — so TypeScript will *not* reject a bogus key or a wrong operand (e.g. `{ contains: 123 }` or `{ notAnOperator: true }` both compile). Use the table above as the contract; don't rely on autocomplete or the compiler to catch a typo. Source: `RichTextFieldState` carries only the generic `operators: OperatorSetDefinition` from `DefaultFieldState` (`rich-text.ts:221`; `field-class-types.ts:97`), and `ExtractWhereType` reads `operators.column` as `OperatorMap = Record<string, OperatorFn<any, any> | undefined>` (`field-class-types.ts:327`; `fields/types.ts:237`).
</Callout>

<Callout type="info" title="`contains` matches rendered text, not JSON keys">
In json mode, `contains` walks the document with `jsonb_path_query_array(col, '$..text')` and matches inside the extracted text nodes — so it searches the words an editor sees, not node types or attribute values. To filter on whether a *block type* exists, that's the [`f.blocks()`](/docs/concepts/blocks) field, which has `hasBlockType` / `blockCount` operators. Source: `rich-text.ts:116`.
</Callout>
...
{/* TODO @user: once `concepts/querying` exists, link it from the line above as the canonical home of the full query language — `AND` / `OR` / `NOT`, pagination, `orderBy`, and the jsonb operators (docs-map §1.5 / §4). */}
...
- **Need stackable, reorderable, nested content sections instead of one prose blob?** That's [`f.blocks()`](/docs/concepts/blocks) — a page-builder field, not a single editor.
...
The admin renders `f.richText()` with a TipTap editor whose toolbar and feature set are configurable through the field's `.admin()` config. You can pick a `preset` (`"minimal" | "simple" | "standard" | "advanced"`), toggle individual `features` (headings, links, images, tables, slash commands, character count, …), or enable image uploads to a collection. Source: `packages/admin/src/client/components/fields/rich-text-editor/presets.ts:12` (`RichTextPreset`), `types.ts:13` (`RichTextFeatures`), `types.ts:67` (`RichTextEditorProps`).
...
- [Blocks](/docs/concepts/blocks) — the page-builder sibling: stackable, reorderable, nested content sections instead of one prose document.
...
{/* TODO @user: nav-reconcile — once the Admin section exists (`admin/index`, reserved in docs-map §1.10), link it as `/docs/admin` from the `.admin()` Callout above and from this Related list. */}

{/* TODO @user: screenshot of the rich-text editor in the admin form (toolbar + a formatted document). */}
