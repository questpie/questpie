...
<Callout type="info">
  `f` is a proxy over the registered field types. Accessing an unknown type throws at build time — `Unknown field type: "x". Available types: …` — so typos surface immediately. Source: `packages/questpie/src/server/fields/builder.ts`.
</Callout>
...
<Callout type="warn">
  `.trim()`, `.lowercase()`, and `.uppercase()` set runtime state but are **not currently applied to the Zod schema** — `derive-schema.ts` only emits `maxLength` / `minLength` / `pattern` for strings, so these three do not transform or validate the value yet. To normalize input, use a `.zod()` refinement or a field hook instead. Source: `packages/questpie/src/server/fields/derive-schema.ts`.
</Callout>
...
Default mode is `"integer"`. `bigint` and `decimal` use Drizzle's `mode: "number"`, so they read back as JS `number` (not `bigint`/`string`). `.step(n)` adds a `value % n === 0` refinement. For numbers, `.min()` / `.max()` set **value bounds** (distinct from the string-length `.min()` / `.max()` on `text`). Source: `packages/questpie/src/server/modules/core/fields/number.ts`.

### Boolean

| Field | Constructor | Column | Read type | Type-specific methods |
|---|---|---|---|---|
| `f.boolean()` | none | `boolean` | `boolean` | — |

```ts
isActive: f.boolean().default(true).required(),
```

No type-specific methods — refine with the shared methods only. Source: `packages/questpie/src/server/modules/core/fields/boolean.ts`.
...
| `f.time(config?)` | `{ precision?: 0–6; withSeconds?: boolean }` | `time` (precision `0`) | `string` (`HH:MM:SS`) | — |
...
The `target` collection name is checked against your registry once codegen runs, so a typo'd target fails to type-check. Source: `packages/questpie/src/server/modules/core/fields/relation.ts`.
...
| `<field>.array()` | — (chain method) | `jsonb` | `T[]` of the inner read type | combine with `.minItems()` / `.maxItems()` |
| `f.from(column, zodSchema?)` | a Drizzle column builder (or `(name) => builder`) + optional Zod schema | whatever the column produces | from the Zod schema, or `unknown` | escape hatch for custom PG types |
...
// from — bring an arbitrary Drizzle column (e.g. PostGIS):
location: f.from(customGeometryColumn, z.object({ lat: z.number(), lng: z.number() })),
```

Use `f.object()` when the nested shape is fixed and you want per-key validation; use `f.json()` when the data is free-form (narrow the type with `.$type<T>()` and add runtime validation with `.zod()`). `f.from()` is the escape hatch for any Drizzle column the built-ins don't cover. Sources: `packages/questpie/src/server/modules/core/fields/{object,json,from}.ts`.
...
| Method | Effect |
|---|---|
| `.drizzle(fn)` | Mutate the underlying Drizzle column builder (``.default(sql`...`)``, `.$type<T>()`). Applied last when building the column. |
...
`.hooks()` runs per field. Hook values are typed to the field's data type and must return the same type (transform, not reshape). On write the order is `validate → beforeCreate`/`beforeUpdate` → `beforeChange`; on read, `afterRead`. The hook context gives you `{ field, collection, operation, req?, user?, doc, originalValue?, db, config }`. Source: `packages/questpie/src/server/fields/field-class.ts` + `types.ts`.

### Custom field types

A new field type is declared with `fieldType(name, { create, methods })` — the declarative way to add a type with its own chain methods. Every built-in exports a matching `<name>FieldType` (`textFieldType`, `numberFieldType`, `relationFieldType`, …) that codegen discovers from your `fields/` directories. Source: `packages/questpie/src/server/fields/field-type.ts`.
...
{/* TODO @user: add a screenshot of an admin form rendered from these fields (text input, relation picker, rich-text editor). */}

## Related

- [Collections](/docs/concepts/collections) — where fields are declared and how they project to tables and the admin.
- [Relations](/docs/concepts/relations) — belongsTo / hasMany / manyToMany / polymorphic in depth.
- [Validation](/docs/concepts/validation) — how the derived Zod schemas run on create and update.
- [Hooks](/docs/concepts/hooks) — collection- and field-level lifecycle hooks.
