...
<Callout type="warn" title="Purge is independently default-deny">
	...
	durable retention](/docs/concepts/soft-delete-retention).
</Callout>
...
You set it on the **context**, then pass that context to CRUD calls. Inside a seed, `createContext` is handed to you. See [Seeds](/docs/concepts/seeds) for the full seed lifecycle:
