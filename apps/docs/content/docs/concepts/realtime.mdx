...
Snapshot delivery remains the default. Its `maxFindLimit` is `100`; oversized
topics are rejected instead of silently clamped or split. The raw realtime API
can explicitly request native keyed deltas for an unwindowed collection `find`
with only own-column filters. Windowing, ordering, relations, counts, and globals
remain snapshots, so correctness never depends on a partial delta. Delta groups
periodically re-bootstrap and reset immediately when a configured watched access
resource changes.

See [Client realtime](/docs/client/realtime) for `live()`, `liveIter()`, lifecycle options, and TanStack Query integration.

For multi-tenant and high-fanout systems, read
[Scalable realtime modeling](/docs/concepts/scalable-realtime). It explains
scope identity, conservative routing, immutable buckets, server opt-out, and
when channels or materialized inboxes are a better model than generic row
realtime.
...
request-handling replica supports desired topology protocol v2, topology
