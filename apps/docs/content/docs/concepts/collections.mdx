...
- **create / update:** `beforeOperation` → `beforeValidate` → `beforeChange` → _[claim]_ → `beforeWrite` → _[DB write]_ → `afterChange` → `afterRead`
- **delete:** `beforeOperation` → `beforeDelete` → _[claim]_ → `beforeWrite` → _[DB write]_ → `afterDelete` → `afterRead`
...
Generated collection mutations can perform the same coordination without a
hand-written transaction by declaring the server-only
[`beforeWrite` fact guard](/docs/concepts/hooks#transactional-dependent-row-fact-guards).
Its bounded helper locks and returns current dependent facts in deterministic
physical-table/id order after the mutation's fresh primary claim and before any
generated DML or effects. The hook and active transaction are not serialized by
introspection and are not exposed to clients.
