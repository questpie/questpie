...
// The string you pass to collection("posts") is the key on app.collections —
// keep the file name matching it by convention.
...
const post = await app.collections.posts.create({
  data: { title: "Hello", slug: "hello", published: true },
});
...
Every selected row carries a computed **`_title: string`** — the value of the field you named in `.title()`, resolved through localization (i18n `COALESCE`) and virtuals. It's the row label in the admin and what `search` matches against. `$infer` is a type-only helper (its runtime value is an empty object); the select type also includes virtual outputs, localized field outputs, and — for [upload collections](#uploads) — the resolved `url`. Source: `collection-builder.ts:1075`.

<Callout type="info" title="`find()` returns a paginated envelope, not an array">
`find()` always resolves to `{ docs, totalDocs, totalPages, page, hasPrevPage, hasNextPage, prevPage, nextPage, ... }`. Read your rows off `result.docs`. Source: `packages/questpie/src/server/collection/crud/types.ts:1415`. Pass `groupBy` and you get a *grouped* envelope instead (`groups`, `totalGroups`), where `limit`/`offset` paginate the groups.
</Callout>
...
- **`.fields()` is cumulative.** Calling it again (or `.merge()` then `.fields()`) keeps prior fields and overrides by key. Re-declaring a key replaces that field cleanly (dropping its stale localized/relation state). This is what makes the module-extension recipe safe: `collection("user").merge(starter.collections.user).fields(...)` keeps the starter's fields and adds yours. Source: `collection-builder.ts:188`.
- **`.title(({ f }) => f.title)`** takes a field **name**, not an expression — `f` is a proxy that returns the accessed key as a string. It sets the source for the computed `_title` column. Source: `collection-builder.ts:534`.

<Callout type="warn" title="`f.upload()` (field) vs `.upload()` (builder method) are different things">
`f.upload({ to: "assets" })` is a **field** — a typed relation from this row to a *separate* upload collection (default target `"assets"`). The collection-level **`.upload()`** method (below) turns *this* collection itself into the byte store. Use the field when you want "this post has a cover image stored in my media collection"; use the method when you're building the media collection. Source: `packages/questpie/src/server/modules/core/fields/upload.ts:89` (field) vs `collection-builder.ts:799` (method).
</Callout>

<Callout type="warn" title="Use the field builder, not raw Drizzle columns">
`.fields()` has a deprecated raw-column overload (`{ col: pgColumn() }`) kept only for tests that need raw `.references()` FKs. Raw columns are never localized and carry no field definition, so they're invisible to the admin, validation, and the typed client. Always use the `({ f }) => …` form. Source: `collection-builder.ts:215`.
</Callout>
...
`CollectionOptions` is `{ schema?, timestamps?, softDelete?, versioning? }`. There is no `singleton` option — singletons are [Globals](/docs/concepts/globals). Source: `packages/questpie/src/server/collection/builder/types.ts:108`.

### `timestamps`

`boolean`, **default `true`**. Adds `createdAt` and `updatedAt` (`notNull`, `defaultNow`) to the main table. To drop them you must set `timestamps: false` explicitly. Source: `types.ts:131`.
...
For uniqueness that should free up again after a soft-delete (e.g. a unique email that another row may reclaim), use `softDeleteUniqueIndex()` inside `.indexes()` — it scopes the constraint to `WHERE deleted_at IS NULL`. Source: `types.ts:136`, `index-helpers.ts:20`.

### `versioning`

`boolean | CollectionVersioningOptions`, **default `false`**. `true` uses defaults; the object form is `{ enabled?, maxVersions?, workflow? }` (`maxVersions` defaults to **50**). Enabling it creates a `<name>_versions` table (and `<name>_i18n_versions` if the collection has localized fields) and unlocks `findVersions` / `revertToVersion` on the CRUD object. Source: `types.ts:166`, `types.ts:30`.
...
`WorkflowOptions` is `{ stages?, initialStage? }` and each `WorkflowStageOptions` is `{ label?, description?, transitions? }` (omit `transitions` for unrestricted moves; `initialStage` defaults to the first stage). Enabling workflow adds `versionStage` / `versionFromStage` columns, a `transitionStage()` CRUD method, `beforeTransition` / `afterTransition` hooks, and an `access.transition` rule. Source: `types.ts:63`, `types.ts:50`.

<Callout type="warn" title="Workflow requires versioning">
Workflow stages create version snapshots, so workflow lives **under** `versioning`. Setting `workflow` auto-enables versioning. Setting `versioning: { enabled: false }` together with a `workflow` **throws at construction** — the two are contradictory. Source: `packages/questpie/src/server/collection/builder/collection.ts:636` (constructor).
</Callout>

### `schema`

`string`. The Postgres schema all four tables (main, i18n, versions, i18n_versions) are created under; generated migrations emit `CREATE SCHEMA IF NOT EXISTS`. Defaults to `public`. Cross-schema relations render fully-qualified (`REFERENCES "other_schema"."table"("id")`). Source: `types.ts:126`.
...
On a single builder, each `.access()` call **replaces** the previous one (last call wins) — unlike `.hooks()`, which merges. When you `.merge()` two builders, access objects are shallow-merged with the *other* builder winning per key. Source: `collection-builder.ts:656`, `collection-builder.ts:1084`.
</Callout>
...
The `key`/`filename`/`mimeType`/`size` columns are nullable (a row can exist before its blob); `visibility` is `notNull` default `"public"`. Source: `collection-builder.ts:799`, `types.ts:81`.

<Callout type="info" title="`visibility` gates bytes, not rows">
`visibility` only controls who can fetch the **file bytes**. Reading and listing the upload **rows** still goes through the normal `.access()` chain. To override just the byte-serving rule, set `access.serve`. Source: `types.ts:81`.
</Callout>
...
`UploadFile` is `{ name, type, size, arrayBuffer?, stream? }` — `stream` is preferred over `arrayBuffer` for large files. Source: `crud/types.ts:45`.
...
A `FacetFieldConfig` is `true | { type: "array" } | { type: "range"; buckets } | { type: "hierarchy"; separator? }`. Indexing only happens if a search adapter is configured, and facet keys should match your `metadata` field names. Source: `collection-builder.ts:693`, `packages/questpie/src/server/modules/core/integrated/search/types.ts:34`.
...
| `.upload(opts)` | Make it a media collection. | — |
| `.set(key, value)` | Attach arbitrary state (extension seam). | — |
| `.merge(other)` | Combine with another builder of the same name. | see below |
| `.build()` | Resolve relations and materialize the `Collection`. | — |

The **admin** methods — `.admin()`, `.list()`, `.form()`, `.actions()`, `.preview()` — are contributed by the `@questpie/admin` module via codegen (they appear on the builder once admin is enabled, attached through `.set()`). They configure the panel UI and are documented in the admin docs. Source: `packages/admin/src/server/augmentation/index.ts:111`.
...
Source: `collection-builder.ts:507`.
...
`.merge()` spreads the full state first so the admin extension keys (`admin`, `adminList`, `adminForm`, `adminActions`, `adminPreview`) survive before explicit keys are overridden — this is a framework invariant. Pending (unresolved) relations are merged by field name; a field the other side redefines drops this side's pending entry. Source: `collection-builder.ts:1084`; framework invariant.
</Callout>

{/* TODO @ia: single-source — this page is currently the de-facto home for CRUD methods + the query language, and also re-teaches the per-operation access table, the secure-by-default callout, the hook fire-order, and the claim-checked-bulk callout (canonically owned by /docs/concepts/access-control and /docs/concepts/hooks). Consistent today. When concepts/crud + concepts/querying are authored (docs-playbook §5 / docs-map §1.5), shrink this "CRUD reference" section and the access/hooks re-explanations above to framing + see-links, and add inline see [CRUD](/docs/concepts/crud) / see [Querying](/docs/concepts/querying) at the points of contact (CRUD method table, find() options paragraph). Do NOT add those links until the pages exist. */}
...
Every method takes a `CRUDContext` as its **second** argument — pass `{ accessMode: "user", session }` to enforce access, `{ locale }` to read/write a localized variant, `{ stage }` to target a workflow stage. The context type is index-signature-free on purpose: a typo'd key is a compile error, not a silent fallback to `accessMode: "system"`. Source: `crud/types.ts:1612`, `crud/types.ts:72`.

<Callout type="warn" title="`updateMany` / `deleteMany` are claim-checked">
Bulk operations lock the matching rows and re-evaluate the `where` inside the transaction, returning only the rows that won the race. `updateMany` returns the written rows (an empty array means you lost a concurrent claim); `deleteMany` returns `{ success, count }`. The legacy `update` / `delete` bulk aliases are deprecated and removed in v4 — use `updateMany` / `deleteMany`. Source: `crud/types.ts:1612`.
</Callout>

`find()` options include `where`, `with` (relation hydration), `columns`, `orderBy`, `limit`, `offset`, `extras`, `search` (case-insensitive ILIKE against `_title`), `locale`, `localeFallback`, `includeDeleted` (soft-delete only), `stage`, and `groupBy`. The full query language — `where` operators, relation filters (`some`/`none`/`every`, `is`/`isNot`), `with` aggregations — is covered under [Relations](/docs/concepts/relations) and the query reference. Source: `crud/types.ts:911`.
...
After adding or renaming a file, run `questpie generate` (or `questpie dev`, which watches) to regenerate `.generated/` — that's what wires the new collection into `app.collections.*`, the admin, REST, OpenAPI, and the client. Then `questpie push` (dev) or `questpie migrate:generate` (production) to materialize the table. Source: `packages/questpie/src/cli/index.ts:50`.

<Callout type="idea" title="Scaffold it">
`questpie add collection my-thing` creates the file with the right boilerplate and runs codegen for you. Run `questpie add --list` to see every scaffold type. Source: `packages/questpie/src/cli/index.ts:375`.
</Callout>
...
The collection-builder types — `CollectionBuilder`, `CollectionBuilderState`, `CollectionSelect` / `CollectionInsert` / `CollectionUpdate`, `CollectionOptions`, `CollectionAccess`, `CollectionHooks`, `RelationConfig`, and the CRUD/query types (`CRUD`, `FindManyOptions`, `Where`, `With`, …) — are all exported publicly from `questpie/builders` if you need to reference them directly in shared helpers. For app-aware aliases that resolve relations through your generated `AppContext`, prefer the generated types from `#questpie`. Source: `collection-builder.ts:1075`, `builder/index.js`.
...
{/* TODO @user: add an admin screenshot of the posts list + edit form once a dev server is running (preview tools or manual capture). */}
