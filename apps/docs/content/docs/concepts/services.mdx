...
`ctx` is typed as `ServiceCreateContext`. Before codegen it falls back to the base app context plus `{ app }`; after `questpie generate` it resolves to the full typed surface — `db`, `session`, `collections`, `globals`, `queue`, `email`, `storage`, `kv`, `logger`, `search`, `realtime`, `t`, `services`, plus any namespaced service buckets. Source: `packages/questpie/src/server/services/define-service.ts:40`.

<Callout type="info" title="Services can depend on services">
Read another service off `ctx.services` inside your factory — `create: (ctx) => ({ … ctx.services.billing … })`. The container resolves the dependency before it constructs yours. The factory may be `async`, so awaiting setup (open a connection, fetch a token) is fine — `create` returns `TInstance | Promise<TInstance>`. Source: `define-service.ts:52`.
</Callout>

## Two ways to write it

The object form and the chained form are **equivalent** — the chained methods just build the same state object that the object form passes directly. Use whichever reads better. Both are accepted by the `service()` factory's overloads. Source: `packages/questpie/src/server/services/define-service.ts:113`.
...
Each chained method (`create`, `lifecycle`, `namespace`, `dispose`) spreads `this.state` into a **brand-new** `ServiceBuilder` and never mutates in place — the builder is immutable. The builder itself resolves nothing; the app's service container interprets the resulting `state` at registration time. Source: `define-service.ts:59`.

<Callout type="warn" title="`namespace()` takes `string | null`, not `undefined`">
The chained `.namespace()` method only accepts `string | null`. To put a service in the default `ctx.services` bucket, **omit** `namespace` entirely — don't pass `undefined`. (The object form's type allows `undefined`, but omitting is the idiom.) Source: `define-service.ts:103`.
</Callout>

## Lifecycle: singleton vs request

`lifecycle` decides how often `create` runs. The public type is `ServiceLifecycle = "singleton" | "request"`. Source: `packages/questpie/src/server/services/define-service.ts:8`.
...
The **public** enum is `"singleton" | "request"`. There is a separate, **non-exported** internal proof-of-concept (`ScopedContainer`) whose `Lifecycle` is `"singleton" | "scoped"` — different values, different type, and not part of the public API. Build your services against `"singleton" | "request"` only. Source: `define-service.ts:8`.
</Callout>
...
`service(state?)` returns a `ServiceBuilder`. All four fields are optional on the type, but a usable service must supply `create`. The factory has three overloads (no-arg, full-state, and partial-state) — they all funnel into `new ServiceBuilder(state ?? {})`. Source: `packages/questpie/src/server/services/define-service.ts:113`.
...
Because the context builder pre-populates `ctx.email` directly, this `namespace: null` registration does **not** overwrite it — the same `if (!(name in result))` guard above applies. You consume it as `ctx.email` (it is **not** `ctx.mailer`). Its full surface — `send`, `sendTemplate`, `renderTemplate`, the `email()` template factory, and the SMTP / Resend / Plunk / Console adapters — lives on the [Emails](/docs/concepts/emails) page. Source: `packages/questpie/src/server/modules/core/services/email.ts:9`.
...
- [Collections](/docs/concepts/collections) — the typed `ctx.collections` API your services read and write through.
