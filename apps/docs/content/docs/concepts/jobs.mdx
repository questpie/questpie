...
The optional second argument is [`PublishOptions`](#publishoptions), spread-merged **over** the job's own `options` (`{ ...jobDef.options, ...publishOptions }`, so call-time wins). Throws if the payload fails the schema.
...
`options` here is `Omit<PublishOptions, "startAfter">` — `startAfter` is meaningless for a recurring schedule. This is the **imperative** scheduling path; the declarative one is [`options.cron`](#recurring-jobs) on the job itself. Throws on adapters without scheduling support (Cloudflare Queues).
