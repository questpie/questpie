...
Most app projects need auth, file storage, email, background jobs, and search — but wiring them together is tedious. QUESTPIE integrates these services directly into the app builder, so they're available through `app.auth`, `app.storage`, `app.email`, `app.queue`, `app.search`, `app.kv`, and `app.logger` with zero extra configuration.

## Mental Model

Infrastructure is configured in `runtimeConfig()`:
...
- **Type-safe access** in all handler contexts (`{ email, queue, kv, logger, storage }`)
...
import { runtimeConfig, pgBossAdapter, SmtpAdapter, ConsoleAdapter } from "questpie";
