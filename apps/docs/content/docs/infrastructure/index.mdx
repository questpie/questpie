...
throws. With no `queue` block the queue service hands back an empty client, so
codegen still types `app.queue.<job>` while the property is missing and
dispatching throws on it. `ctx.executor.run()` throws while `executor` is unset,
...
else. The route that reads `kv`, the collection that indexes into `search` and
the hook that publishes a job are all written against the service, never against
the adapter, so this file is the only thing that changes.
...
use. Every provider SDK QUESTPIE imports itself is an optional peer dependency,
and the Redis-backed adapters import no SDK at all, taking a connected client or
a function returning one. `search` is the odd slot, taking the adapter instance
...
Storage is the one slot that can configure itself. Set all four of
`QUESTPIE_STORAGE_ENDPOINT`, `QUESTPIE_STORAGE_BUCKET`,
`QUESTPIE_STORAGE_ACCESS_KEY` and `QUESTPIE_STORAGE_SECRET_KEY` with no
`storage` key in your config, and QUESTPIE builds an S3-compatible Files SDK
adapter for you, defaulting `QUESTPIE_STORAGE_REGION` to `auto`. Set the
...
`logger.adapter` is the slot the table cannot list. It takes a `LoggerAdapter`
and the built-in Pino one satisfies it, but that type is not re-exported from
any entry point today, so there is no contract for you to import.
...
`singleton` and `executionTerminalState`. Omit one of the first four and it is
inferred from whether the matching method is present, `singleton` falls back to
false and `executionTerminalState` needs both an explicit `true` and the method.
The queue service resolves that set once and reads it before dispatching, so
`queue.<job>.schedule()` throws on an adapter reporting `scheduling: false`,
which is how Cloudflare Queues says it has no cron.
...
enforcement. `PostgresSearchAdapter` reports `semantic: false` and also throws
on a `mode` it does not implement, so an unsupported query fails loudly instead
of quietly turning into keyword matching.

Storage inherits the Files SDK's flags rather than declaring its own. An adapter
sets `supportsRange`, `supportsMetadata` and their siblings, `app.storage.capabilities`
derives a resolved object from them on every read, and a `download({ range })`
against an adapter without `supportsRange` throws before any provider call. KV
