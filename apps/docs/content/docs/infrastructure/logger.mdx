...
Structured logging is essential for debugging and monitoring. The logger service gives you consistent, leveled output across all API operations — CRUD, hooks, jobs, search indexing — with a pluggable adapter interface for custom transports.
...
```ts
// No logger config needed — uses Pino with level "info" by default
...
The logger is available on the app instance as `app.logger`:

```ts
app.logger.debug("Processing batch", { batchSize: 100 });
app.logger.info("Record created", { collection: "posts", id: record.id });
app.logger.warn("Slow query detected", { duration: 2500 });
app.logger.error("Failed to send email", error);
```
...
```ts
const jobLogger = app.logger.child({ module: "queue", jobName: "sendEmail" });
...
the app uses the logger internally for:
...
this.search.initialize().catch((err) => {
  this.logger.error("[app] Failed to initialize search adapter:", err);
});
```
...
// Configure mock logger in runtimeConfig for tests
