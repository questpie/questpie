...
import { runtimeConfig, pgNotifyAdapter } from "questpie";
...
```ts
import { runtimeConfig, redisStreamsAdapter } from "questpie";
...
the app exposes SSE endpoints for each collection and global:

```
GET /api/realtime/collections/{name}?seq={lastSeq}
GET /api/realtime/globals/{name}?seq={lastSeq}
```
