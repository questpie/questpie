...
.build({
  ...
  storage: {
    basePath: "/api",  // URL prefix for file access
  },
...
|---|---|---|---|
| `storage.basePath` | `string` | `"/"` | URL path prefix for file endpoints |
...
- **Wrong `basePath`** — The storage `basePath` must match your app mount path so file URLs resolve correctly.
