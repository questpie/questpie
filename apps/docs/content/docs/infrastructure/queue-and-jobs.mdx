...
export const sendAppointmentConfirmation = job<App>()({
...
const bookingWorkflow = workflow<App>()<{
  barberId: string;
  serviceId: string;
  date: string;
}>("processBooking")
...
- `job<App>()({...})` gives typed `app` in job handlers.
- `workflow<App>()(...)` gives typed `app` in each workflow step.
- In untyped contexts, use `getApp<App>(app)` or `getContext<App>(ctx)`.
