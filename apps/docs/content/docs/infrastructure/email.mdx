...
There is not one. `email.adapter` is required, and the core email service throws
while the app boots when it is missing.

```
QUESTPIE: 'email.adapter' is required. Provide adapter in .build({ email: { adapter: ... } })
```

Email is an infrastructure service resolved eagerly inside `createApp()`, so a
missing adapter fails at startup rather than on your first send. The closest
thing to a default is what the starters write for you:
...
<Callout type="info" title="`MailerService` has a fallback you never reach">
	Built directly with no adapter it makes a `ConsoleAdapter` outside production
	and throws inside it. The core service asserts your adapter before it ever
	constructs the mailer, so that branch only surfaces in tests. Set `adapter`.
</Callout>
