...
Access the KV service through the app instance:
...
.hooks({
  afterChange: async ({ app, data }) => {
    ...
    await app.kv.delete(`settings-cache`);
  },
})
...
handler: async ({ app }) => {
  ...
  // Check cache first
  const cached = await app.kv.get<number>("stats:total-posts");
  if (cached !== null) return { count: cached };

  // Compute and cache
  const result = await app.api.collections.posts.count({});
  await app.kv.set("stats:total-posts", result, 60);
  return { count: result };
}
```
