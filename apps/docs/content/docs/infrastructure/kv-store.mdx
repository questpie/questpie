...
Many API operations benefit from short-lived caching — expensive query results, computed aggregates, rate limit counters, or temporary tokens. The KV service provides a simple `get`/`set` interface with optional TTL and tag-based invalidation, available everywhere as `app.kv`.
...
```ts
// No kv config needed — uses MemoryKVAdapter by default
...
```ts
import { runtimeConfig, IORedisKVAdapter } from "questpie";
...
```ts
// Set a value
await app.kv.set("user:123:profile", { name: "Jane", role: "admin" });

// Set with TTL (seconds)
await app.kv.set("rate:login:192.168.1.1", 1, 60); // expires in 60s

// Get a typed value
const profile = await app.kv.get<{ name: string; role: string }>("user:123:profile");

// Check existence
const exists = await app.kv.has("user:123:profile");

// Delete
await app.kv.delete("user:123:profile");
...
## Using KV in Hooks and Functions

Access the KV service directly from handler context:
...
.hooks({
  afterChange: async ({ data, kv }) => {
    ...
    await kv.delete(`settings-cache`);
  },
})
...
handler: async ({ kv, collections }) => {
  // Check cache first
  const cached = await kv.get<number>("stats:total-posts");
  if (cached !== null) return { count: cached };

  // Compute and cache
  const result = await collections.posts.count({});
  await kv.set("stats:total-posts", result, 60);
  return { count: result };
}
```
