...
Auth is configured via the `auth.ts` convention file:
...
auth({
  ...
  basePath: "/api/auth",                    ← auth API path
})
```

Auth runs as part of the app fetch handler. The admin panel uses it for login/signup, and your frontend can use it for user authentication.
...
The `baseURL` and `secret` are configured in `questpie.config.ts`:
...
Add social login by configuring providers in `auth.ts`:
...
Auth sessions are available in hooks, access rules, and function handlers:
...
You can pass additional Better Auth options in the `auth()` definition:
...
- **Forgetting `basePath`** — The auth endpoints need to match your app mount path. If your app is at `/api`, use `/api/auth`.
...
- [Functions](/docs/server/functions) — Using session in function handlers
