...
Admin RPC routes that expose admin config, dashboard widget loaders, action handlers, and reactive field handlers use the same `session.user.role === "admin"` contract. A logged-in non-admin user is not enough for admin access.
