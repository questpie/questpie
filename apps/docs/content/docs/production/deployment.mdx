...
Declare and validate these with the [env primitive](/docs/production/environment) (`env.ts` + `env()`) so a misconfigured deployment fails at boot with every offending variable named — instead of crashing mid-request.
