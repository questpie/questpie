...
<Callout type="info" title="The name is the key, not the filename">
	Collections and globals are keyed by the string you pass to the factory.
	`collection("blog_posts")` inside `collections/posts.ts` is
	`app.collections.blog_posts`. Jobs, services and emails take no such argument,
	so their key is the filename in camelCase, and `jobs/send-invoice.ts` becomes
	`sendInvoice`. A channel keys on its export name, or on the filename when it
	is the default export, because the string it takes is the wire pattern. Routes
	key on the path below `routes/` or `functions/`, so
	`routes/webhooks/stripe.ts` is `webhooks/stripe`.
</Callout>
