...
<Callout type="warn" title="Headless runtimes (Hono, Elysia) do not get the OAuth provider today">
The OAuth provider is declared **only** in the starter module's auth config (`modules/starter/config/auth.ts`), and the starter is pulled in by the **admin** module. Headless runtimes ship neither. On Hono/Elysia you can still add `mcpModule` and the `/api/mcp` route mounts — but with no OAuth provider behind it, an unauthenticated request just gets a `401` with nothing to discover, and there is no consent screen or token endpoint. **OAuth-MCP requires the admin/starter module today.** Local [stdio](/docs/integrations/mcp#stdio-transport) still works everywhere (it needs no OAuth). Wiring the provider into a headless app manually is a documented follow-up.
</Callout>
