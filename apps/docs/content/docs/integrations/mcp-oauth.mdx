...
<Callout type="info" title="Prerequisites">
	Read the [MCP integration](/docs/integrations/mcp) page first - it covers
	...
	top. Familiarity with [Access control](/docs/schema/access-control) helps
...
>
	...
	**headless / custom-auth** app that ships neither can add `oauthModule` on top
	...
	[stdio](/docs/integrations/mcp#stdio-transport) needs no OAuth and works
	everywhere.
</Callout>
...
<Callout type="info" title="`user` and `system` principals are unchanged">
	...
	trusted local [stdio](/docs/integrations/mcp#stdio-transport) worker is a
	`system` principal - it bypasses both the RBAC and scope gates entirely (full
	access). Only the network-facing OAuth path is scope-gated.
</Callout>
