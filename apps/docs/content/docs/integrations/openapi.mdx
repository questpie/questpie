...
<Callout type="warn" title="Generator quality is in active rework">
	...
	**path-parameter** typing, and wiring **route
	[`.meta()`](/docs/concepts/routes#metadata-openapi-and-mcp)**
...
The module is a thin wrapper over two exported [route](/docs/concepts/routes) factories. Drop either into your own `routes/` directory if you want a different path or want to apply your own [access](/docs/schema/access-control) rule:
...
>
	...
	rules](/docs/schema/access-control) at request time. The two declared
	security schemes (`bearerAuth`, `cookieAuth`) advertise *how* to authenticate,
	not *what* is allowed.
</Callout>
