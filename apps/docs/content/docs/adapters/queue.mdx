...
This is the QUESTPIE principle in action: **core, modules, and your own code reach the queue through one contract**. The full "build your own adapter" loop, across every adapter kind, lives in [Building a plugin](/docs/guides/build-a-plugin).
...
- [Building a plugin](/docs/guides/build-a-plugin), implement `QueueAdapter` (and the other adapter contracts) for any backend.
