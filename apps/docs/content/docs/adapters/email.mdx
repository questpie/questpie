...
That `email` block is a `MailerConfig`. `adapter` is the only field you must set; `defaults.from` fills in the `From` address when a send call omits one. (Templates are populated by codegen from `emails/*.ts` — see [Emails](/docs/concepts/emails); you don't list them here.)

<Callout type="warn" title="You must set an adapter — even in dev">
The core email service throws at app build if `email.adapter` is missing: `"QUESTPIE: 'email.adapter' is required. Provide adapter in .build({ email: { adapter: ... } })"`. The mailer *does* have a `ConsoleAdapter` fallback for the no-adapter case, but the core service never reaches it — it constructs `MailerService` only after asserting your adapter exists. So always set `adapter`; use `new ConsoleAdapter()` explicitly for local development rather than relying on a fallback. Source: `core/services/email.ts:15-19`, `mailer/service.ts:49-66`.
</Callout>
...
It logs `From / To / CC / BCC / Reply-To / Subject`, the plain-text body, attachment filenames, and (only when `logHtml: true`) the HTML. It never sends. Source: `adapters/console.adapter.ts:7-86`.
...
`SmtpAdapter` also exposes `verify(): Promise<boolean>`, which proxies nodemailer's SMTP connection check. Source: `adapters/smtp.adapter.ts:10-54`.
...
It returns a `Promise<SmtpAdapter>`, which `MailerConfig.adapter` accepts directly (the mailer awaits the promise lazily on first send). Dev/testing only — it makes a network call to create the account. Source: `adapters/smtp.adapter.ts:60-81`.
...
| Option | Type | Default | Notes |
|---|---|---|---|
| `apiKey` | `string` | — *(required)* | Your Resend API key (or a key for a Resend-compatible provider). |
...
| `idempotencyKey` | `string \| ((options) => string \| undefined)` | — | Static or per-email. When present, sets the `Idempotency-Key` header. |
| `fetch` | `typeof fetch` | global `fetch` | Override for tests or non-standard runtimes. |
| `afterSendCallback` | `(response: ResendSendResponse, options) => void \| Promise<void>` | — | Runs with the parsed JSON body (e.g. the message `id`) after Resend accepts the email. |

Buffer attachments are base64-encoded automatically. On a non-2xx response it throws `Resend API error (<status> <statusText>): <body>`. Source: `adapters/resend.adapter.ts:11-108`.
...
| `fromName` | `string` | — | Display name, applied only when `from` is a bare email (no `"Name <email>"` form). |
| `subscribed` | `boolean` | — | Subscribe recipients to marketing in Plunk. Leave unset for transactional mail. |
| `fetch` | `typeof fetch` | global `fetch` | Override for tests or non-standard runtimes. |
| `afterSendCallback` | `(response: PlunkSendResponse, options) => void \| Promise<void>` | — | Runs with the parsed body after Plunk accepts the email. |

<Callout type="warn" title="Plunk rejects CC and BCC">
`PlunkAdapter` throws if a message sets `cc` or `bcc` — Plunk's transactional API doesn't document those fields. It also treats a `success: false` body as an error even on an HTTP 2xx response. If you need CC/BCC, use SMTP or Resend. Source: `adapters/plunk.adapter.ts:70-96, 125-135`.
</Callout>
...
- **`defaults.from` is the only fallback sender.** If a send call omits `from` and you set no `defaults.from`, mail goes out as `noreply@example.com`. Set `defaults.from` in config. Source: `mailer/service.ts:97`.
- **`adapter` may be a `Promise`.** `MailerConfig.adapter` accepts `MailAdapter | Promise<MailAdapter>`; the promise is awaited lazily on the first send (how `createEtherealSmtpAdapter()` plugs in). Source: `mailer/service.ts:49-51`.
...
That's the whole contract. Source: `mailer/adapter.ts:6-8`, `mailer/types.ts:24-28` (`SerializableMailOptions` types `from`/`text`/`html` as present `string`s), `mailer/service.ts:91-100` (serialize: `from` defaulted, at least one of `text`/`html` non-empty).
...
- [Emails](/docs/concepts/emails) — templates, `EmailResult`, and the `ctx.email.send` / `sendTemplate` service this adapter delivers for.
- [Configuration](/docs/concepts/configuration) — where `runtimeConfig({ email })` lives and how adapters are wired.
- [Environment](/docs/concepts/environment) — typed `env()` for API keys and SMTP credentials.
- [Jobs](/docs/concepts/jobs) — dispatch sends from a job so provider failures retry.
