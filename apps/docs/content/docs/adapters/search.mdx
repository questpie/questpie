...
The **search adapter** is the backend behind the [Search](/docs/concepts/search) lifecycle. It powers `app.search`, the client `search` API, and [`.searchable()`](/docs/schema/collections#searchable) indexing. Swap the adapter and the application contract stays unchanged.
...
>
	...
	[access](/docs/schema/access-control) is governed by the `reindexAccess`
...
- [Collections → Searchable](/docs/schema/collections#searchable), the `.searchable()` config that controls what each collection indexes.
- [Configuration](/docs/concepts/configuration), the `runtimeConfig` / `questpie.config.ts` pattern that wires the `search` adapter.
- [Access control](/docs/schema/access-control), how search results respect row-level access rules, and how `reindexAccess` gates the reindex route.
