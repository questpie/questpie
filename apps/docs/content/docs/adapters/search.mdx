...
- **Indexing handled for you.** Searchable collections are re-indexed automatically after writes (debounced via the `index-records` [job](/docs/code/jobs)); you rarely call `index()` by hand.
...
You almost never index by hand. Searchable collections are re-indexed automatically after a write: the search service batches changed records over a 100 ms debounce window and dispatches the core `index-records` [job](/docs/code/jobs) (when a [queue adapter](/docs/ship/configuration) is configured), falling back to synchronous indexing otherwise.
...
- [Configuration](/docs/ship/configuration), the `runtimeConfig` / `questpie.config.ts` pattern that wires the `search` adapter.
...
- [Jobs](/docs/code/jobs), the `index-records` job behind automatic re-indexing.
