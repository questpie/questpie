...
Both shapes also accept the shared base options below. Source: `packages/questpie/src/server/config/types.ts:357` (`StorageConfig`), `:289` (`StorageBaseConfig`).
...
Source: `packages/questpie/src/server/config/types.ts:289`.
</Callout>

The removed legacy `driver` / `files` keys are typed as `never` — using them is a compile error. The boot-time validator (`assertValidStorageConfig`) also rejects them, rejects unknown keys, and rejects `adapter` + `location` together. Source: `packages/questpie/src/server/config/cloud-env.ts:55`.
...
With `adapter`, QUESTPIE constructs `new Files({ adapter })` directly — it does **not** serve the bytes; the provider (or its CDN) does, and `app.storage.url(key)` returns a provider URL (presigned, or your `publicBaseUrl`). Source: `packages/questpie/src/server/modules/core/integrated/storage/create-driver.ts:41`.
...
`SignUploadOptions`: `expiresIn` (required, seconds), `contentType?`, `maxSize?`, `minSize?` (default `1`). The result is a `SignedUpload`: either `{ method: "PUT", url, headers? }` or `{ method: "POST", url, fields }` (S3/R2 use the POST form when `maxSize` is set, which enforces the cap via a `content-length-range` policy). Source: files-sdk `SignUploadOptions` / `SignedUpload`.
...
A user-uploaded `.html` or SVG served inline executes in your domain's origin — stored XSS. When generating a download URL for untrusted content, pass `responseContentDisposition: "attachment"` to `app.storage.url(key, { … })` to force a download instead of inline rendering. Source: files-sdk `UrlOptions`.
</Callout>
...
The adapter is created with `forcePathStyle: true` and is **lazy** — the `files-sdk/s3` import (and its AWS SDK peers) loads on the first storage operation, not at boot. If the endpoint is set but bucket/keys are missing, QUESTPIE warns and falls back to local storage. An explicit `storage` block in your config always wins over the env vars. Source: `packages/questpie/src/server/config/cloud-env.ts:174`.

<Callout type="warn" title="The env-driven S3 path needs the AWS SDK installed">
The lazy adapter requires `@aws-sdk/client-s3`, `@aws-sdk/lib-storage`, `@aws-sdk/s3-presigned-post`, and `@aws-sdk/s3-request-presigner` as dependencies. If they're missing, the first storage operation throws a clear install message. Source: `packages/questpie/src/server/config/cloud-env.ts:268`.
</Callout>
...
`app.storage` is typed as `StorageFromQuestpieConfig<YourConfig>` — a `Files<>` instance specialized to your configured adapter. Source: `packages/questpie/src/exports/storage.ts:1`, `packages/questpie/src/server/config/types.ts:366`.
...
TODO @user: add a screenshot of an upload collection's admin media grid (e.g. `/admin/collections/media`) showing thumbnails served via the configured storage adapter.
</Callout>
...
- [Client SDK — uploads](/docs/concepts/client-sdk#uploads) — `client.collections.media.upload(file, { onProgress })` with XHR progress.
