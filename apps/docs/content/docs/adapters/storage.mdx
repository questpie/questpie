...
Storage is the `storage` key of your [runtime config](/docs/ship/configuration). The default, **omit it entirely**, stores files on the local disk under `./uploads`, which is all you need in development:
...
For large files, route the **bytes** straight from the browser to S3/R2 instead of through your server. Mint a presigned upload from a [route](/docs/code/routes), hand it to the client, and have the client PUT/POST the file to the provider directly:
...
- [Configuration](/docs/ship/configuration), where the `storage` block lives in `runtimeConfig()`.
- [Environment](/docs/ship/environment), typed `env()` for bucket names, endpoints, and credentials.
