...
- **Compiles file convention into a typed app.** Scans your conventional dirs, derives a key per file, and emits `.generated/index.ts` (the production singleton), `.generated/app-factory.ts` (fresh app creation), `.generated/factories.ts` (the `collection()` / `global()` builders that know your modules), and the `#questpie` type barrel, all in one shot.
...
- **Supports flat or by-feature source layout.** The same category can live at `collections/posts.ts` or `features/blog/collections/articles.ts`; both feed the same generated maps.
...
src/questpie/server/.generated/
  index.ts         # production singleton + App / AppConfig / CollectionDoc / createContext
  app-factory.ts   # typed fresh app factory without singleton acquisition
...
1. **Resolve the target graph.** The built-in `coreCodegenPlugin()` is _always_ prepended, then your config plugins, then module-extracted plugins. All contributions to a given target (e.g. `"server"`) merge into one `ResolvedTarget`.
...
QUESTPIE supports two equivalent source layouts for category files. Use the flat
layout when the app is small; use `features/<feature>/...` when a domain slice
...
- A category declared with `dirs` is scanned in both places: `dir/` and `features/<feature>/dir/`.
- The feature name is organizational only. `features/blog/collections/articles.ts` contributes `app.collections.articles`, not `app.collections.blog.articles`.
- Keys are unique per category across both layouts. `collections/posts.ts` and `features/blog/collections/posts.ts` conflict instead of overriding each other.
- Recursive categories keep only the path below the category directory. `features/blog/routes/webhooks/stripe.post.ts` keys as `webhooks/stripe:POST`.
- Private feature directories (`features/_internal/...`), private files (`_helpers.ts`), `index.ts`, tests, and declaration files are skipped.
- `questpie dev` watches `features/` on the server target, so adding or removing a feature file regenerates the typed app.

The core server categories that support this are `collections`, `globals`,
`jobs`, `routes` / `functions`, `messages`, `services`, `emails`, `migrations`,
`seeds`, and `fieldTypes`. Plugin categories use the same rule when they declare
`dirs` or a directory/map pattern such as `blocks/*.ts`.

Single-file discover patterns are different: `modules.ts`, `env.ts`,
`fields.ts`, `config/app.ts`, `config/auth.ts`, and `config/admin.ts` are
root-level files. A plugin can opt into collecting root plus feature single files
with `mergeStrategy: "spread"` for array-shaped contributions such as
`sidebar.ts` or dashboard widgets; spread order is root first, then features in
alphabetical order.

## Export & discovery rules

Codegen has to turn a file into a `key` and an import. The rules are deterministic, identical across every category, and worth knowing because they decide what your entity is _named_ on `app.*`.
...
- **Non-factory categories** (jobs, services, emails, …) always key off the **file name**, never the export name. A single-entity file resolves its key as `kebabToCamelCase(basename(path))` unconditionally, so `jobs/send-newsletter.ts` is `sendNewsletter` whether you `export default` or `export const sendNewsletter`. The export name is used only as the _import binding_ (`import { sendNewsletter as _job_sendNewsletter }`), not as the entity key.
...
Codegen detects the export style from the file content (no AST, a fast content scan), in priority order: `export default` → default; `export const X = { … }` (an object **literal**, not a factory call) → a named _bundle_ (spread into the parent); `export const/function/class X` → a single named export; `export { X }` → named.
...
| ----------------- | ----------------------------------------------------------------------------------------------------------------------- | ----------------------------- |
| `index.ts`        | The production singleton + public type barrel (`App`, `AppConfig`, `CollectionDoc`, `CollectionWhere`, `createContext`) | `#questpie` (the type barrel) |
| `app-factory.ts`  | `createAppForRuntime(runtime)`, a typed fresh-app factory that does not acquire the generated singleton                 | `#questpie/app-factory`       |
...
Two more, `AccessRuleContext<K>` and `HookRuleContext<K>`, narrow `ctx.data` to a collection's row inside shared access/hook helpers. They carry a **cycle rule**: import them only from files _not_ imported by a collection (routes, services, jobs, scripts); helpers a collection imports take the package-level `AccessContext` / `HookContext` from `questpie` instead.
...
## Fresh app instances for tests

Import `createAppForRuntime` from the separate generated entrypoint when a test harness needs an isolated database or other runtime adapters:
...
const app = await createAppForRuntime({
	...
	db: { pglite },
	email: { adapter: testMailAdapter },
	queue: { adapter: testQueueAdapter },
});

try {
	// Each call returns a fresh, fully typed app.
} finally {
	await app.destroy();
}
```

The factory reuses the same generated app definition as production but never calls `acquireGeneratedApp()` and never imports `questpie.config.ts`. Import it directly from `#questpie/app-factory`; importing `#questpie` evaluates the normal process-wide singleton by design. This entrypoint is intended for test infrastructure and other explicitly managed lifecycles, not request handlers.

## CLI reference

Codegen is driven by three commands. The broader migrate / seed / push surface is covered by [Configuration](/docs/concepts/configuration) and [Seeds](/docs/concepts/seeds).
...
Plugin-driven scaffolding from each target's `scaffolds`. Built-in types (from `coreCodegenPlugin()`): `collection`, `global`, `job`, `service`, `email` (`.tsx`), `route`, `seed`, `migration`. Modules add more (the admin module contributes `block`). `--list` prints every scaffold type with its description and the targets that declare it. When a scaffold type is declared by multiple targets, `questpie add` writes a file in **all** of them (e.g. a server block definition _and_ its admin-client renderer), pass `--target` to scope to one. Existing files are skipped with a warning, and codegen re-runs afterward. The name is expanded to kebab / camel / pascal / title casings and passed to each template. See [Seeds](/docs/concepts/seeds) for the generated seed file contract.
...
`modules.ts` default-exports the pre-built modules your app depends on. Codegen reads it in a dedicated pre-pass _before_ the rest of discovery, so a module's collections, globals, jobs, and its **own codegen plugin**, are folded in first.
...
Declaring the **same** scaffold name on two targets is the point: `questpie add block hero` then writes a server block definition _and_ an admin-client renderer in one command.
