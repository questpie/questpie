...
flowchart TD
	Repo["questpie/"]
	Packages["packages/"]
	Core["questpie/<br>Core framework"]
	Admin["admin/<br>Admin UI"]
	OpenAPI["openapi/<br>OpenAPI integration"]
	Elysia["elysia/<br>Elysia adapter"]
	Hono["hono/<br>Hono adapter"]
	Next["next/<br>Next.js integration"]
	TanStack["tanstack-query/<br>TanStack Query client"]
	Create["create-questpie/<br>Project scaffolder"]
	Apps["apps/docs/<br>Documentation site"]
	Examples["examples/"]
	CityPortal["city-portal/"]
	Barbershop["tanstack-barbershop/"]
	Turbo["turbo.json<br>Turborepo task config"]
	Oxlint[".oxlintrc.json<br>Linting configuration"]
	Oxfmt[".oxfmtrc.json<br>Formatting configuration"]
	PackageJson["package.json<br>Workspace root"]
...
flowchart TD
	...
	Src["src/"]
	Exports["exports/<br>Public API entry points"]
	Index["index.ts<br>Main entry: ."]
	Client["client.ts<br>Client entry: ./client"]
	Server["server.ts<br>Server entry: ./server"]
	Dist["dist/<br>Build output, gitignored"]
	DistIndex["index.mjs + index.d.mts"]
	DistClient["client.mjs + client.d.mts"]
	DistOther["Other compiled entries"]
...
<Callout type="note">
`include` explicitly adds `.generated/` directories so TypeScript processes them. Without this, codegen output will not be type-checked.
</Callout>

## Related Pages

<Cards>
  <Card title="Building a Module" href="/docs/extend/building-a-module" description="Create and publish module packages" />
  <Card title="Building a Plugin" href="/docs/extend/building-a-plugin" description="Create codegen plugins" />
  <Card title="Codegen" href="/docs/concepts/codegen" description="Generated output details" />
</Cards>
