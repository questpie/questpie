...
2. **An access `where` filter**, `read`/`update`/`delete` rules that return `{ tenant: tenantId }`. A rule that returns a `where` object grants _filtered_ access: rows are narrowed, not denied. (See [Access control](/docs/schema/access-control) for the full `where`-returning model.)
...
- [Access control](/docs/schema/access-control), the `where`-returning rule model that does collection-level scoping, plus field-level rules and the `serve`/`introspect` chains.
- [Configuration](/docs/concepts/configuration), `appConfig()` in full (`locale`, `access`, `hooks`, `context`) and the `config/*.ts` convention.
- [Globals](/docs/schema/globals), the singleton model that `.options({ scoped })` extends.
- [Collections](/docs/schema/collections), fields, access, hooks, and the `schema` option used for hard isolation.
- [Hooks](/docs/schema/hooks), where `beforeChange` stamps the scope FK, and the transaction lifecycle.
- [Relations](/docs/schema/relations), `with:` hydration, whose access behavior drives the cross-scope leakage gotcha above.
