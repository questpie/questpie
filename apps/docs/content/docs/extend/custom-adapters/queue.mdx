...
<Callout type="warn">
If you need strict per-group FIFO behavior, implement it as provider-native behavior in a dedicated adapter. Do not emulate grouping by creating one physical queue per group or by overloading `singletonKey`; those patterns are operational hacks and cannot provide predictable cross-worker semantics.
</Callout>
...
The `queue` config requires both an `adapter` and a `jobs` map. Jobs are defined separately -- see the [Queue guide](/docs/infrastructure/queue) for details.
...
- [PgBossAdapter](https://github.com/questpie/questpie/blob/main/packages/questpie/src/server/modules/core/integrated/queue/adapters/pg-boss.ts) -- PostgreSQL-based, supports long-running consumers, runOnce, scheduling, and singleton jobs
- [CloudflareQueuesAdapter](https://github.com/questpie/questpie/blob/main/packages/questpie/src/server/modules/core/integrated/queue/adapters/cloudflare-queues.ts) -- push-based queue for Cloudflare Workers
- [QueueAdapter interface source](https://github.com/questpie/questpie/blob/main/packages/questpie/src/server/modules/core/integrated/queue/adapter.ts)
