...
export interface SearchAdapter {
	...
	getTableSchemas?(): Record<string, unknown>;
...
<Callout type="note">
The interface uses `remove()` rather than `delete()`. If your provider SDK calls the operation "delete", map it internally inside `remove()`.
</Callout>
...
type MeiliIndex = {
	...
	addDocuments(documents: MeiliDocument[]): Promise<unknown>;
...
export class MeilisearchAdapter implements SearchAdapter {
	...
	constructor(
		private options: { host: string; apiKey: string },
		private client: MeiliClient,
	) {}

	async initialize(ctx: AdapterInitContext): Promise<void> {
		this.logger = ctx.logger;
		// The typed provider client is injected in the constructor.
...
- [PostgresSearchAdapter](https://github.com/questpie/questpie/blob/main/packages/questpie/src/server/modules/core/integrated/search/adapters/postgres.ts) -- FTS + trigram, facets, access filters, ~1100 lines
- [PgVectorSearchAdapter](https://github.com/questpie/questpie/blob/main/packages/questpie/src/server/modules/core/integrated/search/adapters/pgvector.ts) -- extends Postgres adapter with vector embeddings
- [SearchAdapter interface source](https://github.com/questpie/questpie/blob/main/packages/questpie/src/server/modules/core/integrated/search/types.ts)
