...
Keep `modules.ts` statically discoverable. Codegen imports it before runtime app creation, so codegen-aware modules should not be hidden behind environment checks or configured through `module(options)` calls. Put runtime options in plugin-discovered `config/*.ts` files.
