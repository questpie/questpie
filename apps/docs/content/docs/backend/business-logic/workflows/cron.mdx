...
In long-running Node/Bun workers, `app.queue.listen()` registers the maintenance job schedule with the queue adapter. On Cloudflare Workers, configure a Cron Trigger for `*/5 * * * *`; the Worker scheduled handler publishes the maintenance job, and the maintenance job evaluates workflow-level `cron` definitions.
