...
- the Visual Edit Workspace sends `FULL_RESYNC` after a transition so the iframe discards stale local patches and re-runs the loader against the new stage snapshot
...
- [Visual Edit Workspace](/docs/workspace/live-preview/visual-edit) — Patch-based preview during form editing
