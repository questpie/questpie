...
.preview({
  enabled: true,
  position: "right",
  defaultSize: 50,    // `defaultWidth` accepted as alias
  minSize: 30,        // `minWidth` accepted as alias
  url: ({ record }) => `/barbers/${record.slug}?preview=true`,
})
```

The same `.preview()` config drives both shipped form views — `v.collectionForm` (default split-screen, refreshes on save) and `v.visualEditForm` (opt-in canvas + inspector workspace, field-level patches without a save round-trip). Switch views with `.form(({ v }) => v.visualEditForm({...}))`. See the [Live Preview docs](/docs/workspace/live-preview) for the full guide.
