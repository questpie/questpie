...
const privateCollection = collection("internal-logs")
  .fields(f => ({ ... }))
  .access({
    read: false, // No one can see this in admin
  });

const adminOnlyCollection = collection("settings")
  .fields(f => ({ ... }))
  .access({
...
const sessions = collection("sessions").admin({ hidden: true });
const accounts = collection("accounts").admin({ hidden: true });
const apiKeys = collection("apikeys").admin({ hidden: true });
```

**Draft/staging collections:**
```ts
// Collection not ready for editors yet
const newFeature = collection("beta-feature")
  .fields(f => ({ ... }))
...
const syncedProducts = collection("synced-products")
  .fields(f => ({ ... }))
...
When no `sidebar.ts` convention file is configured, collections appear automatically based on their visibility settings:

```ts
// Collections defined in src/questpie/server/collections/
// posts.ts      → Visible (not hidden)
// pages.ts      → Visible (not hidden)
// sessions.ts   → Hidden via .admin({ hidden: true })
...
With a `sidebar.ts` convention file, you control exactly what appears:
...
```ts
const auditLogs = collection("audit-logs")
  .fields(f => ({
    action: f.text(),
    userId: f.text(),
    timestamp: f.date(),
...
// Use a sidebar.ts convention file to control what appears:
