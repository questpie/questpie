...
flowchart LR
  Block["block(\"hero\")"]
  Fields[".fields({ title, image })"]
  Admin[".admin({ label, icon })"]
  Prefetch[".prefetch({ with: { ... } })"]
  Renderer["HeroRenderer"]
  Props["Receives { values, data }"]
  JSX["Returns JSX"]

  Block --> Fields --> Renderer
  Block --> Admin --> Renderer
  Block --> Prefetch --> Props --> Renderer --> JSX
```
...
- [Defining Blocks](/docs/admin/blocks/defining-blocks) — Server-side block definitions with fields and admin config
- [Renderers](/docs/admin/blocks/renderers) — Client-side React components for rendering blocks
- [Prefetch](/docs/admin/blocks/prefetch) — Prefetching relation data for blocks
- [Live Preview](/docs/admin/live-preview) — Preview annotations and iframe editing flow
