...
```json
{
  "backgroundImage": {
    "id": "uuid-here",
    "url": "http://your-site.com/api/storage/files/...",
    "filename": "hero.jpg",
    "mimeType": "image/jpeg"
  }
}
...
### Circular Dependencies — Solved by Codegen

With the file-convention architecture, circular dependency issues are a thing of the past. Block definitions live in the `blocks/` directory and codegen automatically handles all wiring and type resolution. You never need to manually manage import order or use workarounds.

The `ctx` object in prefetch handlers is fully typed with access to all your collections and globals via the flat context pattern (e.g., `ctx.collections.posts.find(...)`, `ctx.globals.siteSettings.get(...)`). No casting or type workarounds needed.
