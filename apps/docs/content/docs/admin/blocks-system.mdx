...
- `blocks.ts` wants to import `App` from `app.ts` to get typed `ctx.app` in prefetch
- **Circular import!**

### The BaseCMS Pattern (Current Workaround)

Split your app setup into a "base" (everything except blocks) and the final build:
...
export type BaseCMS = (typeof baseCms)["$inferApp"];

// Step 3: Add blocks and build
export const app = baseCms.blocks(blocks).build({ ... });
...
const latestPostsBlock = block("latest-posts")
  .fields(({ f }) => ({
    count: f.number({ label: "Count", default: 3 }),
  }))
  .prefetch(async ({ values, ctx }) => {
    // Cast the untyped ctx.app to your typed API
...
- `BaseCMS` has all the same collections, globals, and APIs as the final `App` — blocks just aren't part of the type
