...
This page covers UI language and messages. For localized content values, see [Content Languages](/docs/collections/localization).
...
Each file default-exports the flat message dictionary for that locale. The filename is the locale key.

<Callout type="warn">
Do not wrap the export in `{ en: ... }`. Each file must default-export the flat dictionary directly, or the messages will not load.
</Callout>
...
<Cards>
  <Card title="Content Languages" href="/docs/collections/localization" description="Configure localized record values" />
  <Card title="Workspace Localization" href="/docs/admin/localization" description="Editor-facing content locale switcher" />
  <Card title="Admin API" href="/docs/reference/admin-api" description="`adminConfig()` reference" />
  <Card title="File Convention" href="/docs/concepts/file-convention" description="`messages/` discovery" />
</Cards>
