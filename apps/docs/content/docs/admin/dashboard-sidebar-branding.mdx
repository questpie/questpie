...
- **Configuring branding on `qa()`** — Branding belongs on the server builder (`.branding()`).
- **Configuring admin UI locale on `qa()`** — Use the server builder's `.adminLocale()` and `useServerTranslations`.
